...
export class MessageRouter {
    ...
    handleTick(payload) {
        ...
        if (chunkX !== null && chunkZ !== null) {
            ...
            const workerControllers = [
                this.game.bakerController,
                this.game.woodcutterController,
                this.game.gardenerController,
                this.game.minerController,
                this.game.stoneMasonController,
                this.game.blacksmithController,
                this.game.ironWorkerController,
                this.game.tileWorkerController,
                this.game.fishermanController
            ];
            for (const controller of workerControllers) {
                controller?.checkProprietorSpawnsOnTick?.(chunkX, chunkZ);
            }
        }
    ...
    handleObjectRemoved(payload) {
        ...
        if (payload.isMobileClaim && payload.claimedBy) {
            ...
            // Check if peer is manning this artillery
            if (peerData?.mannedArtillery?.artilleryId === payload.objectId) {
                console.log(`[MessageRouter] Ignoring removal of ${payload.objectId} - peer ${claimingPeerId} is manning`);
                return;
            }

            // Check if peer has this artillery loaded on their ship
            if (peerData?.loadedArtillery?.some(a => a.artilleryId === payload.objectId)) {
                console.log(`[MessageRouter] Ignoring removal of ${payload.objectId} - peer ${claimingPeerId} has loaded on ship`);
                return;
            }

            // Check if peer has this horse loaded on their ship
        ...
        if (activeAction?.object?.id === payload.objectId) {
            console.log(`[MessageRouter] Cancelling active action - object ${payload.objectId} removed by another player`);
        ...
        if (objectToRemove) {
            ...
            if (modelType === 'dock' && this.game.terrainGenerator) {
                ...
                console.log(`[MessageRouter] Removed dock terrain leveling at (${dockX.toFixed(1)}, ${dockZ.toFixed(1)})`);
        ...
        if (this.game.gameState?.home?.structureId === payload.objectId) {
            console.log(`[MessageRouter] Home structure ${payload.objectId} removed - clearing spawn point`);
        ...
        if (objectToRemove) {
            ...
            if ((modelType === 'pine' || modelType === 'apple') &&
                ...
                console.log(`[FallingTree] Spawning falling ${modelType} tree`);
    ...
    handleBoatSinking(payload) {
        ...
        if (!boatMesh) {
            console.log(`[BoatSinking] Boat ${objectId} not found in registry`);
            return;
        }

        // Skip if already sinking (attacker already started their local animation)
        if (this.game.boatSinkingSystem?.isSinking(objectId)) {
            console.log(`[BoatSinking] Boat ${objectId} already sinking`);
            return;
        }

        console.log(`[BoatSinking] Starting sink animation for ${objectId} (${modelType})`);
    ...
    handleObjectAdded(payload) {
        const { objectId, objectType, position, quality, scale, chunkId,
                ...
                isMobileRelease, isBrownBearStructure, isDeerTreeStructure } = payload;
    ...
    handleLockStolen(payload) {
        ...
        if (this.game?.inventoryUI?.crateUI) {
            ...
            if (crateUI.lockState.held && crateUI.lockState.structureId === structureId) {
                ...
                console.log(`[Lock] Lock stolen for ${structureId}: ${reason}`);
    ...
    handleTreePlanted(payload) {
        ...
        // Set name property for AI controllers (woodcutter, baker, etc.)
        ...
        billboard.userData = {
            ...
            scale: scale,
            // Vegetables/trees: null means no resource display (one-time gather for veggies)
            totalResources: null,
    ...
    handleTreeGrowthComplete(payload) {
        ...
        if (treeObject) {
            ...
            if (modelType === 'apple' && this.gameState) {
                const chunkKey = chunkId.replace('chunk_', '');
    ...
    handleRoadPlaced(payload) {
        ...
        console.log(`Road placed: ${roadId} at position [${position}] rotation ${roadRotation} in ${chunkId} (${material})`);
        ...
        if (this.game.dirtOverlay) {
            ...
            console.log(`[Road] Painted pill road at (${position[0].toFixed(1)}, ${position[2].toFixed(1)}) rotation ${roadRotation} material ${material}`);
        }
        ...
        if (this.game.navigationManager) {
            ...
            if (navMap) {
                ...
                console.log(`[Nav] Added road at (${position[0].toFixed(1)}, ${position[2].toFixed(1)}) with radius 1.25`);
            }
        }
    ...
    handleStructureDamaged(payload) {
        ...
        if (structure) {
            ...
            const mannedArtillery = this.gameState.artilleryManningState?.mannedArtillery;
            const isSelfWear = mannedArtillery && mannedArtillery.userData?.objectId === structureId;

            if (isSelfWear) {
                // Self-wear from firing - no effects or toast needed
                console.log(`[Artillery Wear] ${structureId}: Durability ${currentDurability?.toFixed(1) || 'N/A'}`);
                return;
            }
            ...
            console.log(`[Structure Damaged] ${structureId}: Durability ${currentDurability?.toFixed(1) || 'N/A'}`);
    ...
    handleStructureSoldToProprietor(payload) {
        ...
        if (structure) {
            ...
            console.log(`[Proprietor] Structure ${structureId} sold to NPC`);
        } else {
    ...
    handleConstructionMaterialsUpdated(payload) {
        ...
        if (site) {
            ...
            console.log(`[Construction] Materials synced for ${constructionId}`);
        }
    }
    ...
    handleKicked(payload) {
        console.log('Kicked from server:', payload.reason);
    ...
    handleAdminBroadcast(payload) {
        console.log('Admin broadcast:', payload.message);
    ...
    handlePositionRequest(payload) {
        ...
        if (mobileState?.isActive && waterVehicleSpawnBlock.includes(mobileState.entityType)) {
            console.log(`[MessageRouter] Position request denied - player on water vehicle (phase: ${mobileState.phase})`);
            this.networkManager.sendMessage('position_response', {
                requestId,
                unavailable: true,
        ...
        if (terrainHeight !== undefined && terrainHeight < LAND_THRESHOLD) {
            console.log(`[MessageRouter] Position request denied - player in water (height: ${terrainHeight.toFixed(1)})`);
            this.networkManager.sendMessage('position_response', {
                requestId,
                unavailable: true,
                reason: 'in_water'
    ...
    checkStructureDecay() {
        ...
        const structureTypes = new Set(['house', 'crate', 'tent', 'outpost', 'campfire',
                                        'market', 'dock', 'tileworks', 'ship', 'boat', 'sailboat', 'ship2', 'horse', 'cart', 'artillery',
                                        'fisherman', 'ironworks', 'blacksmith', 'bakery', 'gardener', 'miner', 'woodcutter', 'stonemason', 'bearden']);
        ...
        for (const objectId of this.gameState.decayableStructures) {
            ...
            if (modelType && structureTypes.has(modelType)) {
                ...
                // Check for dying structure effect (smoke + sound when <= 1 hour remaining)
                const hoursUntilRuin = this.getHoursUntilRuin(object.userData);
                const ONE_HOUR = 1.0;

                if (hoursUntilRuin <= ONE_HOUR && hoursUntilRuin > 0 && !object.userData.isRuin) {
                    ...
                    // Structure is healthy or already dead - remove effect if present
