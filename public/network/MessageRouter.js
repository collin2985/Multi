...
export class MessageRouter {
    ...
    handleRoadPlaced(payload) {
        ...
        if (this.game.navigationManager) {
            ...
            if (navMap) {
                navMap.addRoad(position[0], position[2], 1.0);
                console.log(`[Nav] Added road at (${position[0].toFixed(1)}, ${position[2].toFixed(1)}) with radius 1.0`);
            }
        }
    ...
    handleDockShipSpawned(payload) {
        ...
        // Check if baker should spawn at nearby bakeries
        if (this.game.bakerController) {
            this.game.bakerController.checkBakerSpawn({
                dockId,
                dockPosition,
                chunkId
            });
        ...
        // Check if gardener should spawn at nearby gardener buildings
        if (this.game.gardenerController) {
        ...
        // Check if woodcutter should spawn at nearby woodcutter buildings
        if (this.game.woodcutterController) {
            this.game.woodcutterController.checkWoodcutterSpawn({
                dockId,
                dockPosition,
                chunkId
            });
        ...
        // Check if miner should spawn at nearby miner buildings
        if (this.game.minerController) {
            this.game.minerController.checkMinerSpawn({
                dockId,
                dockPosition,
                chunkId
            });
        ...
        // Check if stonemason should spawn at nearby stonemason buildings
        if (this.game.stoneMasonController) {
            this.game.stoneMasonController.checkStoneMasonSpawn({
                dockId,
                dockPosition,
                chunkId
            });
        ...
        // Check if iron worker should spawn at nearby ironworks
        if (this.game.ironWorkerController) {
            this.game.ironWorkerController.checkIronWorkerSpawn({
                dockId,
                dockPosition,
                chunkId
            });
        ...
        // Check if tile worker should spawn at nearby tileworks
        if (this.game.tileWorkerController) {
            this.game.tileWorkerController.checkTileWorkerSpawn({
                dockId,
                dockPosition,
                chunkId
            });
        ...
        // Check if blacksmith should spawn at nearby blacksmith buildings
        if (this.game.blacksmithController) {
            this.game.blacksmithController.checkBlacksmithSpawn({
                dockId,
                dockPosition,
                chunkId
            });
        ...
        // Check if fisherman should spawn at nearby fisherman structures
        if (this.game.fishermanController) {
            this.game.fishermanController.checkFishermanSpawn({
                dockId,
                dockPosition,
                chunkId
            });
    ...
    handlePositionRequest(payload) {
        ...
        // Check 3: Player is on a boat (block spawn - boats move unpredictably)
        // Allow spawn on horses and carts (they're stable enough)
        const mobileState = this.gameState.mobileEntityState;
        if (mobileState?.isActive && mobileState.entityType === 'boat') {
            console.log(`[MessageRouter] Position request denied - player on boat (phase: ${mobileState.phase})`);
            this.networkManager.sendMessage('position_response', {
                requestId,
                unavailable: true,
