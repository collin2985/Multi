...
export class MessageRouter {
    ...
    handleObjectRemoved(payload) {
        ...
        if (this.game.gameState) {
            ...
            if (chunkKey) {
                ...
                // Market structures: unregister for Baker AI
    ...
    handleCrateInventoryUpdated(payload) {
        ...
        if (crateObject) {
            ...
            // Control smoke for ironworks based on firewood presence
            if (crateObject.userData.modelType === 'ironworks') {
                const smokeEffect = this.game.effectManager.smokeEffects.get(crateId);
                if (smokeEffect) {
                    const hasFirewood = inventory.items.some(item =>
                        item.type && item.type.endsWith('firewood') && item.durability > 0
                    ...
                    if (hasFirewood && !smokeEffect.active) {
                        smokeEffect.start();
                        console.log(`Started ironworks smoke for ${crateId} (firewood added)`);
                    } else if (!hasFirewood && smokeEffect.active) {
                        smokeEffect.stop();
                        console.log(`Stopped ironworks smoke for ${crateId} (firewood removed)`);
                    }
                }
            }

            // Control smoke for blacksmith based on firewood presence
            if (crateObject.userData.modelType === 'blacksmith') {
                const smokeEffect = this.game.effectManager.smokeEffects.get(crateId);
                if (smokeEffect) {
                    const hasFirewood = inventory.items.some(item =>
                        item.type && item.type.endsWith('firewood') && item.durability > 0
                    ...
                    if (hasFirewood && !smokeEffect.active) {
                        smokeEffect.start();
                        console.log(`Started blacksmith smoke for ${crateId} (firewood added)`);
                    } else if (!hasFirewood && smokeEffect.active) {
                        smokeEffect.stop();
                        console.log(`Stopped blacksmith smoke for ${crateId} (firewood removed)`);
                    }
                }
            }

            // Control smoke for bakery based on firewood presence
            if (crateObject.userData.modelType === 'bakery') {
                const smokeEffect = this.game.effectManager.smokeEffects.get(crateId);
                if (smokeEffect) {
                    const hasFirewood = inventory.items.some(item =>
                        item.type && item.type.endsWith('firewood') && item.durability > 0
                    ...
                    if (hasFirewood && !smokeEffect.active) {
                        smokeEffect.start();
                        console.log(`Started bakery smoke for ${crateId} (firewood added)`);
                    } else if (!hasFirewood && smokeEffect.active) {
                        smokeEffect.stop();
                        console.log(`Stopped bakery smoke for ${crateId} (firewood removed)`);
                    }
                }
            }
    ...
    /**
     * Handle NPC collect response (firewood, tarts)
     */
