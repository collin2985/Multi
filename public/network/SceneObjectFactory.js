...
export class SceneObjectFactory {
    ...
    addObjectFromChange(change) {
        ...
        if (existingObject) {
            ...
            if ((structureType === 'crate' || structureType === 'tent' || structureType === 'house' || structureType === 'garden' || structureType === 'market' || structureType === 'campfire' || structureType === 'fisherman' || structureType === 'tileworks' || structureType === 'ironworks' || structureType === 'blacksmith' || structureType === 'bakery') && change.inventory) {
    ...
    createObjectInScene(data, chunkKey) {
        ...
        if (data.isGrowing && isTree) {
            ...
            billboard.name = visualType;  // Set name for AI controllers (woodcutter, baker, etc.)
            billboard.userData.yOffset = config.yOffset;  // Store for GUI adjustment
            billboard.userData.modelType = visualType;  // Store tree type (no prefix - ID already has 'planted_')
        ...
        if (objectInstance) {
            ...
            // Handle crate/tent/house/garden/market/campfire/tileworks/apple/ironworks/blacksmith/bakery inventory
            const structureType = data.name || data.objectType;
            if ((structureType === 'crate' || structureType === 'tent' || structureType === 'house' || structureType === 'garden' || structureType === 'market' || structureType === 'campfire' || structureType === 'tileworks' || structureType === 'apple' || structureType === 'fisherman' || structureType === 'ironworks' || structureType === 'blacksmith' || structureType === 'bakery') && data.inventory) {
            ...
            if (structureType === 'fisherman') {
                ...
                if (objectInstance.userData.inventory) {
                    ...
                    if (hasFirewood) {
                        ...
                        if (smokeEffect && !smokeEffect.active) {
                            smokeEffect.start();
                            console.log(`Started fisherman smoke for ${objectInstance.userData.objectId} (initial firewood present)`);
                        }
                    }
                }
            }
