...
export class SceneObjectFactory {
    ...
    addObjectFromChange(change) {
        ...
        if (objectType === 'road' || change.isRoad) {
            const roadRadius = 0.75;
            const materialType = change.materialType || 'limestone';

            // Paint road onto dirt overlay texture
            if (this.game.dirtOverlay) {
                this.game.dirtOverlay.paintRoadImmediate(change.position[0], change.position[2], roadRadius, materialType);
            }
    ...
    createObjectInScene(data, chunkKey) {
        ...
        // Debug log for horse spawning
        if (objectType === 'horse') {
            console.log('[Horse Debug] createObjectInScene called:', { objectType, chunkKey, data });
        }
        ...
        if (objectType === 'road' || data.isRoad) {
            ...
            // Paint road onto dirt overlay texture
            if (this.game.dirtOverlay) {
                this.game.dirtOverlay.paintRoadImmediate(data.position[0], data.position[2], roadRadius, materialType);
            }
        ...
        // Force horse Y to terrain height (server may send incorrect Y)
        if (objectType === 'horse' && this.game.terrainGenerator) {
        ...
        if (data.isGrowing && isTree) {
            ...
            const config = billboardConfig[visualType] || billboardConfig.oak;
            ...
            // Debug log for horse
            if (objectType === 'horse') {
                console.log('[Horse Debug] createInstance result:', objectInstance ? 'SUCCESS' : 'FAILED (null)');
            }
        }
