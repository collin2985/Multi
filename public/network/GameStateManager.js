...
export class GameStateManager extends EventEmitter {
    ...
    processP2PMessage(message, fromPeer) {
        ...
        // Store username from player_tick even before avatar exists (fixes race condition)
        // This ensures peerData.username is set before avatar creation so nametag is correct
        if (message.type === 'player_tick' && peerData && message.u) {
            if (peerData.username !== message.u) {
                peerData.username = message.u;
            }
            // Also store faction for enemy detection
