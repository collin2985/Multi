...
export class GameStateManager extends EventEmitter {
    ...
    processP2PMessage(message, fromPeer) {
        ...
        // Bear P2P messages
        if (message.type === 'bear_spawn') {
            if (this.game?.bearController) {
                this.game.bearController.handleSpawnMessage(message);
            }
            return;
        }
        if (message.type === 'bear_state') {
            if (this.game?.bearController) {
                this.game.bearController.handleStateMessage(message);
            }
            return;
        }
        if (message.type === 'bear_sync') {
            if (this.game?.bearController && message.bear) {
                this.game.bearController.syncBearFromPeer(message.bear);
            }
            return;
        }
        if (message.type === 'bear_death') {
            if (this.game?.bearController) {
                this.game.bearController.handleDeathMessage(message);
            }
            return;
        }
        if (message.type === 'bear_harvested') {
            if (this.game?.bearController) {
                this.game.bearController.handleHarvestMessage(message);
            }
            return;
        }
        if (message.type === 'bear_attack') {
            if (this.game?.bearController) {
                this.game.bearController.handleAttackMessage(message);
            }
            return;
        }
        if (message.type === 'bear_despawn') {
            if (this.game?.bearController) {
                this.game.bearController.handleDespawnMessage(message);
            }
            return;
        }
        ...
        switch (message.type) {
            ...
            case 'player_shoot_bear':
                this.handlePlayerShootBear(message.payload, fromPeer, avatar);
                break;
    ...
    handlePlayerShootDeer(payload, fromPeer, avatar) {
        ...
        // If the shot hit and THIS client is authority, apply death
        if (payload.isHit && payload.chunkKey) {
            if (this.game?.deerController?.isAuthority(payload.chunkKey)) {
                this.game.deerController.killDeer(payload.chunkKey, fromPeer);
            }
        }
    }

    /**
     * Handle player shooting at a bear
     * @private
     */
    handlePlayerShootBear(payload, fromPeer, avatar) {
        // Play rifle sound for peer's player
        if (avatar && this.audioManager) {
            this.audioManager.playPositionalSound('rifle', avatar);
        }
        ...
        // If the shot hit and THIS client is authority, apply death
        if (payload.isHit && payload.chunkKey) {
            if (this.game?.bearController?.isAuthority(payload.chunkKey)) {
                this.game.bearController.killBear(payload.chunkKey, fromPeer);
            }
        }
    }
    ...
    handlePlayerRespawn(payload, fromPeer, avatar) {
        ...
        // Reset hasRifle state (will be updated by next tick message)
        const peerData = this.peerGameData.get(fromPeer);
        if (peerData) {
    ...
    updatePeerChunkRegistry(peerId, peerData, position) {
        ...
        // Check if peer left any AI's authority region
        if (this.game?.banditController && oldChunkKey) {
            this.game.banditController.onPeerChunkChanged(peerId, oldChunkKey, newChunkKey);
        }
        if (this.game?.deerController && oldChunkKey) {
            this.game.deerController.onPeerChunkChanged(peerId, oldChunkKey, newChunkKey);
        }
        if (this.game?.bearController && oldChunkKey) {
            this.game.bearController.onPeerChunkChanged(peerId, oldChunkKey, newChunkKey);
        }
    }
