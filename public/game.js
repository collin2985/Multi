...
class MultiplayerGame {
    ...
    async initializeWithSpawn(spawnX, spawnZ) {
        ...
        this.initializer.setupComponents(spawnX, spawnZ);
        this.setupGameLoop();
    ...
    setupUI() {
        ui.initializeUI({
            ...
            onHarvestBear: () => {
                if (this.gameState.nearestBearCorpse && !this.gameState.isMoving && !this.gameState.activeAction) {
                    this.startHarvestBearAction(this.gameState.nearestBearCorpse);
                }
            },
            onRepairStructure: () => {
    ...
    async start() {
        ...
        // Switch loading screen to chunk loading phase
    ...
    /**
     * Start bear harvesting action (6 seconds, like vegetables)
     */
    startHarvestBearAction(bearCorpse) {
        console.log('[DEBUG] startHarvestBearAction called with:', bearCorpse);
        // Guard: Prevent starting new action if one is already in progress
        if (this.gameState.activeAction) {
            return;
        }

        if (!this.playerObject || !bearCorpse) {
            console.log('[DEBUG] startHarvestBearAction early return - playerObject:', !!this.playerObject, 'bearCorpse:', !!bearCorpse);
            return;
        }
        ...
        // Set active action
        this.gameState.activeAction = {
            bearCorpse: bearCorpse,
            startTime: Date.now(),
            duration: CONFIG.ACTIONS.BUILD_DURATION,  // 6 seconds
            actionType: 'harvest_bear'
        };
        console.log('[DEBUG] activeAction set:', this.gameState.activeAction);
        console.log('[DEBUG] stored bearCorpse:', this.gameState.activeAction.bearCorpse);
        ...
        ui.updateStatus('Harvesting bear...');
    }

    /**
     * Complete bear harvesting - add meat to inventory
     */
    completeHarvestBearAction(bearCorpse) {
        console.log('[DEBUG] completeHarvestBearAction called', { bearCorpse });
        if (!bearCorpse) {
            console.log('[DEBUG] bearCorpse is null/undefined!');
            return;
        }
        console.log('[DEBUG] chunkKey:', bearCorpse.chunkKey);
        console.log('[DEBUG] bearController exists:', !!this.bearController);

        // Get quality from chunk (like mushrooms/vegetables)
        const worldSeed = TERRAIN_CONFIG.SEED || 12345;
        const quality = QualityGenerator.getQuality(worldSeed, bearCorpse.chunkX, bearCorpse.chunkZ, 'bear');
        console.log('[DEBUG] quality:', quality, 'worldSeed:', worldSeed);
        ...
        if (this.tryAddItemToInventory(rawMeat)) {
            ...
            // Remove bear from world (broadcasts to peers)
            const result = this.bearController.harvestBear(bearCorpse.chunkKey);
            console.log('[DEBUG] harvestBear result:', result);
        } else {
            console.log('[DEBUG] Inventory full!');
            ui.showToast('Inventory full!', 'warning');
        }
    ...
    setupGameLoop() {
        ...
        this.gameLoop.onUpdate((deltaTime, now) => {
            ...
            // One-time initial spawn height adjustment
            if (this.needsInitialHeightAdjustment && this.terrainGenerator && this.playerObject) {
                const terrainHeight = this.getGroundHeightAt(this.playerObject.position.x, this.playerObject.position.z);
                if (terrainHeight !== undefined && terrainHeight !== null) {
                    const adjustedHeight = terrainHeight + 0.03;
                    this.playerObject.position.y = adjustedHeight;
                    // Also update PlayerController's targetY to prevent sinking on first movement
                    if (this.playerController) {
                        this.playerController.targetY = adjustedHeight;
                    }
                    this.needsInitialHeightAdjustment = false;
                }
            }
            ...
            if (cartState.isAttached && cartState.attachedCart) {
                ...
                const pullDistance = Math.sqrt(pullDirX * pullDirX + pullDirZ * pullDirZ);

                // Only apply physics if tether is taut (distance > threshold)
                if (pullDistance > CART.TETHER_LENGTH + CART.MIN_MOVE_THRESHOLD) {
            ...
            this.playerCombat.updateShooting(
                ...
                (target, isHit, playerPos) => {
                    ...
                    // Notify bears of player gunshot (they flee from gunshots)
                    if (this.bearController) {
                        this.bearController.registerGunshot(playerPos.x, playerPos.z);
                    }
                    ...
                    if (target.isDeer) {
                        ...
                        if (isHit && target.isLocal) {
                            this.deerController.killDeer(target.chunkKey, this.gameState.clientId);
                        }
                    ...
                    // Handle bear shots separately
                    if (target.isBear) {
                        // Broadcast bear shoot event
                        this.networkManager.broadcastP2P({
                            type: 'player_shoot_bear',
                            payload: {
                                position: playerPos.toArray(),
                                chunkKey: target.chunkKey,
                                isHit: isHit,
                        ...
                        // Apply hit if local authority
                        if (isHit && target.isLocal) {
                            this.bearController.killBear(target.chunkKey, this.gameState.clientId);
                        }
                        // If not local authority, peer will handle kill from broadcast
            ...
            if (this.combatHUD) {
                if (inCombat && target) {
                    ...
                    // Determine target type for HUD display
                    const targetType = target.isDeer ? 'deer' : (target.isBear ? 'bear' : 'bandit');

                    this.combatHUD.update(true, distance, shootingRange, hitChance, ammoCount, hasRifle, targetType);
                } else {
            ...
            // Update bear AI
            if (this.bearController) {
                this.bearController.update(deltaTime);

                // Check for nearby bear corpse (for harvesting)
                const playerPos = this.playerObject.position;
                this.gameState.nearestBearCorpse = this.bearController.getNearestHarvestableBear(
                    playerPos.x, playerPos.z
                );
            }
            ...
            if (this.gameLoop.frameCount % 6 === 0 && this.playerController && this.playerObject) {
                ...
                ui.updatePlayerSpeed(speedInfo, this.gameState.isMoving);
            ...
            if (playerPos && this.dirtOverlay) {
                ...
                // Always update shader uniforms to stay in sync with meshWorldOffset
                if (this.clipmap) {
    ...
    runPeriodicChecks(now) {
        if (now - this.gameState.lastChunkUpdateTime > CONFIG.GAME_LOOP.CHUNK_UPDATE_INTERVAL) {
            if (this.chunkManager.updatePlayerChunk(this.playerObject.position.x, this.playerObject.position.z)) {
                ...
                // Update tree scene membership for physics radius changes
                this.updateTreeSceneMembership(currentPlayerChunkX, currentPlayerChunkZ, lastChunkX, lastChunkZ);

                // Update navigation maps for the 3x3 grid around player
                if (this.chunkManager) {
                    this.chunkManager.updateNavMapsAroundPlayer(currentPlayerChunkX, currentPlayerChunkZ, lastChunkX, lastChunkZ);
                }

                // Update AI tent presence and authority for local player chunk change
                if (this.banditController) {
                    this.banditController.updateTentPresence(currentPlayerChunkX, currentPlayerChunkZ);

                    // Check authority changes for local player
                    const oldChunkKey = `${lastChunkX},${lastChunkZ}`;
                    const newChunkKey = `${currentPlayerChunkX},${currentPlayerChunkZ}`;
                    this.banditController.onPeerChunkChanged(this.gameState.clientId, oldChunkKey, newChunkKey);
                }

                // Update trappers for new 3x3 area
    ...
    updateTreeSceneMembership(currentChunkX, currentChunkZ, lastChunkX, lastChunkZ) {
        ...
        const STRUCTURE_TYPES = ['tent', 'campfire', 'outpost', 'house', 'crate', 'garden', 'market', 'dock', 'tileworks', 'horse', 'cart', 'boat', 'ship'];
     ...
     * Attaches a nearby cart to the player for towing
     */
    handleAttachCart() {
        const nearestCart = this.gameState.nearestTowableEntity;
        if (!nearestCart || nearestCart.type !== 'cart') return;

        const cart = nearestCart.object;
        const cartState = this.gameState.cartAttachmentState;
     ...
     * Releases the currently towed cart
     * Now properly awaits crate unload if one is loaded
     */
    ...
    startBoarding(mobileEntity) {
        ...
        if (mobileEntity.type === 'boat') {
            ...
            if (this.physicsManager) {
                ...
                // Prime the character controller with zero movement to initialize Rapier's
                // internal collision state. Without this, the first real movement check
                // may report pre-existing contacts as blocking collisions.
                this.physicsManager.computeCharacterMovement(entityId, { x: 0, y: 0, z: 0 });
            }
        }
    ...
    completeDisembark() {
        ...
        // Send release message to server (adds entity back at new position)
        ...
        // Ensure player Y is at terrain height (prevent underground clipping)
        if (this.terrainGenerator) {
            const terrainY = this.terrainGenerator.getWorldHeight(
                this.playerObject.position.x,
                this.playerObject.position.z
            );
            // Set player Y to exact terrain height
            this.playerObject.position.y = terrainY + 0.03;

            // Update PlayerController's targetY to prevent wrong lerp on next frame
            if (this.playerController) {
        ...
        if (entityType === 'boat') {
            // Re-register boat with AnimationSystem for wave animation
            if (this.animationSystem) {
                this.animationSystem.registerShip(entity);
            }
        } else if (entityType === 'horse') {
            // Cleanup horse animation state
            if (state.entityWalkAction) {
                state.entityWalkAction.stop();
            }
            ...
            // Remove horse character controller (collision detection)
            if (this.physicsManager) {
                this.physicsManager.removeCharacterController(entityId);
            }
            // Restore player walk animation
            if (this.animationAction) {
                this.animationAction.play();
            }

            // Auto-release cart when dismounting from horse
            if (this.gameState.cartAttachmentState.isAttached) {
                this.handleReleaseCart();
            }
        }
        ...
        // Immediately check proximity to re-detect the dismounted entity
        // (Don't clear nearestMobileEntity - let proximity detection handle it)
        this.checkProximityToObjects();

        const statusMsg = entityType === 'horse' ? 'Dismounted' : 'Disembarked';
        ui.updateStatus(statusMsg);
    }
...
modelManager.loadAllModels().then(async () => {
    ...
    game.spawnScreen = new SpawnScreen(
        ...
        async (spawnType, data) => {
            ...
            if (spawnType === 'home') {
                ...
                const result = findValidSpawnPoint(spawnTerrainGenerator, 0, initialZ, minZ, maxZ, bc);
                spawnX = result.x;
                spawnZ = result.z;
            }
    ...
    game.loginModal.onAuthComplete = async (authType, data) => {
        ...
        if (authType === 'login' && data.playerData) {
            ...
            // Returning player - show spawn screen with options
            game.spawnScreen.show({ isRespawn: false });
        } else {
            // Guest or new account - they already picked faction
            // Go straight to random spawn in their faction zone
            ...
            // Find a valid spawn point above water
            const initialZ = minZ + Math.random() * (maxZ - minZ);
            const result = findValidSpawnPoint(spawnTerrainGenerator, 0, initialZ, minZ, maxZ);

            // Give starting inventory (this is always a random spawn)
            game.gameState.inventory.items = game.gameState.getDefaultInventoryItems();
            game.gameState.slingItem = game.gameState.getDefaultSlingItem();

            // Complete game initialization with spawn point
            await game.initializeWithSpawn(result.x, result.z);
