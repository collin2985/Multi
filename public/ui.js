...
function initDOMCache() {
    ...
    domCache.returnToHelmBtn = document.getElementById('returnToHelmBtn');
    // External gunner buttons
    domCache.takeHelmBtn = document.getElementById('takeHelmBtn');
    domCache.switchCannonBtn = document.getElementById('switchCannonBtn');
...
export const ui = {
    ...
    updateButtonStates(isInChunk, nearestObject, hasAxe, hasSaw, isOnCooldown = false, nearestConstructionSite = null, isMoving = false, nearestStructure = null, hasHammer = false, nearWater = false, hasFishingNet = false, onGrass = false, mushroomAvailable = false, vegetableSeedsAvailable = false, limestoneAvailable = false, seedsAvailable = false, seedTreeType = null, isClimbing = false, occupiedOutposts = null, vegetablesGatherAvailable = false, activeAction = null, nearestMobileEntity = null, mobileEntityState = null, canDisembark = false, nearestDeerCorpse = null, nearestBrownbearCorpse = null, nearestTowableEntity = null, cartAttachmentState = null, nearestLoadableCrate = null, crateLoadState = null, artilleryAttachmentState = null, nearestMannableArtillery = null, artilleryManningState = null, grassQualityInfo = null, mobileEntitySystem = null, nearestLoadableArtillery = null, artilleryShipLoadState = null, nearestSwitchableMobileEntity = null, nearestLoadableHorse = null, horseShipLoadState = null) {
        ...
        if (isMounted) {
            ...
            const isOnBoat = waterVehicleTypes.includes(mobileEntityState?.entityType);
        ...
        const isOnBoatForFishing = mobileEntityState?.isActive && mobileEntityState?.phase === 'piloting' && waterVehiclesFishing.includes(mobileEntityState?.entityType);
        ...
        if (repairStructureBtn && !isMounted && !isManningArtillery) {
            ...
            // Horse feeding requires vines instead of hammer
            const isHorse = structureType === 'horse';
            const hasVines = window.game?.gameState?.inventory?.items?.some(item => item.type === 'vines') || false;
            const hasRequiredTool = isHorse ? hasVines : hasHammer;
        ...
        if (demolishStructureBtn && !isMounted && !isManningArtillery) {
            ...
            if (isDemolishableStructure) {
                ...
                // Sold structures cannot be demolished by anyone
                const isSoldToProprietor = nearestStructure.userData?.proprietor === 'npc';

                const canDemolish = hasHammer && !isMoving && (!isOwnerProtected || isOwner) && !isSoldToProprietor;
        ...
        if (mobileBtn && !isManningArtillery) {
            ...
            if (phase === 'boarding' || phase === 'disembarking') {
            ...
            } else if (phase === 'piloting' || phase === 'crewing') {
                // Piloting/Riding or Crewing (external gunner) - check for switchable boat first, then exit option
                const waterVehicleTypes = ['boat', 'sailboat', 'ship2'];
                const isOnWaterVehicle = waterVehicleTypes.includes(entityType);

                if (phase === 'piloting' && isOnWaterVehicle && nearestSwitchableMobileEntity && !isMoving) {
                    // Near another boat while piloting - show Switch button (not for crewing)
                ...
                const nearObject = nearestMobileEntity.object;
                const isGunnerBoarding = nearObject?.userData?._isGunnerBoardingAvailable;
                const config = window.game?.mobileEntitySystem?.getConfig(nearType);

                // Determine button label
                let buttonLabel;
                if (isGunnerBoarding) {
                    buttonLabel = 'Board as Gunner';
                } else if (config?.buttonLabel) {
                    buttonLabel = config.buttonLabel;
                } else if (nearType === 'horse') {
        ...
        const isPilotingBoat = mobileEntityState?.phase === 'piloting' &&
            ['boat', 'sailboat', 'ship2'].includes(mobileEntityState?.entityType);
        ...
        const boatEntityType = mobileEntityState?.entityType;
        ...
        if (loadCrateBtn) {
            ...
            const cartCanLoad = isCartAttached && !isCrateLoaded;
        ...
        if (unloadCrateBtn) {
            ...
            // 1. Player has CART attached with crate OR piloting stopped boat with crates
            // 2. Not moving (!isMoving)
            const cartCanUnload = isCartAttached && isCrateLoaded;
            const boatCanUnload = boatStopped && boatHasCrates;
        ...
        // Check if piloting a stopped ship2 (for artillery loading)
        const isPilotingShip2 = mobileEntityState?.phase === 'piloting' &&
            mobileEntityState?.entityType === 'ship2';
        ...
        const shipIsTowed = mobileEntityState?.currentEntity?.userData?.isBeingTowed;
        ...
        if (unloadShipArtilleryBtn) {
            ...
            const canUnloadArtillery = ship2Stopped && ship2HasArtillery && !shipIsTowed;
        ...
        if (unloadShipHorseBtn) {
            ...
            // 3. Ship is not being towed
            const canUnloadHorse = ship2Stopped && ship2HasHorses && !shipIsTowed;
            if (canUnloadHorse) {
        ...
        const portArtillery = loadedArtillery.find(a => a.slotIndex === 0);
        const starboardArtillery = loadedArtillery.find(a => a.slotIndex === 1);

        if (manPortCannonBtn) {
            // Show "Man Port Cannon" when:
            // 1. Piloting stopped ship2
            // 2. Port artillery is loaded (slot 0)
            // 3. Ship is not being towed
            const canManPort = ship2Stopped && portArtillery && !shipIsTowed;
            if (canManPort) {
        ...
        if (manStarboardCannonBtn) {
            ...
            // 1. Piloting stopped ship2
            // 2. Starboard artillery is loaded (slot 1)
            // 3. Ship is not being towed
            const canManStarboard = ship2Stopped && starboardArtillery && !shipIsTowed;
            if (canManStarboard) {
        ...
        // Return to Helm button (visible when pilot switched to gunner, NOT for external gunners)
        const returnToHelmBtn = domCache.returnToHelmBtn;
        if (returnToHelmBtn) {
            // Show "Return to Helm" when:
            // 1. Currently manning artillery
            // 2. The manned artillery is mounted on a ship2
            // 3. Ship is stopped (so player can switch positions)
            // 4. Player is the PILOT who switched (phase === 'piloting'), NOT an external gunner (phase === 'crewing')
            const mannedArtillery = artilleryManningState?.mannedArtillery;
            const isOnShip = mannedArtillery?.parent?.userData?.modelType === 'ship2';
            // Check if ship is stopped (need velocity from parent ship)
            const shipEntity = mannedArtillery?.parent;
            const shipVelocity = shipEntity?.userData?.velocity || 0;
            const shipStopped = Math.abs(shipVelocity) < 0.0001;
            // Only show for pilot-who-switched (phase === 'piloting'), not external gunner (phase === 'crewing')
            const isPilotWhoSwitched = mobileEntityState?.phase === 'piloting' &&
                                       mobileEntityState?.myShipPosition?.includes('Gunner');
            const canReturnToHelm = isManningArtillery && isOnShip && shipStopped && isPilotWhoSwitched;
            if (canReturnToHelm) {
                returnToHelmBtn.style.display = 'inline-block';
                returnToHelmBtn.disabled = false;
                returnToHelmBtn.style.opacity = '1';
                returnToHelmBtn.style.cursor = 'pointer';
            } else {
                returnToHelmBtn.style.display = 'none';
            }
        }

        // Take Helm button (external gunner only, when helm is unoccupied and ship stopped)
        const takeHelmBtn = domCache.takeHelmBtn;
        if (takeHelmBtn) {
            // Show "Take Helm" when:
            // 1. External gunner (phase === 'crewing', not pilot-who-switched)
            // 2. Manning ship-mounted artillery
            // 3. Ship is stopped
            // 4. Helm is unoccupied
            const isExternalGunner = mobileEntityState?.phase === 'crewing' && isManningArtillery;
            const shipId = mobileEntityState?.entityId;
            const helmOccupied = shipId && window.game?.mobileEntitySystem?.isOccupied(shipId);
            const mannedArtillery = artilleryManningState?.mannedArtillery;
            const shipEntity = mannedArtillery?.parent;
            const shipVelocity = shipEntity?.userData?.velocity || 0;
            const shipStopped = Math.abs(shipVelocity) < 0.0001;

            const canTakeHelm = isExternalGunner && !helmOccupied && shipStopped;
            if (canTakeHelm) {
        ...
        // Switch Cannon button (when other cannon is unoccupied and ship stopped)
        const switchCannonBtn = domCache.switchCannonBtn;
        if (switchCannonBtn) {
            // Show "Switch Cannon" when:
            // 1. Manning ship-mounted artillery
            // 2. Ship is stopped
            // 3. Other cannon slot is unoccupied
            const isShipMounted = artilleryManningState?.isShipMounted;
            const mannedArtillery = artilleryManningState?.mannedArtillery;
            const shipEntity = mannedArtillery?.parent;
            const shipVelocity = shipEntity?.userData?.velocity || 0;
            const shipStopped = Math.abs(shipVelocity) < 0.0001;

            // Check if other cannon is available
            let hasOtherCannon = false;
            if (isManningArtillery && isShipMounted && shipStopped) {
                const artilleryShipState = window.game?.gameState?.artilleryShipLoadState;
                const loadedArtillery = artilleryShipState?.loadedArtillery || [];
                const currentArtilleryId = artilleryManningState?.artilleryId;
                hasOtherCannon = loadedArtillery.some(a =>
                    a.artilleryId !== currentArtilleryId &&
                    !window.game?.mobileEntitySystem?.isOccupied(a.artilleryId)
                );
            }

            if (isManningArtillery && isShipMounted && shipStopped && hasOtherCannon) {
                switchCannonBtn.style.display = 'inline-block';
                switchCannonBtn.disabled = false;
                switchCannonBtn.style.opacity = '1';
                switchCannonBtn.style.cursor = 'pointer';
            } else {
                switchCannonBtn.style.display = 'none';
            }
        }
        ...
        if (leaveArtilleryBtn) {
            ...
            // Ship-mounted gunners use Disembark/Take Helm/Switch Cannon buttons instead
            const isShipMounted = artilleryManningState?.isShipMounted;
            if (isManningArtillery && !isShipMounted) {
    ...
    updateMilitiaButton(nearestStructure, isMoving = false) {
        ...
        if (nearestStructure && !isMoving && window.game?.gameState) {
            ...
            // Check if structure already has militia (entity exists)
            let hasMilitia = false;
            if (window.game?.banditController?.entities) {
                hasMilitia = window.game.banditController.entities.has(structureId);
            }

            // For outposts: also check not occupied by player and not occupied by militia tracking
            let outpostOccupied = false;
            if (isOutpost) {
                outpostOccupied = window.game?.occupiedOutposts?.has(structureId) ||
                                  window.game?.outpostMilitia?.has(structureId);
            }
            ...
            if (isArtillery) {
                ...
                const attachState = window.game?.gameState?.artilleryAttachmentState;
                const manningState = window.game?.gameState?.artilleryManningState;
                ...
                if (window.game?.artilleryMilitia?.has(artilleryId)) {
                    artilleryAvailable = false;
                }
            }
    ...
    initializeUI(callbacks) {
        ...
        setupButton(document.getElementById('returnToHelmBtn'), () => {
            if (callbacks.onReturnToHelm) {
                callbacks.onReturnToHelm();
            }
        });
        ...
        setupButton(document.getElementById('switchCannonBtn'), () => {
            if (callbacks.onSwitchCannon) {
                callbacks.onSwitchCannon();
            }
        });
