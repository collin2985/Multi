...
function initDOMCache() {
    ...
    domCache.harvestBearBtn = document.getElementById('harvestBearBtn');
...
export const ui = {
    ...
    updateButtonStates(isInChunk, nearestObject, hasAxe, hasSaw, isOnCooldown = false, nearestConstructionSite = null, isMoving = false, nearestStructure = null, hasHammer = false, nearWater = false, hasFishingNet = false, onGrass = false, mushroomAvailable = false, vegetableSeedsAvailable = false, seedsAvailable = false, seedTreeType = null, isClimbing = false, occupiedOutposts = null, vegetablesGatherAvailable = false, activeAction = null, nearestMobileEntity = null, mobileEntityState = null, canDisembark = false, nearestDeerCorpse = null, nearestBearCorpse = null, nearestTowableEntity = null, cartAttachmentState = null, nearestLoadableCrate = null, crateLoadState = null) {
        ...
        if (isMounted) {
            ...
            if (domCache.harvestBearBtn) domCache.harvestBearBtn.style.display = 'none';
        ...
        if (crateBtn && !isMounted) {
            if (nearestStructure && !isMoving) {
                ...
                // Exclude dock, outpost, and construction sites from storage
                if (structureType === 'dock' || structureType === 'outpost' || isConstructionSite) {
                    ...
                    if (quality !== undefined && currentDurability !== undefined) {
                        ...
                        if (hoursUntilRuin !== undefined) {
                            tooltip += `<br>Time left: ${hoursUntilRuin.toFixed(1)}h`;
                        }
                    }
                    ...
                    if (structureType === 'market') {
                        // Find nearest dock to this market (cached for performance)
                        // Only recalculate every 60 frames or when market changes
                        const marketId = nearestStructure.userData?.objectId;
                        const now = performance.now();
                        if (!ui._dockCache ||
                            ui._dockCache.marketId !== marketId ||
                            now - ui._dockCache.timestamp > 1000) {  // Refresh every 1 second
                            ...
                            ui._dockCache = { marketId, nearestDockDist, timestamp: now };
                        }
                        const nearestDockDist = ui._dockCache.nearestDockDist;

                        if (nearestDockDist <= TRADE_RANGE) {
                            tooltip += `<br><span style="color:#4a4">Dock: ${nearestDockDist.toFixed(0)} units (trade enabled)</span>`;
                        } else if (nearestDockDist < Infinity) {
                            tooltip += `<br><span class="tooltip-warning">Dock: ${nearestDockDist.toFixed(0)} units (need <${TRADE_RANGE})</span>`;
                        } else {
                            tooltip += `<br><span class="tooltip-warning">No dock nearby (no trade)</span>`;
                        }
                    }
        ...
        // Skip all action button display logic if mounted
        if (nearestObject && !isMounted) {
            const objectType = nearestObject.name;
            ...
            if (structureTypes.includes(structureType) || isConstructionSite) {
                ...
                if (structureType === 'outpost') {
                    ...
                    // Normal structure demolition
                    let structureName;
                    if (isConstructionSite) {
                        // For construction sites, show "Demolish Construction Site"
                        structureName = 'Construction Site';
                    } else {
                        structureName = structureType.charAt(0).toUpperCase() + structureType.slice(1);
                    }

                    // Check ownership for protected structures
                    const ownerProtectedStructures = ['house', 'outpost', 'market', 'garden', 'tileworks', 'dock', 'ship'];
                    ...
                    // Don't show demolish button for non-owned protected structures
                    ...
                    removeBtn.style.opacity = '1';
                    removeBtn.style.cursor = 'pointer';
                    // Set tooltip for demolish button
                    buttonTooltips.set('removeBoxBtn', structureTooltip);
                    sawPlanksBtn.style.display = 'none';
                    buttonTooltips.delete('sawPlanksBtn');
        ...
        if (domCache.gatherVinesBtn && !isMounted) {
        ...
        if (domCache.gatherMushroomBtn && !isMounted) {
        ...
        if (domCache.gatherVegetableSeedsBtn && !isMounted) {
        ...
        if (domCache.gatherSeedsBtn && !isMounted) {
        ...
        if (domCache.gatherVegetablesBtn && !isMounted) {
        ...
        if (domCache.harvestDeerBtn && !isMounted) {
        ...
        // Harvest bear button - show when near dead bear corpse and stopped
        if (domCache.harvestBearBtn && !isMounted) {
            if (nearestBearCorpse && !isMoving && !activeAction) {
                domCache.harvestBearBtn.style.display = 'inline-block';
                domCache.harvestBearBtn.disabled = false;
                domCache.harvestBearBtn.style.opacity = '1';
                domCache.harvestBearBtn.style.cursor = 'pointer';
                buttonTooltips.set('harvestBearBtn', `Bear Corpse<br>Quality varies by region`);
            } else {
                domCache.harvestBearBtn.style.display = 'none';
                buttonTooltips.delete('harvestBearBtn');
            }
        }
        ...
        // Cart buttons (Attach Cart / Release Cart)
        const attachCartBtn = domCache.attachCartBtn;
        const releaseCartBtn = domCache.releaseCartBtn;
        const isAttached = cartAttachmentState?.isAttached;

        if (attachCartBtn) {
            // Show attach button when near a cart and not currently towing one
            // Strict check: must be a valid towable entity object with type 'cart'
        ...
        if (releaseCartBtn) {
            // Show release button when currently towing a cart
            if (isAttached) {
        ...
        // Crate loading buttons (Load Crate / Unload Crate)
        ...
        if (unloadCrateBtn) {
            ...
            const canUnloadCrate = isAttached && isCrateLoaded && !isMoving;
            if (canUnloadCrate) {
    ...
    updateMerchantButton(nearMerchant, isMoving = false) {
        ...
        if (btn) {
            btn.style.display = (nearMerchant && !isMoving) ? 'inline-block' : 'none';
        }
    },
    ...
    updateTrapperButton(nearTrapper, isMoving = false) {
        ...
        if (btn) {
            btn.style.display = (nearTrapper && !isMoving) ? 'inline-block' : 'none';
        }
    },
    ...
    initializeUI(callbacks) {
        ...
        setupButton(document.getElementById('harvestBearBtn'), () => {
            if (callbacks.onHarvestBear) {
                callbacks.onHarvestBear();
            }
        });
        ...
        setupButtonTooltip(document.getElementById('harvestBearBtn'));
