...
function initDOMCache() {
    ...
    domCache.statusLine1 = document.getElementById('statusLine1');
    domCache.statusLine2 = document.getElementById('statusLine2');
    domCache.statusLine3 = document.getElementById('statusLine3');
...
function updateStatusLineHelper(lineNumber, message, duration = 3000) {
    ...
    const statusLine = domCache[`statusLine${lineNumber}`];
...
export const ui = {
    ...
    updateButtonStates(isInChunk, nearestObject, hasAxe, hasSaw, isOnCooldown = false, nearestConstructionSite = null, isMoving = false, nearestStructure = null, hasHammer = false, nearWater = false, hasFishingNet = false, onGrass = false, mushroomAvailable = false) {
        // Ensure DOM cache is initialized
        if (!domCache.initialized) initDOMCache();
        ...
        // Grass gathering button - show when standing on grass
        const gatherGrassBtn = document.getElementById('gatherGrassBtn');
        if (gatherGrassBtn) {
            const canGatherGrass = onGrass && !isOnCooldown && !isMoving;
            gatherGrassBtn.style.display = canGatherGrass ? 'inline-block' : 'none';
        }
    ...
    updateSpeedDebug(speedData) {
        const speedDebugEl = document.getElementById('speedDebug');
        if (!speedDebugEl) return;

        if (!speedData || !speedData.isMoving) {
            speedDebugEl.style.display = 'none';
            return;
        }

        speedDebugEl.style.display = 'block';
        ...
        // Build modifier list
        if (speedData.onRoad) modifiers.push('🛣️ Road +30%');
        if (speedData.slope !== undefined && speedData.slope > 5) {
            modifiers.push(`⛰️ Slope ${speedData.slope.toFixed(0)}° (-${Math.round((1 - speedData.slopeMultiplier) * 100)}%)`);
        }
        if (speedData.surfaceType && speedData.surfaceType !== 'grass') {
            const surfaceMultiplier = speedData.surfaceMultiplier || 1.0;
            const surfacePenalty = Math.round((1 - surfaceMultiplier) * 100);
            if (surfacePenalty > 0) {
                modifiers.push(`🏜️ ${speedData.surfaceType} (-${surfacePenalty}%)`);
            }
        }

        const modifierText = modifiers.length > 0 ? '<br>' + modifiers.join('<br>') : '';

        // Add coordinate info for debugging
        let coordText = '';
        if (speedData.worldX !== undefined && speedData.worldZ !== undefined) {
            coordText = `<br><span style="color: #888; font-size: 10px;">Pos: (${speedData.worldX.toFixed(1)}, ${speedData.worldZ.toFixed(1)})</span>`;
        }
        if (speedData.chunkId !== undefined) {
            coordText += `<br><span style="color: #888; font-size: 10px;">Chunk: ${speedData.chunkId}</span>`;
        }

        speedDebugEl.innerHTML = `Speed: ${percentage}%${modifierText}${coordText}`;
    },
    ...
    initializeUI(callbacks) {
        ...
        setupButton(document.getElementById('gatherGrassBtn'), () => {
            callbacks.onStartGatherGrass();
        });
