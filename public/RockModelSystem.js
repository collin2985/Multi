...
export class RockModelSystem {
    constructor(scene) {
        ...
        // Per rock type - can be reduced on lower quality settings
        this.maxInstances = CONFIG.ROCK_MODEL_MAX_INSTANCES || 50000;
    ...
    async loadModel(loader, rockType) {
        try {
            const gltf = await loader.loadAsync(`./models/${rockType}.glb`);
    ...
    addRockInstanceInternal(rockObject, rockType, position, rotation, baseScale) {
        ...
        if (availableSet.size === 0) {
            console.warn(`[RockModelSystem] No available instance slots for ${rockType}`);
            return -1;
        }
