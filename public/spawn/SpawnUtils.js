...
const SPAWN_CONFIG = {
    ...
    // Westward search
    WEST_STEP: 10,          // Units to move west per iteration (finer for better land detection)
    MAX_ITERATIONS: 5000,   // Max westward steps before retry (50,000 units)
...
export function findValidSpawnPoint(terrainGenerator, initialX, initialZ, minZ, maxZ, banditController = null) {
    ...
    // Use provided Z bounds for faction restriction, fallback to full world if not specified
    ...
    for (let retry = 0; retry < SPAWN_CONFIG.MAX_RETRIES; retry++) {
        // Pick random X within full world bounds
        const startX = SPAWN_CONFIG.MIN_X + Math.random() * (SPAWN_CONFIG.MAX_X - SPAWN_CONFIG.MIN_X);
        // Pick random Z within faction-restricted bounds
