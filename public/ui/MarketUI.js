...
export class MarketUI {
    ...
    updateMarketRow(itemType, data) {
        ...
        // Update buy price span
        const buyPriceText = `🪙${data.buyPrice || '-'}`;
    ...
    calculateMarketPrice(itemType, quality, durability, totalStock, priceType) {
        ...
        const supplyMultiplier = 1 + ((maxQty - totalStock) / maxQty) * 0.5;
    ...
    renderMarketInventory() {
        ...
        for (const entry of expectedOrder) {
            ...
            if (topMarketItem) {
                ...
                const itemSize = getItemSize(itemType);
                const canFit = this.inventoryUI.hasInventorySpace(itemSize.width * itemSize.height);

                if (!canFit) {
                    buyDisabled = true;
                    buyDisabledReason = `No space (${itemSize.width}×${itemSize.height})`;
                } else if (playerCoins < buyPrice) {
                    buyDisabled = true;
    ...
    showMarketConfirmation(type, itemType, item, price, breakdown) {
        ...
        document.getElementById('marketConfirmTitle').textContent = type === 'buy' ? 'Confirm Purchase' : 'Confirm Sale';
        ...
        document.getElementById('marketConfirmMessage').textContent =
            `${type === 'buy' ? 'Buy' : 'Sell'} ${itemDesc} for 🪙${price}?`;
    ...
    executeBuyHorse(quality, price) {
        ...
        this.networkManager.sendMessage('buy_horse', {
            ...
            rotation: playerRotation * (180 / Math.PI),  // Convert to degrees
    ...
    renderHorseSellUI() {
        ...
        const marketList = document.getElementById('marketList');
        marketList.innerHTML = `
            <div class="market-category-header">Your Horse</div>
            <div class="market-row">
                <div class="market-item-name">Horse (Q:${Math.floor(quality)})</div>
                <div class="market-sell-col">
                    <span class="market-price">+${priceInfo.price}</span>
                    <button class="market-btn sell-btn" id="sellHorseBtn" ${canSell ? '' : 'disabled'}
                            style="${canSell ? '' : 'opacity: 0.5; cursor: not-allowed;'}">Sell</button>
                </div>
            </div>
            <div style="padding: 10px; color: #888; font-size: 12px;">
                ${canSell
                    ? 'Selling will dismount you from the horse.'
                    : 'Cannot sell here - stop on flat land first.'}
            </div>
        `;
