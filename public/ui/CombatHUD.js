...
export class CombatHUD {
     ...
     */
    update(inCombat, distance = 0, shootingRange = 10, hitChance = 0.2, ammoCount = 0, hasRifle = true, targetType = 'bandit', targetId = null) {
        if (!inCombat) {
            this.hide();
            return;
        }
     ...
     */
    updateArtillery(isManning, lastFireTime = 0, cooldown = 12000, artilleryQuality = 50, targetDistance = 0, maxRange = 28) {
        if (!isManning) {
            // Exit artillery mode
        ...
        if (!this.isArtilleryMode) {
            ...
            this.warningEl.style.display = 'none';  // Hide enemy warning in artillery mode
            this.noRifleWarning.style.display = 'none';
            this.noAmmoWarning.style.display = 'none';
            this.show();
        }
        ...
        // Update accuracy display based on artillery quality
        const qualityBonus = (artilleryQuality - 50) / 50 * 0.10;
        const baseAccuracy = 0.35 + qualityBonus;
