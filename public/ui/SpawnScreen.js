...
export class SpawnScreen {
    constructor(gameState, networkManager, onSpawnSelected, onLogout = null, onOpenFriends = null) {
        ...
        this.factionSelectorVisible = false;
    ...
    setupMessageHandlers() {
        ...
        if (this.networkManager) {
            this.networkManager.on('friends_list_response', (data) => {
                ...
                if (this.isVisible && !this.friendPositionPending && !this.factionSelectorVisible) {
                    this.render();
                }
            });
        }
    ...
    showFactionSelector() {
        this.factionSelectorVisible = true;
        ...
        this.container.innerHTML = `
            <button id="factionCloseBtn" style="
                position: absolute;
                top: 10px;
                right: 10px;
                width: 32px;
                ...
                padding: 0;
                z-index: 10;
            " onmouseover="this.style.background='rgba(255,255,255,0.3)'" onmouseout="this.style.background='rgba(255,255,255,0.1)'">&times;</button>
        ...
        document.getElementById('factionCloseBtn').addEventListener('click', () => {
            this.factionSelectorVisible = false;
            this.render();  // Go back to main spawn screen
        });
    }
    ...
    async confirmFactionChange(newFactionId) {
        ...
        if (confirmed) {
            ...
            if (window.game?.updateLocalPlayerFactionColors) {
                window.game.updateLocalPlayerFactionColors(newFactionId);
            }
