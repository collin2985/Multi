...
export class CrateInventoryUI {
    ...
    handleLockResponse(payload) {
        ...
        if (success) {
            ...
            // Show error message via toast (more visible than chat)
            if (window.ui) {
                window.ui.showToast(reason || 'Cannot access this storage', 'error');
            }
    ...
    updateCrateSection() {
        ...
        const shouldShow = this.gameState.nearestStructure &&
                           ...
                           this.gameState.nearestStructure.userData?.modelType !== 'bearden' &&
                           this.gameState.nearestStructure.userData?.modelType !== 'miner' &&
                           this.gameState.nearestStructure.userData?.modelType !== 'stonemason' &&
                           this.gameState.nearestStructure.userData?.modelType !== 'gardener' &&
