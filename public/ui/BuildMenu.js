...
export class BuildMenu {
    ...
    startStructurePlacement(structure) {
        ...
        if (structure.type === 'road') {
            ...
            // No Y offset needed - after rotateX, geometry already spans Y from 0 to PREVIEW_HEIGHT
    ...
    advanceStructurePlacementPhase(mouseY) {
        ...
        if (placement.phase === 'position') {
            // Special handling for plantable trees and docks - skip rotation, place immediately
            // (roads now have rotation phase for pill shape orientation)
            if (structure && (structure.type === 'dock' || structure.isPlantable)) {
