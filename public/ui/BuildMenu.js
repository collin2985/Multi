...
export class BuildMenu {
    ...
    startStructurePlacement(structure) {
        ...
        // Dock rotation is auto-detected based on shoreline orientation
        // (set by findValidDockPlacement in updateStructurePreview)
        if (structure.type === 'dock') {
            this.structurePlacement.rotation = 90; // Default, will be overwritten by auto-detection
        }

        // Fisherman rotation is auto-detected based on shoreline orientation
        // (set by findValidFishermanPlacement in updateStructurePreview)
        if (structure.type === 'fisherman') {
            this.structurePlacement.rotation = 0; // Default, will be overwritten by auto-detection
        }

        // Plantables don't rotate (roads now have rotation phase)
        if (structure.isPlantable) {
        ...
        if (structure.type === 'road') {
            ...
            // Center geometry and position with bottom at y=0
            geometry.computeBoundingBox();
            const center = new THREE.Vector3();
            geometry.boundingBox.getCenter(center);
            geometry.translate(-center.x, -center.y, -center.z);
            geometry.translate(0, PREVIEW_HEIGHT / 2, 0);
            ...
            // Explicitly center geometry to ensure rotation pivot is at center
            geometry.computeBoundingBox();
            const center = new THREE.Vector3();
            geometry.boundingBox.getCenter(center);
            geometry.translate(-center.x, -center.y, -center.z);

            // Market: keep centered at y=0 so preview is half above/half below mouse point
            // Other structures: shift so bottom is at y=0
            if (structure.type !== 'market') {
                geometry.translate(0, PREVIEW_HEIGHT / 2, 0);
            }

            previewMesh = new THREE.Mesh(geometry, previewMaterial);
            // No additional mesh offset needed - geometry is centered with bottom at y=0 (or centered for market)
        } else {
            // Fallback: 1x1 box
    ...
    advanceStructurePlacementPhase(mouseY) {
        ...
        if (placement.phase === 'position') {
            // Special handling for plantable trees, docks, and fisherman - skip rotation, place immediately
            // (roads now have rotation phase for pill shape orientation)
            if (structure && (structure.type === 'dock' || structure.type === 'fisherman' || structure.isPlantable)) {
                // Dock/fisherman rotation is already set by findValidDockPlacement/findValidFishermanPlacement
    ...
    confirmStructurePlacement() {
        ...
        // TEST: Temporarily only requires plank (was parts + plank)
        if (structure && structure.type === 'artillery') {
            ...
            // TEST: Only using plank quality (was average of parts + plank)
            const plankQuality = plank.quality || 50;
            const avgQuality = plankQuality;

            // TEST: Parts removal disabled
        ...
        // Structures that level terrain: crate, house, garden, outpost, tent, market, tileworks
    ...
    updateStructurePreview(terrainX, terrainZ, mouseY) {
        ...
        // Docks and fisherman use immediate validation (position is auto-calculated)
        const structure = this.structurePlacement.structure;
        if (structure && (structure.type === 'dock' || structure.type === 'fisherman')) {
            // Run validation immediately for docks and fisherman
            const result = this.structureManager.validateStructurePlacement(
                this.structurePlacement,
                'position',
                this.playerObject ? this.playerObject.position : null
    ...
    onRotationChange(rotation) {
        ...
        if (placement.previewBox) {
            placement.previewBox.rotation.y = rotation * (Math.PI / 180);
        }

        // Recalculate height based on new rotation (different sample points may have different terrain)
        // IMPORTANT: rotation parameter is in DEGREES, must convert to radians for trig
        const structure = placement.structure;
        if (structure && structure.type !== 'dock' && structure.type !== 'fisherman') {
            const dims = CONFIG.CONSTRUCTION.GRID_DIMENSIONS[structure.type];
            ...
            if (structure.type === 'road') {
                // Roads use exact terrain height at center + small offset (same as position phase)
                ...
                if (placement.previewBox) {
                    placement.previewBox.position.y = roadHeight;
                }
            } else if (structure.type === 'market') {
                // Market uses 3 sample points along depth axis: center, +5, -5
