...
export function getItemSize(itemType) {
    // Define tool types
    const largeTools = ['axe', 'saw', 'pickaxe', 'rifle']; // 2x5
    const smallTools = ['hammer', 'chisel']; // 1x2
    // Note: fishingnet removed from mediumTools - now defaults to 1x1 to match crafting output
    ...
    const metalMaterials = ['iron', 'ironingot', 'parts']; // 1x1

    // Define food types
    const foodItems = ['apple', 'vegetables', 'fish', 'cookedfish', 'cookedmeat', 'roastedvegetables', 'appletart', 'mushroom', 'rawmeat']; // 1x1

    // Define seed types
    const seedItems = ['pineseed', 'appleseed', 'firseed', 'vegetableseeds']; // 1x1
