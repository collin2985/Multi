...
export class InventoryUI {
    ...
    toggleInventory() {
        ...
        if (!this.gameState.inventoryOpen && this.gameState.nearestStructure) {
            if (!this.isHouseOwner(this.gameState.nearestStructure)) {
        ...
        if (this.gameState.inventoryOpen) {
            ...
            this.crateUI.updateCrateSection(); // Show/hide crate section based on proximity
    ...
    renderInventory() {
        ...
        this.tooltipUI.hideTooltip(); // Hide tooltip before clearing items
    ...
    renderSlingSlot() {
        ...
        if (slingItem) {
            ...
            // Rifle in sling is always displayed horizontally (rotated)
            ...
            // Add mousedown handler for dragging from sling
            slingItemContainer.addEventListener('mousedown', (e) => {
                e.preventDefault();
                e.stopPropagation();
                this.onSlingItemMouseDown(e, slingItem);
            });

            // Add tooltip handlers
            slingItemContainer.addEventListener('mouseenter', (e) => {
                if (!this.inventoryPickedItem) {
                    this.tooltipUI.showTooltip(e, slingItem);
                }
            });
            slingItemContainer.addEventListener('mousemove', (e) => {
                if (!this.inventoryPickedItem) {
                    this.tooltipUI.updateTooltipPosition(e);
                }
            });
            slingItemContainer.addEventListener('mouseleave', () => {
                this.tooltipUI.hideTooltip();
            });
        } else {
            // No rifle - show empty slot
    ...
    _updateTargetHighlights(newChiselTarget, newCombineTarget) {
        ...
        if (oldChiselId && oldChiselId !== newChiselId) {
            const oldEl = document.querySelector(`[data-item-id="${oldChiselId}"]`);
            if (oldEl) oldEl.classList.remove('chisel-target');
        }
        if (oldCombineId && oldCombineId !== newCombineId) {
            const oldEl = document.querySelector(`[data-item-id="${oldCombineId}"]`);
            if (oldEl) oldEl.classList.remove('chisel-target');
        }

        // Add new highlights
        if (newChiselId && newChiselId !== oldChiselId) {
            const newEl = document.querySelector(`[data-item-id="${newChiselId}"]`);
            if (newEl) newEl.classList.add('chisel-target');
        }
        if (newCombineId && newCombineId !== oldCombineId) {
            const newEl = document.querySelector(`[data-item-id="${newCombineId}"]`);
            if (newEl) newEl.classList.add('chisel-target');
        }
    ...
    getItemUnderCursor(cursorX, cursorY, draggingItem) {
        ...
        const combinableItems = ['vines', 'Rvines', 'rope', 'Rrope'];
    ...
    _handleCrateAwareMouseMove(event, item) {
        // Get items layers for position calculation (needed for ghost updates)
        ...
        if (newTarget === 'backpack') {
            if (item.type === 'chisel') {
                ...
                if (itemType === 'vines' || itemType === 'rope') {
    ...
    _getChiselTarget(item) {
        if (item.type !== 'chisel') return null;
        try {
    ...
    _getCombineTarget(item) {
        const itemType = item.type.replace('R', '');
        if (itemType !== 'vines' && itemType !== 'rope') return null;
        try {
            const targetItem = this.getItemUnderCursor(this.inventoryMouseX, this.inventoryMouseY, item);
            if (targetItem) {
                const targetType = targetItem.type.replace('R', '');
                return (targetType === itemType) ? targetItem : null;
            }
            return null;
        } catch (error) {
    ...
    onMouseUp(event) {
        ...
        try {
            ...
            const itemType = item.type.replace('R', '');
            if (source === 'backpack' && (itemType === 'vines' || itemType === 'rope') && this.combineTarget) {
    ...
    _handleCrateModePlacement(event, item, source) {
        ...
        if (nearestStructure && nearestStructure.userData.modelType === 'house' && nearestStructure.userData.owner) {
            ...
            if (owner !== currentClientId && owner !== currentAccountId) {
                ...
                console.log(`Access denied: Cannot modify house inventory owned by ${owner}`);
        ...
        if (target === 'backpack') {
            ...
            // Special handling for coins from crate - bypass placement validation when merging
            // Coins always merge regardless of drop position
            if (source === 'crate' && item.type === 'coin') {
                const backpackCoin = this.gameState.inventory.items.find(i => i.type === 'coin');
                if (backpackCoin) {
                    // Merge quantities - always works regardless of backpack space
                ...
                // No coin in backpack - fall through to normal validation below
            }
            ...
            if (this.isValidPlacement(item, gridPos.x, gridPos.y, item.rotation)) {
                ...
                if (source === 'crate') {
                    ...
                    if (item.type === 'coin') {
                        ...
                        // Remove from crate (use findIndex with ID for robust reference matching)
                        const crateInventory = this.crateInventory;
                        const itemIndex = crateInventory.items.findIndex(i => i.id === item.id);
                        if (itemIndex > -1) {
                            crateInventory.items.splice(itemIndex, 1);
                        ...
                        if (item.type && item.type.endsWith('firewood') && item.placedAtTick) {
                            ...
                            console.log(`[Tick] Firewood removed from structure, durability updated to ${item.durability}`);
                        }
    ...
    renderBackpackItem(item, container) {
        ...
        itemWrapper.addEventListener('mouseenter', (e) => this.tooltipUI.showTooltip(e, item));
        itemWrapper.addEventListener('mousemove', (e) => this.tooltipUI.updateTooltipPosition(e));
        itemWrapper.addEventListener('mouseleave', () => this.tooltipUI.hideTooltip());
    ...
    _performCrateSave() {
        ...
        if (crate.userData.modelType === 'market') {
            console.warn('[_performCrateSave] Prevented accidental save to market - markets use buy/sell system');
            return;
        }
