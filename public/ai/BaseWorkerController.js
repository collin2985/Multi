...
export class BaseWorkerController {
    ...
    _handleStuckState(entity) {
        ...
        if (now - entity.stuckTime >= timeout) {
            ...
            // Clean up expired failed targets (>120s old)
    ...
    _moveAlongPath(entity, deltaTime) {
        ...
        if (getTerrainHeight) {
            ...
            // Steep slope detection (throttled to every 500ms)
            if (!entity._lastSteepCheck || Date.now() - entity._lastSteepCheck > 500) {
    ...
    _handleSteepSlopeDetected(entity, slopeX, slopeZ) {
        this.game?.navigationManager?.markSteepSlope(slopeX, slopeZ);
        entity.path = [];
        entity.pathIndex = 0;
        entity._skipNextArrival = true;
        if (entity.lastSafePosition) {
