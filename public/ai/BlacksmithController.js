...
class BlacksmithController extends BaseWorkerController {
    ...
    _assessStructureAndDecide(entity) {
        ...
        // PRIORITY 0: Already carrying output (parts)? -> Deliver it first
        // This handles recovery from stuck/interrupted delivery attempts
        const carryingOutput = entity.carrying.some(item => item?.type === 'parts');
        if (carryingOutput) {
    ...
    _handleAssessingStructure(entity) {
        ...
        switch (decision.action) {
            case 'deliver_output':
                // Resume interrupted delivery - already carrying parts
