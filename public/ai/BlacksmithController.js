/**
 * BlacksmithController.js
 * Manages Blacksmith NPCs that work at blacksmith structures
 *
 * Blacksmith behavior loop:
 * 1. Collect ironingot (5) from market
 * 2. Collect firewood (1) from market
 * 3. Deposit both at blacksmith
 * 4. Wait for blacksmith to process ironingot into parts
 * 5. Collect parts from blacksmith
 * 6. Deliver parts to market
 * 7. Repeat
 *
...
// Blacksmith-specific configuration (base configs inherited from BaseWorkerController)
const BLACKSMITH_CONFIG_DEFAULTS = {
    NPC_COLOR: 0x2F4F4F,              // Dark slate gray (blacksmith color)
    ITEMS_PER_TRIP: 5
};

// Blacksmith states
const BLACKSMITH_STATE = {
    IDLE: 'idle',
...
class BlacksmithController extends BaseWorkerController {
    constructor() {
        super({
            workerType: 'blacksmith',
            configKey: 'BLACKSMITH',
            npcColor: BLACKSMITH_CONFIG_DEFAULTS.NPC_COLOR,
            displayName: 'Blacksmith',
            assessmentStateName: 'ASSESSING_STRUCTURE',
            movementStates: [
                BLACKSMITH_STATE.SEEKING_INPUT,
                BLACKSMITH_STATE.SEEKING_FIREWOOD,
                BLACKSMITH_STATE.RETURNING,
                BLACKSMITH_STATE.DELIVERING
            ]
        });
    }
    ...
    _getConfig(key) {
        return CONFIG.BLACKSMITH?.[key] ?? BLACKSMITH_CONFIG_DEFAULTS[key] ?? super._getConfig(key);
    }
    ...
    _getStateEnum() {
        return BLACKSMITH_STATE;
    }

    _getStructuresInChunk(chunkKey) {
        return this.gameState?.getBlacksmithsInChunk(chunkKey) || [];
    }

    _createWorkerSpecificState(buildingData) {
        return {
            blacksmithId: buildingData.id,  // Alias for backwards compatibility
            previousTask: null,
    ...
    _handleIdleState(entity) {
        ...
        if (homeDistSq <= 9) {  // Within 3 units of blacksmith
            entity.state = BLACKSMITH_STATE.ASSESSING_STRUCTURE;
        } else {
            // Not at blacksmith, go home first
            entity.state = BLACKSMITH_STATE.RETURNING;
            entity.path = [];
        }
    }
    ...
    _assessStructureAndDecide(entity) {
        ...
        if (!structureData) {
            return { action: 'stuck', reason: 'Lost connection to blacksmith.' };
        }
        ...
        // PRIORITY 1: Output (parts) present? -> Take and deliver
        const outputCount = this._countItemsOfType(structureInv, 'parts');
        if (outputCount > 0) {
        ...
        // PRIORITY 4: Input (ironingot) present? -> Wait for processing (62 ticks)
        const inputCount = this._countItemsOfType(structureInv, 'ironingot');
        if (inputCount > 0) {
            return { action: 'wait_for_output' };
        }

        // PRIORITY 5: No input -> Handle input situation (get from market)
        const carryingInput = entity.carrying.some(item => item?.type === 'ironingot');
        if (carryingInput) {
            return { action: 'deposit_input' };
        } else {
            // Get input from market
            if (!market) return { action: 'stuck', reason: 'No market nearby for ironingot.' };
            const marketData = this.gameState?.getStructureById(market.id);
            const marketInv = marketData?.object?.userData?.inventory;
            if (!this._marketHasItem(marketInv, 'ironingot', 1)) {
    ...
    _handleAssessingStructure(entity) {
        ...
        switch (decision.action) {
            case 'collect_output':
                entity.state = BLACKSMITH_STATE.COLLECTING_OUTPUT;
                entity.targetId = decision.target;
                break;

            case 'remove_excess_firewood':
                entity.state = BLACKSMITH_STATE.REMOVING_EXCESS_FIREWOOD;
                entity._excessFirewoodCount = decision.excessCount;
                entity.requestSentAt = null;
                break;

            case 'clear_slot_and_deposit_firewood':
                entity.state = BLACKSMITH_STATE.CLEARING_SLOT_FOR_FIREWOOD;
                entity.requestSentAt = null;
                break;

            case 'get_firewood':
                entity.state = BLACKSMITH_STATE.SEEKING_FIREWOOD;
                entity.targetId = decision.target;
                entity.path = [];
                break;

            case 'wait_for_firewood':
                entity.state = BLACKSMITH_STATE.WAITING_FOR_FIREWOOD;
                break;

            case 'wait_for_output':
                entity.state = BLACKSMITH_STATE.WAITING_FOR_OUTPUT;
                break;

            case 'deposit_input':
                entity.state = BLACKSMITH_STATE.DEPOSITING;
                entity.requestSentAt = null;
                break;

            case 'get_input':
                entity.state = BLACKSMITH_STATE.SEEKING_INPUT;
                entity.targetId = decision.target;
                entity.path = [];
                break;

            case 'wait_for_input':
                entity.state = BLACKSMITH_STATE.WAITING_FOR_INPUT;
                break;
    ...
    _handleWorkerSpecificState(entity, deltaTime) {
        switch (entity.state) {
            case BLACKSMITH_STATE.COLLECTING_INPUT:
                this._handleCollectingInput(entity);
                break;
            case BLACKSMITH_STATE.COLLECTING_FIREWOOD:
                this._handleCollectingFirewood(entity);
                break;
            case BLACKSMITH_STATE.DEPOSITING:
                this._handleDepositing(entity);
                break;
            case BLACKSMITH_STATE.WAITING_FOR_OUTPUT:
                this._handleWaitingForOutput(entity);
                break;
            case BLACKSMITH_STATE.WAITING_FOR_INPUT:
                this._handleWaitingForInput(entity);
                break;
            case BLACKSMITH_STATE.WAITING_FOR_FIREWOOD:
                this._handleWaitingForFirewood(entity);
                break;
            case BLACKSMITH_STATE.COLLECTING_OUTPUT:
                this._handleCollectingOutput(entity);
                break;
            case BLACKSMITH_STATE.DEPOSITING_OUTPUT:
                this._handleMarketDeposit(entity);
                break;
            case BLACKSMITH_STATE.ASSESSING_STRUCTURE:
                this._handleAssessingStructure(entity);
                break;
            case BLACKSMITH_STATE.REMOVING_EXCESS_FIREWOOD:
                this._handleRemovingExcessFirewood(entity);
                break;
            case BLACKSMITH_STATE.CLEARING_SLOT_FOR_FIREWOOD:
    ...
    _getMovementTarget(entity) {
        switch (entity.state) {
            case BLACKSMITH_STATE.SEEKING_INPUT:
            case BLACKSMITH_STATE.SEEKING_FIREWOOD:
            case BLACKSMITH_STATE.DELIVERING:
            ...
            case BLACKSMITH_STATE.RETURNING:
    ...
    _onArrival(entity) {
        ...
        switch (entity.state) {
            case BLACKSMITH_STATE.SEEKING_INPUT:
                entity.state = BLACKSMITH_STATE.COLLECTING_INPUT;
                entity.requestSentAt = null;
                break;
            case BLACKSMITH_STATE.SEEKING_FIREWOOD:
                entity.state = BLACKSMITH_STATE.COLLECTING_FIREWOOD;
                entity.requestSentAt = null;
                break;
            case BLACKSMITH_STATE.RETURNING:
                entity.state = BLACKSMITH_STATE.ASSESSING_STRUCTURE;
                ...
                break;
            case BLACKSMITH_STATE.DELIVERING:
                entity.state = BLACKSMITH_STATE.DEPOSITING_OUTPUT;
                entity.requestSentAt = null;
                break;
        }
    }
    ...
    _handleCollectingInput(entity) {
        // Collect input (ironingot) from market
    ...
    _handleWaitingForOutput(entity) {
        ...
        if (!entity._waitStartTick) {
            ...
            // Also request server to check/complete any processing
            if (this.networkManager) {
                const structureData = this.gameState?.getStructureById(entity.buildingId);
                if (structureData) {
        ...
        // Done waiting - check for output (parts)
        const structureData = this.gameState?.getStructureById(entity.buildingId);
        const structureInv = structureData?.object?.userData?.inventory;
        const outputCount = this._countItemsOfType(structureInv, 'parts');
        ...
        if (outputCount > 0) {
            ...
            // No output yet - check if input still present
            const inputCount = this._countItemsOfType(structureInv, 'ironingot');
            if (inputCount > 0) {
                // Still have input, wait another 62 ticks
                entity._waitStartTick = currentTick;

                // Request server to check processing again
                ...
                // No input, no output - something went wrong, reassess
                entity._waitStartTick = null;
                entity.state = BLACKSMITH_STATE.ASSESSING_STRUCTURE;
            }
        }
    ...
    _handleWaitingForInput(entity) {
        // Check market for input (ironingot) every 30 seconds
        const now = Date.now();
    ...
    handleCollectResponse(data) {
        ...
        if (itemType === 'firewood' || itemType?.endsWith?.('firewood')) {
            ...
            entity.state = BLACKSMITH_STATE.RETURNING;
            entity.targetId = null;
        } else if (itemType === 'parts') {
            // Got output from structure, deliver to market
            entity.state = BLACKSMITH_STATE.DELIVERING;
            const market = this._findMarketNearPosition(entity.homePosition, this._getConfig('MARKET_MAX_DISTANCE'));
    ...
    handleRemoveFirewoodResponse(data) {
        ...
        const entity = this.entities.get(structureId);
        if (!entity || entity.state !== BLACKSMITH_STATE.REMOVING_EXCESS_FIREWOOD) return;
    ...
    handleClearDepositResponse(data) {
        ...
        const entity = this.entities.get(structureId);
        if (!entity || entity.state !== BLACKSMITH_STATE.CLEARING_SLOT_FOR_FIREWOOD) return;
    ...
    // =========================================================================
    // AUTHORITY OVERRIDE - Blacksmith uses homePosition (matches baker behavior)
    // =========================================================================

    /**
     * Override: Blacksmith uses homePosition (building location) for authority calculation
     * This is different from Gardener/Woodcutter which use current position
     */
    ...
    // =========================================================================
    // SYNC OVERRIDE - Blacksmith returns ALL entities (matches baker behavior)
    // =========================================================================

    /**
     * Override: Blacksmith returns ALL entities, not just authority-owned
     */
    ...
    checkBlacksmithSpawn(dockData) {
        return this.checkWorkerSpawn(dockData);
    }

    getBlacksmithDialogueData(blacksmithId) {
        return this.getWorkerDialogueData(blacksmithId);
    }

    onBlacksmithDestroyed(blacksmithId) {
        return this.onBuildingDestroyed(blacksmithId);
    }

    getActiveBlacksmithsForSync() {
        return this.getActiveWorkersForSync();
    }

    syncBlacksmithsFromPeer(workerList, peerId) {
        return this.syncWorkersFromPeer(workerList, peerId);
    }
}

// Singleton instance
export const blacksmithController = new BlacksmithController();
export { BLACKSMITH_STATE };
