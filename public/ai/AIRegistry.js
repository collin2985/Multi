...
export class AIRegistry {
     ...
     * @param {string} type - 'bandit' | 'bear' | 'deer'
     ...
     * @param {number} radius - Search radius
     * @param {string} perspective - 'player' | 'bandit' | 'bear' | 'deer'
     * @returns {boolean} True if threats exist
     */
    hasHostilesNear(x, z, radius, perspective) {
        // Define what's hostile from each perspective
        const hostileTypes = {
            player: ['bandit', 'bear'],  // Players fear bandits and bears
            bandit: ['bear'],            // Bandits fear bears
            bear: [],                    // Bears fear nothing (except structures, handled separately)
            deer: ['player', 'bandit', 'bear']  // Deer fear everything
        };
     ...
     * @param {string} type - Controller type ('bandit', 'bear', 'deer')
    ...
    killEntity(type, entityId, killedBy) {
        ...
        if (type === 'bandit' && controller.killEntity) {
            ...
            return true;
        } else if (type === 'bear' && controller.killBear) {
            controller.killBear(entityId, killedBy);
            return true;
        } else if (type === 'deer' && controller.killDeer) {
