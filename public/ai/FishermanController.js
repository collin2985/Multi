...
class FishermanController extends BaseWorkerController {
        ...
        for (const item of inventory.items) {
            ...
            if (item.processingStartTick && item.processingDurationTicks) {
                ...
                if (ticksElapsed >= item.processingDurationTicks && !item._npcCompletionSent) {
                    ...
                    console.log(`[Fisherman] Sent processing_complete for ${item.type} (${item.id})`);
                }
            }
        }
    ...
    _pickNewWanderTarget(entity) {
        ...
        const dist = this._getConfig('PERIMETER_DISTANCE');

        const targetX = entity.homePosition.x + Math.cos(angle) * dist;
        const targetZ = entity.homePosition.z + Math.sin(angle) * dist;
        const targetY = this.getTerrainHeight?.(targetX, targetZ) ?? 0;

        // If target is in water, try other angles to find land
        if (targetY < 0) {
            for (let i = 1; i <= 8; i++) {
                const testAngle = angle + (i * Math.PI / 4);
                const testX = entity.homePosition.x + Math.cos(testAngle) * dist;
                const testZ = entity.homePosition.z + Math.sin(testAngle) * dist;
                const testY = this.getTerrainHeight?.(testX, testZ) ?? 0;
                if (testY >= 0) {
        ...
        entity.wanderTarget = { x: targetX, z: targetZ };
        entity.path = [];
        entity.pathIndex = 0;
    }
