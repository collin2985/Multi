...
class WoodcutterController extends BaseWorkerController {
    ...
    _getMovementTarget(entity) {
        ...
        if (entity.state === WOODCUTTER_STATE.SEEKING_LOG) {
            ...
            if (!log && entity.expectedLogPosition) {
            ...
            // The timeout logic in _onArrival will handle giving up after 5 seconds
        ...
        switch (entity.state) {
            ...
            case WOODCUTTER_STATE.SEEKING_LOG:
                ...
                if (log) {
                    ...
                    // Log not found - wait up to 5 seconds for it to appear
                    if (!entity.logSearchStartTime) {
                        entity.logSearchStartTime = Date.now();
                    }
                    if (Date.now() - entity.logSearchStartTime > 5000) {
    ...
    _startDelivering(entity) {
        const market = this._findMarketNearPosition(entity.homePosition, this._getConfig('MARKET_MAX_DISTANCE'));
        if (!market) {
            this._enterStuckState(entity, 'Cannot find nearby market for delivery');
            return false;
        }
    ...
    _findLogNearPosition(position, radius) {
