...
class GardenerController extends BaseWorkerController {
    ...
    _decideNextTask(entity) {
        ...
        if (hasVegetables) {
            if (!market) return { task: 'stuck', reason: 'Cannot find nearby market.' };
    ...
    _handleWaitingForHarvest(entity) {
        ...
        // Cleanup old tree records periodically
        if (now - (entity._lastTreeCleanup || 0) > this._getConfig('TREE_CLEANUP_INTERVAL')) {
            this._cleanupOldTreeRecords(entity);
    ...
    _cleanupOldTreeRecords(entity) {
        if (!entity.treeGrid) return;

        const oneHourAgo = Date.now() - (60 * 60 * 1000);

        for (const [cellKey, trees] of entity.treeGrid.entries()) {
            const filtered = trees.filter(t => t.plantedAt > oneHourAgo);
            if (filtered.length === 0) {
                entity.treeGrid.delete(cellKey);
            } else if (filtered.length !== trees.length) {
                entity.treeGrid.set(cellKey, filtered);
            }
        }
    }
