...
class BakerController extends BaseWorkerController {
    ...
    _assessBakeryAndDecide(entity) {
        ...
        if (tartCount > 0) {
            if (!market) return { action: 'stuck', reason: 'No market nearby for tart delivery.' };
    ...
    _handleWaitingForTarts(entity) {
        ...
        if (tartCount > 0) {
            ...
            // No tarts yet - check if apples still present
            const appleCount = this._countItemsOfType(bakeryInv, 'apple');
            if (appleCount > 0) {
                // Still have apples, wait another 62 ticks
                entity._waitStartTick = currentTick;

                // Request server to check processing again
                if (this.networkManager && bakeryData) {
                    this.networkManager.sendMessage('npc_check_bakery_processing', {
                        bakeryId: entity.buildingId,
                        chunkId: `chunk_${bakeryData.chunkKey}`
                    });
                ...
                // No apples, no tarts - something went wrong, reassess
                entity._waitStartTick = null;
                entity.state = BAKER_STATE.ASSESSING_BAKERY;
            }
        }
