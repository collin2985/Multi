...
export class BrownBearController extends BaseAIController {
    ...
    _interpolateEntity(entity, deltaTime) {
        ...
        // Smoothly interpolate rotation with speed limit (~120 deg/sec)
    ...
    _calculateAuthority(denId) {
        ...
        const CHUNK_SIZE = AI_CONFIG.BROWN_BEAR.CHUNK_SIZE;
    ...
    handleStateMessage(data) {
        const { denId, authorityId, authorityTerm, position, rotation, state, target, chaseTargetType, wanderDirection, fleeDirection } = data;

        const entity = this.entities.get(denId);
        if (!entity) {
            this.pendingStates.set(denId, { position, rotation, state, authorityId, authorityTerm, target, chaseTargetType, wanderDirection, fleeDirection });
            return;
        }
