 ...
 * - Interpolation
 *
 * Subclasses: BanditController, BearController, DeerController
 */

export class BaseAIController {
    /**
     * @param {Object} config
     * @param {string} config.entityType - 'bandit' | 'bear' | 'deer'
     * @param {string} config.entityIdField - 'tentId' | 'chunkKey'
     * @param {string} config.messagePrefix - For P2P message types (e.g., 'bandit', 'bear')
    ...
    onPeerDisconnected(peerId) {
        for (const [entityId, entity] of this.entities) {
            if (entity.authorityId === peerId) {
                ...
                console.log(`[${this.entityType} ${entityId}] Authority transferred from ${peerId} to ${entity.authorityId}`);
            }
        }
    }
    ...
    /**
     * Get 3x3 chunk keys around a center position
