...
export class AIController extends BaseAIController {
    ...
    checkSpawnsOnTick(chunkX, chunkZ) {
        ...
        // HOISTED: Pre-fetch all player positions once (was fetched per-tent per-player)
        ...
        for (let dx = -1; dx <= 1; dx++) {
            for (let dz = -1; dz <= 1; dz++) {
                ...
                for (const tent of tents) {
                    ...
                    // Uses string < comparison (lexicographic) - deterministic for "session_xxx" IDs
                    ...
                    for (const [playerId, pos] of playerPositions) {
                        ...
                        if (dxP * dxP + dzP * dzP < spawnRangeSq) {
                            // String comparison - works correctly for "session_xxx" format
