...
export const CONFIG = {
    ...
    // ==========================================
    AI_AUTHORITY: {
        SOFT_CAP: 15,  // Skip new spawns above this - let other peers handle them
    ...
    FACTION_ZONES: {
        1: { name: 'Southguard', minX: -2000, maxX: 2000, minZ: -1000, maxZ: 0 },
        3: { name: 'Northmen', minX: -2000, maxX: 2000, minZ: 0, maxZ: 1000 }
    },
    ...
    CRATE_VEHICLES: {
        ...
        // Ship2 deck positions (model pre-scaled 0.25x in Blender)
        ship2: {
            slots: [
                { x: -0.125, y: 0.05, z: -0.375 },  // Port-aft
                { x: 0.125,  y: 0.05, z: -0.375 },  // Starboard-aft
                { x: -0.125, y: 0.05, z: -0.75 },   // Port-forward
                { x: 0.125,  y: 0.05, z: -0.75 }    // Starboard-forward
            ]
        },

        // Ship2 artillery positions (port/starboard broadside, facing outward)
        ship2_artillery: {
            slots: [
                { x: -0.225, y: 0.125, z: -0.625, rotation: -Math.PI / 2 },  // Port (faces left)
                { x: 0.225,  y: 0.125, z: -0.625, rotation: Math.PI / 2 }    // Starboard (faces right)
            ]
        },
    ...
    HORSE_VEHICLES: {
        ...
        CAPACITY: {
            ship2: 4        // Four horses on ship2 deck
        },

        // Ship2 deck positions for horses (model pre-scaled 0.25x in Blender)
        ship2: {
            slots: [
                { x: -0.125, y: 0.075, z: 0.125 },   // Port-midship
                { x: 0.125,  y: 0.075, z: 0.125 },   // Starboard-midship
                { x: -0.125, y: 0.075, z: 0.5 },     // Port-stern
                { x: 0.125,  y: 0.075, z: 0.5 }      // Starboard-stern
            ]
        },
    ...
    CART_PHYSICS: {
        ...
        // Speed penalties when towing
        EMPTY_CART_SPEED_MULTIPLIER: 0.9,     // 10% slower with empty cart (player & horse)
        LOADED_CART_SPEED_MULTIPLIER: 0.5,    // 50% slower with cart + crate (player on foot)
        ...
        MOUNTED_LOADED_CART_SPEED_MULTIPLIER: 0.667,  // TEST: Horse + loaded cart = 1.0 off-road, 2.0 on-road
    },

    // ==========================================
    // ARTILLERY PHYSICS (Horse-Only Towing)
    // ==========================================
    ARTILLERY_PHYSICS: {
        // Hitch geometry (same as cart)
        HITCH_OFFSET: 0.4,
        TETHER_LENGTH: 0.3,

        // Movement (slower than cart - heavier)
        ARTILLERY_SPEED: 2.0,
        PIVOT_SPEED: 0.08,
        ...
        // Speed penalty (heavier than cart)
        SPEED_MULTIPLIER: 0.667,              // TEST: was 0.7 - now horse+artillery = 1.0 off-road
        ...
        MOUNTED_TURN_RATE_MULTIPLIER: 0.5,   // Horse turns 50% slower with artillery
    },
    ...
    OBJECTS: {
        ...
        TREE_TYPES: new Set(['oak', 'fir', 'pine', 'cypress', 'apple', 'vegetables', 'deertree']),
    ...
    CONSTRUCTION: {
        ...
        GRID_DIMENSIONS: {
            dock: { width: 4.0, depth: 10.0, height: 3.0 },
        ...
        MATERIALS: {
            ...
            artillery: {
                oakplank: 1
                // TEMP: parts requirement removed for testing
            },
            wall: {
        ...
        STRUCTURE_QUALITY_CAPS: {
            ...
            sailboat: 15,   // Max quality 15 = ~48.6 hours lifespan (2 days)
            ship2: 100,     // Max quality 100 = ~738 hours lifespan (~30 days), survives 1 artillery hit
            ship: 15,       // Max quality 15 = ~48.6 hours lifespan (2 days)
    ...
    MARKET: {
        ...
        PRICES: {
            ...
            parts: { buyPrice: 200, sellPrice: 200, minQuantity: 0, maxQuantity: 300 },
