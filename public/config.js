...
export const CONFIG = {
    ...
    ACTIONS: {
        ...
        COMBINE_DURATION: 6000,          // 6 seconds to combine items (grass -> rope, rope -> net)
    ...
    CHUNKS: {
        ...
        PHYSICS_RADIUS: 2,               // Physics colliders in 5x5 grid (matches LOAD_RADIUS)
                                         // Required for navigation maps to have obstacles
    ...
    CONSTRUCTION: {
        ...
        GRID_DIMENSIONS: {
            ...
            campfire: { width: 1.0, depth: 1.0, height: 1.5 },
            ...
            apple: { radius: 0.7 },    // diameter 1.4 (increased from 0.5)

            // Logs - rectangular bounds (all logs use same dimensions)
            oak_log: { width: 0.1, depth: 1.5 },
            pine_log: { width: 0.1, depth: 1.5 },
            fir_log: { width: 0.1, depth: 1.5 },
            cypress_log: { width: 0.1, depth: 1.5 },
            log: { width: 0.1, depth: 1.5 },  // generic log item
    ...
    AUDIO: {
        ...
        // Ocean ambient sound settings
        OCEAN_SOUND_FADE_START_X: -10,  // X position where fade-in begins
        OCEAN_SOUND_FULL_VOLUME_X: 7,   // X position where full volume is reached
        OCEAN_SOUND_MAX_VOLUME: 0.15,   // Maximum volume for ocean sound (reduced by 50%)
        OCEAN_CROSSFADE_DURATION: 6,    // Seconds for crossfade between loops

        // Plains ambient sound settings
        PLAINS_SOUND_FADE_START_X: 0,   // X position where fade-in begins
        PLAINS_SOUND_FULL_VOLUME_X: -10, // X position where full volume is reached
        PLAINS_SOUND_MAX_VOLUME: 0.3,   // Maximum volume for plains sound
        PLAINS_CROSSFADE_DURATION: 6,   // Seconds for crossfade between loops

        // Mountain ambient sound settings
        MOUNTAIN_SOUND_FADE_START_Y: 4.0, // Y position (altitude) where fade-in begins
        MOUNTAIN_SOUND_FULL_VOLUME_Y: 5.5, // Y position where full volume is reached
        MOUNTAIN_SOUND_MAX_VOLUME: 0.3,  // Maximum volume for mountain sound
        MOUNTAIN_CROSSFADE_DURATION: 6,  // Seconds for crossfade between loops
    ...
    MARKET: {
        ...
        PRICES: {
            ...
            rope: { buyPrice: 20, sellPrice: 14, minQuantity: 0, maxQuantity: 100 },
