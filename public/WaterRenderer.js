...
const waterVertexShader = `
    ...
    void main() {
        ...
        // Optimization: Skip expensive calculations for very deep water (> 15 units)
        // In deep ocean, we don't need detailed slopes for foam/shore effects
        bool isDeepWater = waterDepth > 15.0;

        // No wave dampening - full waves at all depths
        float depthFactor = 1.0;

        // Optimized wave system - compute sin/cos once and reuse for displacement + derivatives
        float waveDisplacement = 0.0;
        float slopeX = 0.0;
        float slopeZ = 0.0;

        // Wave 1: Primary wave (always calculated)
        float t1 = u_time * 1.5 * u_wave_speed;
        float phaseX1 = worldXZ.x * u_wave_frequency + t1;
        float phaseY1 = worldXZ.y * u_wave_frequency * 0.7 + t1 * 0.8;
        float sX1 = sin(phaseX1);
        float cX1 = cos(phaseX1);
        float sY1 = sin(phaseY1);
        float cY1 = cos(phaseY1);
        waveDisplacement += sX1 * cY1 * 0.5;
        slopeX += u_wave_frequency * cX1 * cY1 * 0.5;
        slopeZ += -u_wave_frequency * 0.7 * sX1 * sY1 * 0.5;

        // Wave 2: Secondary wave (skip derivatives for deep water)
        vec2 pos2 = worldXZ * 1.8;
        float freq2 = u_wave_frequency * 1.7;
        float t2 = u_time * 2.1 * u_wave_speed;
        float phaseX2 = pos2.x * freq2 + t2;
        float phaseY2 = pos2.y * freq2 * 0.7 + t2 * 0.8;
        float sX2 = sin(phaseX2);
        float cX2 = cos(phaseX2);
        float cY2 = cos(phaseY2);
        waveDisplacement += sX2 * cY2 * 0.3;

        if (!isDeepWater) {
            // Only compute full derivatives for shallow water
            float sY2 = sin(phaseY2);
            slopeX += freq2 * cX2 * cY2 * 0.3;
            slopeZ += -freq2 * 0.7 * sX2 * sY2 * 0.3;

            // Wave 3: Tertiary wave (shallow water only)
            vec2 pos3 = worldXZ * 2.3;
            float freq3 = u_wave_frequency * 0.9;
            float t3 = u_time * 1.8 * u_wave_speed;
            float phaseX3 = pos3.x * freq3 + t3;
            float phaseY3 = pos3.y * freq3 * 0.7 + t3 * 0.8;
            waveDisplacement += sin(phaseX3) * cos(phaseY3) * 0.2;
        }

        // Apply wave displacement and cache slope for fragment shader
...
const waterFragmentShader = `
    ...
    void main() {
        ...
        // Simple transparency: starts fading in at depth 0.2, fully opaque at 3.2
        float alpha = clamp((local_depth - 0.2) / 3.0, 0.0, 0.8);
        ...
        if (u_enable_foam) {
            ...
            // Subtle variation instead of pulsing - keeps foam visible
            ...
            // Gentle variation - foam stays mostly visible (85% to 100% intensity)
            foam *= combinedPulse * 0.15 + 0.85; // Much less fading

            // No animated boundaries - foam zone stays constant
            // (Removed breathing effect to keep foam visible)
            ...
            if (local_depth < 0.4) {
                ...
                // Smoother, rounder curved patterns with less frequency variation
                float curvedWave1 = sin(distFromOrigin * 0.08 - u_time * 0.8) * 0.5 + 0.5;
                float curvedWave2 = sin(distFromOrigin * 0.05 - u_time * 0.6 + 1.57) * 0.5 + 0.5;
                float wavePattern = curvedWave1 * 0.5 + curvedWave2 * 0.5;

                // Stable foam with rounded texture - stays visible (70-100%)
                float stableWave = 0.7 + wavePattern * 0.3;

                // Combine with shore proximity for rounded foam edge
                float waveRunUp = softEdge * stableWave;
                ...
                // Fade in foam over first 0.5 depth (transparent at shore edge)
                float shoreFade = smoothstep(0.0, 0.5, local_depth);
                shoreFoam *= shoreFade;
            ...
            // === DEEP EDGE CURVED FOAM ===
            // Add curved patterns where foam fades out in deeper water

            // Add noise to depth FIRST to break up straight edges from flat terrain
            float depthNoise = perlinNoise(vec3(vWorldPosition.x * 0.4, vWorldPosition.z * 0.4, u_time * 0.2));
            float noisyDepth = local_depth + depthNoise * 0.2; // Vary depth by +/- 0.2 units

            float deepEdgeRange = u_foam_max_depth - 0.3; // Start curving 0.3 units before max depth

            // Use smooth falloff instead of hard cutoff - no more straight lines!
            // This creates a soft zone around the deep edge
            float deepEdgeProximity = smoothstep(deepEdgeRange - 0.2, deepEdgeRange, noisyDepth) *
                                     (1.0 - smoothstep(u_foam_max_depth, u_foam_max_depth + 0.5, noisyDepth));

            // Only compute curves if we're in the zone (optimization)
            if (deepEdgeProximity > 0.01) {
                // Different wave origin for variety (moves opposite direction from shore waves)
                float deepWaveOriginX = sin(u_time * 0.25) * 25.0;
                float deepWaveOriginZ = cos(u_time * 0.2) * 20.0;

                // Radial distance creates curved fronts
                float deepDistFromOrigin = length(vec2(vWorldPosition.x - deepWaveOriginX, vWorldPosition.z - deepWaveOriginZ));

                // Curved wave patterns create texture variation (not full fade out)
                float deepCurvedWave1 = sin(deepDistFromOrigin * 0.12 - u_time * 1.8) * 0.5 + 0.5;
                float deepCurvedWave2 = sin(deepDistFromOrigin * 0.08 - u_time * 1.3 + 1.57) * 0.5 + 0.5;
                float wavePattern = deepCurvedWave1 * 0.5 + deepCurvedWave2 * 0.5;

                // Base foam stays visible (0.7) + wave texture adds variation (0.3)
                // This keeps foam visible while adding curved patterns
                float stableFoam = 0.7 + wavePattern * 0.3; // Range: 0.7 to 1.0

                float deepCurvedFoam = deepEdgeProximity * stableFoam * 0.6;

                // Add to base foam
                foam = foam + deepCurvedFoam;
            }
...
export class WaterRenderer {
    constructor(scene = null, waterLevel = GAME_CONFIG.WATER.LEVEL, terrainRenderer = null, sceneManager = null) {
        ...
        this.reusableFrustum = new THREE.Frustum(); // Reuse instead of creating every frame
        this.reusableMatrix = new THREE.Matrix4();  // Reuse instead of creating every frame
        ...
        // Setup GUI for water color controls
        // this.setupGUI();
...
createDefaultHeightTexture() {
    ...
    createSharedMaterial() {
        ...
        const shallowColor = new THREE.Color(0x55ece2); // Light cyan shallow water
        const deepColor = new THREE.Color(0x00c281);    // Teal deep water
        ...
        this.uniforms = {
            ...
            u_chunk_size: { value: ChunkCoordinates.getChunkSize() },
    ...
    setupGUI() {
        // Create GUI for water color controls
        this.gui = new GUI({ title: 'Water Colors' });
        this.gui.domElement.style.position = 'absolute';
        this.gui.domElement.style.top = '10px';
        this.gui.domElement.style.right = '10px';

        // Water color parameters
        this.waterColorParams = {
            shallowColor: '#55ece2',
            deepColor: '#00c281'
        };

        this.gui.addColor(this.waterColorParams, 'shallowColor')
            .name('Shallow Water')
            .onChange((value) => {
        ...
        this.gui.addColor(this.waterColorParams, 'deepColor')
            .name('Deep Water')
            .onChange((value) => {
                const color = new THREE.Color(value);
    ...
    generateHeightTexture(chunkX, chunkZ, heightCalculator) {
        ...
        const chunkSize = ChunkCoordinates.getChunkSize();
        const canvas = document.createElement('canvas');
        canvas.width = canvas.height = size;
        const ctx = canvas.getContext('2d');
        const imgData = ctx.createImageData(size, size);
...
return texture;
    ...
    addWaterChunk(chunkX, chunkZ, heightTexture = null) {
        ...
        const chunkSize = ChunkCoordinates.getChunkSize();
        const geometry = new THREE.PlaneGeometry(chunkSize, chunkSize, 100, 100);
        ...
        mesh.renderOrder = 200; // Render after billboards (100) for proper transparency sorting
    ...
    update(time, camera = null) {
        ...
        let frustum = null;
        if (camera) {
            frustum = this.reusableFrustum;
            this.reusableMatrix.multiplyMatrices(camera.projectionMatrix, camera.matrixWorldInverse);
            frustum.setFromProjectionMatrix(this.reusableMatrix);
        }
