...
float terrainHeight = mix(-10.0, 80.0, heightNormalized);
    ...
    void main() {
        ...
        float transitionFactor = smoothstep(1.5, 2.0, depth);
        ...
        float alpha;
        if (depth <= 1.5) {
            alpha = mix(0.3, 0.8, shallowFactor);
        } else if (depth <= 2.0) {
            alpha = mix(0.8, 1.0, (depth - 1.5) / 0.5);
        } else {
            alpha = 1.0;
        }
        ...
        float shorelineFoam = 1.0 - smoothstep(0.0, 1.0, depth);
