...
        // NOTE: The 0.7 and 0.8 are kept as is, as they control the wave shape (not scale)
...
        
        // CHANGED: All speed values (2.0, 1.5, 2.5) are divided by 8 to slow waves for massive scale
        waveDisplacement += wave(pos.xz, u_wave_frequency, 0.25) * 0.4; // 2.0 / 8 = 0.25
        waveDisplacement += wave(pos.xz * 1.5, u_wave_frequency * 1.3, 0.1875) * 0.3; // 1.5 / 8 = 0.1875
        waveDisplacement += wave(pos.xz * 2.1, u_wave_frequency * 0.8, 0.3125) * 0.2; // 2.5 / 8 = 0.3125
        
...
        // The 2.0 speed multiplier is also divided by 8 here
        vWorldNormal = normalize(vec3(
            -u_wave_height * u_wave_frequency * cos(pos.x * u_wave_frequency + u_time * 0.25), // 2.0 / 8 = 0.25
            1.0,
            -u_wave_height * u_wave_frequency * cos(pos.z * u_wave_frequency + u_time * 0.25) // 2.0 / 8 = 0.25
        ));
...
        // CHANGED: All UV repeat factors (20.0, 15.0, 25.0) are divided by 8
        vec2 scrolledUvA = vUv * 2.5 + vec2(u_time * 0.005, u_time * 0.004);   // 20.0 / 8 = 2.5
        vec2 scrolledUvB = vUv * 1.875 + vec2(u_time * -0.004, u_time * 0.003); // 15.0 / 8 = 1.875
        vec2 scrolledUvC = vUv * 3.125 + vec2(u_time * 0.003, u_time * -0.002); // 25.0 / 8 = 3.125
...
        // CHANGED: Depth clamp distance (5.0) is multiplied by 8 to 40.0
        float depth = clamp(vDepth / 40.0, 0.0, 1.0); // 5.0 * 8 = 40.0
        
...
        // CHANGED: Foam UV repeat factor (20.0) is divided by 8 to 2.5
        vec2 foamUv = vUv * 2.5 + vec2(u_time * 0.002, u_time * 0.001); // 20.0 / 8 = 2.5
...
        // CHANGED: Caustics UV repeat factor (25.0) is divided by 8 to 3.125
        vec2 causticsUv = vUv * 3.125 + vec2(u_time * 0.005, u_time * 0.003); // 25.0 / 8 = 3.125
...
            u_deep_color: { value: new THREE.Color(0x001f5f) },    // Deep ocean blue
            u_foam_color: { value: new THREE.Color(0xffffff) },    // White foam (fallback)
            // CHANGED: u_wave_height multiplied by 8 (0.3 * 8 = 2.4)
            u_wave_height: { value: 2.4 },                        // Massive swells (2.4 meters)
            // CHANGED: u_wave_frequency divided by 8 (0.08 / 8 = 0.01)
            u_wave_frequency: { value: 0.01 },                    // Long, stretched waves
...
        
        // CHANGED: All speed values (2.0, 1.5, 2.5) are divided by 8 to 0.25, 0.1875, 0.3125
        let waveHeight = 0;
        waveHeight += Math.sin(x * freq + time * 0.25) * Math.cos(z * freq * 0.7 + time * 0.2) * 0.4; // 2.0/8=0.25, 1.6/8=0.2
        waveHeight += Math.sin(x * freq * 1.5 + time * 0.1875) * Math.cos(z * freq * 0.91 + time * 0.15); // 1.5/8=0.1875, 1.2/8=0.15
        waveHeight += Math.sin(x * freq * 2.1 + time * 0.3125) * Math.cos(z * freq * 1.47 + time * 0.25) * 0.2; // 2.5/8=0.3125, 2.0/8=0.25
        
