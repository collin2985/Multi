...
export const TERRAIN_CONFIG = {
    CLIPMAP_LEVELS: 6,
    CLIPMAP_SIZE: 129,          // 2^n + 1 for clean alignment
    ...
    WAVE_DAMPING_MIN_DEPTH: 1.0,  // Waves fully suppressed from depth 0 to this depth
    WAVE_DAMPING_MAX_DEPTH: 5.0,  // Waves reach full strength at this depth
...
const SHADER_PNG_TRIPLANAR = `
    vec3 triplanarPNG(sampler2D tex, vec3 pos, vec3 normal, float repeat) {
        ...
        // Sample from 3 directions with tiling
        vec2 uvX = pos.zy * repeat;
        vec2 uvY = pos.xz * repeat;
        vec2 uvZ = pos.xy * repeat;

        vec3 xProj = texture2D(tex, uvX).rgb;
        vec3 yProj = texture2D(tex, uvY).rgb;
        vec3 zProj = texture2D(tex, uvZ).rgb;
    ...
    vec3 getPngTerrainColor(vec3 stablePos, vec3 normal, float height, float slope, float repeat) {
        ...
        // Sample PNG textures with triplanar mapping
        ...
        vec3 grass = mix(mix(grass1, grass2, grassMix), dirt, dirtMix);

        // Sand: wet sand fades out from 0 to 1, dry sand fades in from 0 to 3
        // But NOT on slopes where rock should show
        vec3 sandDry = triplanarPNG(texSand, stablePos, normal, repeat);
        vec3 sandWet = triplanarPNG(texSand2, stablePos, normal, repeat);
        // Wet: 100% at height 0, fades to 0% at height 1
        float wetness = smoothstep(1.0, 0.0, height);
        wetness *= (1.0 - slope);  // No wet sand on slopes (rock takes over)
        vec3 sand = mix(sandDry, sandWet, wetness);

        // Rock
        vec3 rock = triplanarPNG(texRock, stablePos, normal, repeat);

        // Snow (use procedural for now, or could add snow texture)
        vec3 snow = triplanarPNG(texSnow, stablePos, normal, repeat);

        return sand * sandWeight + grass * grassWeight + rock * rockWeight + snow * snowWeight;
    }
`;
...
const SHADER_TERRAIN_COLOR = `
    vec3 getTerrainTexture(vec3 stablePos, vec3 normal, float height, float slope, float distToCamera, float texScale, int texOctaves) {
        ...
        // Get procedural color (always computed for far terrain)
        int octaves = texOctaves;
        ...
        vec3 proceduralColor = triplanarTexture(stablePos, normal, scale, 2, octaves, distToCamera) * sandWeight
                             + triplanarTexture(stablePos, normal, scale, 0, octaves, distToCamera) * grassWeight
                             + triplanarTexture(stablePos, normal, scale, 1, octaves, distToCamera) * rockWeight
                             + triplanarTexture(stablePos, normal, scale, 3, octaves, distToCamera) * snowWeight;

        // Early out if fully procedural (far from camera)
        if (lodBlend >= 0.99) {
            return proceduralColor;
        }

        // Get PNG texture color (near camera)
        vec3 pngColor = getPngTerrainColor(stablePos, normal, height, slope, textureRepeat);

        // Blend between PNG (near) and procedural (far)
        vec3 baseColor = mix(pngColor, proceduralColor, lodBlend);

        // Sample dirt overlay for structure/tree dirt patches
        // Use vTexCoord (meshWorldOffset + pos.xz) which is STABLE and doesn't drift
        // vWorldPos drifts because modelMatrix changes when mesh re-centers
        ...
        // Only sample if within overlay bounds (overlay already has its own range limit)
        if (dirtUV.x >= 0.0 && dirtUV.x <= 1.0 && dirtUV.y >= 0.0 && dirtUV.y <= 1.0) {
            // Sample dirt (R), limestone road (G), and sandstone road (B) weights
            vec3 overlayWeights = texture2D(texDirtOverlay, dirtUV).rgb;
            float dirtWeight = overlayWeights.r;
            float roadWeight = overlayWeights.g;      // Limestone road (gray)
            float yellowRoadWeight = overlayWeights.b; // Sandstone road (yellow)

            // Blend dirt over base terrain (layer 1)
            if (dirtWeight > 0.01) {
                // Sample dirt texture with higher repeat for more detail (matching old system's repeat * 1.5)
                vec3 dirtColor = triplanarPNG(texDirt, stablePos, normal, textureRepeat * 1.5);
                baseColor = mix(baseColor, dirtColor, dirtWeight);
            }

            // Blend limestone road over dirt/terrain (layer 2)
            if (roadWeight > 0.01) {
                vec3 roadColor = triplanarPNG(texRoad, stablePos, normal, textureRepeat * 4.0);
                baseColor = mix(baseColor, roadColor, roadWeight);
            }

            // Blend sandstone road over dirt/terrain (layer 2 - same priority as limestone)
...
const SHADER_FOG = `
    vec3 applyFog(vec3 color, vec3 worldPos, float height) {
        float fogDist = length(worldPos - cameraPosition);
        float fogFactor = smoothstep(fogNear, fogFar, fogDist);
        // Low altitude fog disabled for testing
        // float heightFog = smoothstep(20.0, 0.0, height) * 0.3;
        // fogFactor = max(fogFactor, heightFog * (1.0 - fogFactor));
...
const WATER_VERTEX_SHADER = `
    ...
    void main() {
        ...
        pos += waveDisplacement * waveDamping;
        ...
        // Output world position (after wave displacement, for lighting/fog)
...
const WATER_FRAGMENT_SHADER = `
    ...
    void main() {
        ...
        // Smooth shore transition instead of hard discard (prevents flickering)
        // Widened range reduces jittering when player moves
        float shoreBlend = smoothstep(-0.2, 0.15, terrainHeight);
        if (shoreBlend > 0.99) {
            discard;
        }
        ...
        if (enableSSS > 0.5) {
            ...
            float glow = heightGlow * (0.7 + glowNoise * 0.3) * sssStrength;
            color += sssColor * glow;
        }
        ...
        if (depth > 0.0 && depth < foamMaxDepth * 3.0) {
            ...
            if (shoreProximity > 0.01) {
                // Edge noise for natural variation
                ...
                float shoreFoam = (waveRunUp * 0.5 + smoothFoamLine + foamPatches) * 0.8;
            ...
            // Limit foam to texture depth limit
            float foamDepthFade = 1.0 - smoothstep(foamTextureDepthLimit * 0.7, foamTextureDepthLimit, depth);
            foam *= foamDepthFade;

            // === MULTI-LAYER FOAM TEXTURE ===
            float textureBlend = 1.0 - smoothstep(foamTextureDepthLimit * 0.7, foamTextureDepthLimit, depth);
            if (foam > 0.0 && textureBlend > 0.01) {
                // Three texture layers at different scales
                ...
                // Combine layers
                float foamTexCombined = (foamTex1 * layer1Weight +
                ...
                foamTexCombined = mix(1.0, foamTexCombined, textureBlend);
        ...
        float oceanFoamFade = 1.0 - smoothstep(0.05, 0.20, continentMask);
        foam *= oceanFoamFade;
        ...
        // Apply fog (must match terrain fog exactly)
        float fogDist = length(vWorldPos - cameraPosition);
        float fogFactor = smoothstep(fogNear, fogFar, fogDist);
        color = mix(color, fogColor, fogFactor);

        // Apply shore fade for smooth transition (shoreBlend defined at start of main())
        alpha *= (1.0 - shoreBlend);
...
export class TerrainGenerator {
    ...
    getWorldHeight(worldX, worldZ) {
        ...
        if (this.leveledAreas.length > 0) {
            const leveled = this.getLeveledHeight(worldX, worldZ);
            if (leveled !== null) {
     ...
     */
    addLeveledArea(centerX, centerZ, width, depth, targetHeight, rotationDegrees) {
        // Pre-compute rotation values for fast lookup
        // Negate rotation to match coordinate system (from old terrain.js)
        const rotationRad = -rotationDegrees * Math.PI / 180;
        const cosR = Math.cos(-rotationRad);  // For inverse rotation
        ...
        const newArea = {
            ...
            cosR,
            sinR,
            // Store original values for removal matching
            width,
            depth,
            rotationDegrees
        };
        ...
        console.log(`[TerrainGenerator] Added leveled area at (${centerX.toFixed(1)}, ${centerZ.toFixed(1)}) size ${width}x${depth} height=${targetHeight.toFixed(2)} rot=${rotationDegrees}. Total areas: ${this.leveledAreas.length}`);
    }
     ...
     */
    getLeveledHeight(worldX, worldZ) {
        const transitionSize = 1.0;  // 1 unit smooth transition zone

        for (const area of this.leveledAreas) {
            // Transform point to structure's local coordinate system
            const relX = worldX - area.centerX;
            const relZ = worldZ - area.centerZ;

            // Rotate to structure's local space (inverse rotation)
            ...
            const edgeDist = Math.max(distX - area.halfWidth, distZ - area.halfDepth);

            if (edgeDist <= 0) {
                // Inside structure footprint - fully leveled
                ...
                return { height: area.targetHeight, blend: 1.0 - smoothT };
            }
        }
...
export class SeamMesh {
    constructor(levelIndex, fineLevel, coarseLevel, noise, heightScale) {
        ...
        this.mesh.renderOrder = 100 + levelIndex;
    }
    ...
    createMaterial() {
        this.material = new THREE.ShaderMaterial({
            uniforms: {
                ...
                grassLight: { value: new THREE.Vector3(0.122, 0.459, 0.208) },
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    color = applyFog(color, vWorldPos, vHeight);

                    // Distance fade to prevent pop-out at terrain edge
    ...
    sampleLevelHeight(level, worldX, worldZ) {
        if (!level || !level.initialized) {
            return this.noise.getHeight(worldX, worldZ) * this.heightScale;
        }
...
export class ClipmapLevel {
    ...
    update(snappedCenterX, snappedCenterY, realViewerX, realViewerY, coarserLevel, deltaTime = 0.016) {
        ...
        // Use instant update for structure placement to avoid wave animation
        const lerpSpeed = (this.firstUpdate || this.forceInstantUpdate) ? 1.0 : TERRAIN_CONFIG.LOD_LERP_SPEED * timeScale;
    ...
    sampleCoarseHeight(worldX, worldY, coarserLevel) {
        ...
        const h00 = coarserLevel.displayHeights[cy0 * coarserLevel.size + cx0];
        const h10 = coarserLevel.displayHeights[cy0 * coarserLevel.size + cx1];
        const h01 = coarserLevel.displayHeights[cy1 * coarserLevel.size + cx0];
        const h11 = coarserLevel.displayHeights[cy1 * coarserLevel.size + cx1];
...
export class GeometryClipmap {
    ...
    loadTerrainTextures() {
        ...
        for (const [uniformName, filename] of Object.entries(textureNames)) {
            loader.load(
                ...
                undefined,
                (error) => {
                    console.warn(`Failed to load terrain texture ${filename}:`, error);
                }
            );
        }
    }
    ...
    createLevels() {
        ...
        const baseSize = TERRAIN_CONFIG.CLIPMAP_SIZE;
        const baseScale = TERRAIN_CONFIG.TERRAIN_SCALE;

        const hiddenLevelScale = baseScale * Math.pow(2, numLevels);
        const hiddenLevel = new ClipmapLevel(-1, baseSize, hiddenLevelScale, TERRAIN_CONFIG.HEIGHT_SCALE, this.noise, numLevels + 1);
        ...
        for (let i = 0; i < numLevels; i++) {
            ...
            const level = new ClipmapLevel(i, baseSize, levelScale, TERRAIN_CONFIG.HEIGHT_SCALE, this.noise, numLevels);
    ...
    getStats() {
        ...
        return { triangles: totalTriangles, activeLevels, totalLevels: this.levels.length };
    }
    ...
    forceRefreshRegion(centerX, centerZ, radius) {
        console.log(`[GeometryClipmap] forceRefreshRegion called at (${centerX.toFixed(1)}, ${centerZ.toFixed(1)}) radius=${radius.toFixed(1)}`);
        
        // Mark affected levels for full update on next frame
        // Inner levels (higher index = finer detail) are most likely affected
        let levelsRefreshed = 0;
        ...
        console.log(`[GeometryClipmap] Refreshed ${levelsRefreshed} levels`);
...
export class DepthTextureSystem {
    ...
    render(renderer, viewerX, viewerZ) {
        ...
        // Using 4x texel size reduces flickering from depth texture edge movement
        const texelSize = TERRAIN_CONFIG.DEPTH_RANGE / TERRAIN_CONFIG.DEPTH_TEXTURE_SIZE;
        const snapSize = texelSize * 4;
...
export class WaterSystem {
    constructor(scene, depthSystem, foamTexture) {
        ...
        this.currentUniforms = {
            ...
            // Effect toggles
            enableSSS: 1.0,
            enableDetailNormals: 1.0,
            enableCrestColor: 1.0,
            enableGlitter: 1.0,
            enableDeepColor: 1.0,
    ...
    createChunk(cx, cz) {
        ...
        const material = new THREE.ShaderMaterial({
            ...
            transparent: true,
            // NOTE: depthWrite: false is intentional for transparent water.
            // This allows proper blending with terrain beneath but may cause overdraw
            // with overlapping water chunks. Acceptable since water chunks don't overlap
            // and the visual quality benefit outweighs the minor overdraw cost.
            depthWrite: false,
            side: THREE.DoubleSide,
            // Push water slightly back in depth buffer to prevent z-fighting at shore edges
            polygonOffset: true,
            polygonOffsetFactor: 1,
            polygonOffsetUnits: 1
        });
        ...
        // Render after all terrain LOD levels (100-105)
    ...
    getWaveHeight(x, z) {
        ...
        // Process both waves (same as vertex shader)
        const waves = [this.waveA, this.waveB];
        for (const wave of waves) {
            ...
            const f = k * ((dX * x + dZ * z) - c * this.time * 0.5);

            // Amplitude: a = steepness / k
            const a = steepness / k;

            // Y displacement is a * sin(f)
            totalY += a * Math.sin(f);
        }
