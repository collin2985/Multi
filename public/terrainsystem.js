...
const SHADER_TEXTURES = `
    ...
    vec3 triplanarTexture(vec3 pos, vec3 normal, float scale, int texType, int octaves, float distToCamera) {
        ...
        // Saves 2 biome texture calls (each with fbm3D noise) when blend.y > 0.98
        if (blend.y > 0.98) {
...
const SHADER_PNG_TRIPLANAR = `
    vec3 triplanarPNG(sampler2D tex, vec3 pos, vec3 normal, float repeat) {
        ...
        // Fast path: mostly flat terrain (Y projection dominates)
        // Saves 2 texture samples when blend.y > 0.98 (~21 degree slope threshold)
        if (blend.y > 0.98) {
...
const SHADER_NORMAL_PERTURBATION = `
    vec3 perturbNormal(vec3 normal, vec3 stablePos, float distToCamera) {
        // Skip perturbation for distant terrain - saves 4 noise calls per pixel
...
const WATER_FRAGMENT_SHADER = `
    ...
    void main() {
        ...
        float distClose = 150.0;
        float distMedium = 250.0;
...
export class TerrainGenerator {
    ...
    addLeveledArea(centerX, centerZ, width, depth, targetHeight, rotationDegrees) {
        ...
        const newArea = {
            ...
            maxReach: Math.sqrt(halfWidth * halfWidth + halfDepth * halfDepth) + 1.2,
    ...
    getLeveledHeight(worldX, worldZ, proceduralHeight) {
        const transitionSize = 1.2;  // 1.2 unit transition zone

        for (const area of this.leveledAreas) {
            // Only level terrain that is higher than structure placement
            // This cuts into hillsides but doesn't fill in lower terrain
            if (proceduralHeight <= area.targetHeight) {
                continue;
            }

            // AABB pre-filter: skip areas too far away (avoids expensive rotation)
            ...
            if (edgeDist <= 0) {
                ...
                // In transition zone - cubic ease-in (flat near structure, steep at edge)
        ...
        this.material = new THREE.ShaderMaterial({
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    if (levelIndex == 0) {
                        distanceFade = 1.0 - smoothstep(fadeStart, fadeEnd, distToCamera);
                    }
...
export class WaterSystem {
    ...
    createChunk(cx, cz) {
        ...
        const material = new THREE.ShaderMaterial({
            ...
            depthWrite: false,
            side: THREE.DoubleSide,
            // Larger offset pushes water further back in depth buffer
            // This prevents z-fighting at shore edges where surfaces are nearly coplanar
            polygonOffset: true,
