...
export class InteractionManager {
    constructor(game) {
        ...
        this.rockTypes = new Set(['limestone', 'sandstone', 'clay', 'iron', 'vegetables']);
    ...
    checkProximityToObjects() {
        ...
        const mobileEntityColliders = this.physicsManager.querySphere(
            this.game.playerObject.position,
            mobileEntityRadius,
            COLLISION_GROUPS.PLACED | COLLISION_GROUPS.STRUCTURE
        );
        ...
        const isPilotingBoat = this.gameState.mobileEntityState?.phase === 'piloting' &&
            ['boat', 'sailboat', 'ship2'].includes(this.gameState.mobileEntityState?.entityType);
        const entityType = this.gameState.mobileEntityState?.entityType;
        ...
        if (isPilotingBoat) {
            ...
            // Cart: single crate capacity (use legacy isLoaded check)
            hasRoomForCrate = !crateState?.isLoaded;
        }
        ...
        const shouldTrackMannableArtillery = !this.gameState.artilleryManningState?.isManning &&
                                             !this.gameState.mobileEntityState?.isActive &&
                                             !this.gameState.cartAttachmentState?.isAttached &&
                                             !this.gameState.artilleryAttachmentState?.isAttached;
        ...
        const isPilotingShip2 = this.gameState.mobileEntityState?.phase === 'piloting' &&
            this.gameState.mobileEntityState?.entityType === 'ship2';
        ...
        const currentArtilleryCount = artilleryShipState?.loadedArtillery?.length || 0;
        ...
        const currentHorseCount = horseShipState?.loadedHorses?.length || 0;
        ...
        this.activeProximityObjects.forEach((object, objectId) => {
            ...
            if (object.userData.isConstructionSite) {
                ...
                if (modelType === 'artillery') {
                    ...
                    const isOwnArtillery = this.gameState.artilleryAttachmentState?.attachedArtillery === object;
                    if (!isOwnArtillery && (!mobileEntitySystem || !mobileEntitySystem.isOccupied(entityId))) {
                    ...
                    if (shouldTrackMannableArtillery) {
                        const isManning = this.gameState.artilleryManningState?.mannedArtillery === object;
                    ...
                    if (shouldTrackLoadableArtillery) {
                        const isManning = this.gameState.artilleryManningState?.mannedArtillery === object;
                        // Artillery must not be occupied, not being manned, not being towed
                        if (!isOwnArtillery && !isManning && (!mobileEntitySystem || !mobileEntitySystem.isOccupied(entityId))) {
        ...
        // Also process crates from this larger query when piloting a boat (crate loading range is 2.5 units)
        mobileEntityColliders.forEach(colliderHandle => {
            ...
            if (this.mobileEntityTypes.has(modelType)) {
                ...
                const entityId = sceneObject.userData.objectId;
                const mobileEntitySystem = this.game.mobileEntitySystem;
                const isOccupied = mobileEntitySystem && mobileEntitySystem.isOccupied(entityId);

                // Special case: ship2 with loaded artillery can have gunners even when pilot occupies it
                // Check if this is an occupied ship2 with available gunner positions
                let allowGunnerBoarding = false;
                if (isOccupied && modelType === 'ship2') {
                    // Faction check: neutral players cannot board occupied ships
                    const myFaction = this.gameState.factionId;
                    if (myFaction !== null) {
                        // Get the pilot's faction
                        const pilotClientId = mobileEntitySystem?.getOccupant(entityId);
                        let pilotFaction = null;
                        if (pilotClientId && this.game.networkManager) {
                            const peerData = this.game.networkManager.peerGameData.get(pilotClientId);
                            pilotFaction = peerData?.factionId ?? null;
                        }

                        // Only allow boarding if same faction (both non-null and equal)
                        if (pilotFaction !== null && myFaction === pilotFaction) {
                            // Check if ship has artillery loaded (children with modelType 'artillery')
                            const loadedArtillery = [];
                            sceneObject.traverse(child => {
                                if (child.userData?.modelType === 'artillery') {
                                    const artilleryId = child.userData.objectId;
                                    const artilleryOccupied = mobileEntitySystem?.isOccupied(artilleryId);
                                    if (!artilleryOccupied) {
                                        loadedArtillery.push(child);
                                    }
                                }
                            });
                            // Allow boarding if there are unoccupied artillery slots
                            if (loadedArtillery.length > 0) {
                                allowGunnerBoarding = true;
                                // Store the first available artillery for boarding
                                sceneObject.userData._availableGunnerArtillery = loadedArtillery[0];
                            }
                        }
                    }
                }

                // Skip if occupied and not allowing gunner boarding
                if (isOccupied && !allowGunnerBoarding) return;
            ...
            // Handle crates when piloting a boat (crate loading needs larger radius than 0.75)
            else if (modelType === 'crate' && shouldTrackLoadableCrates && isPilotingBoat) {
            ...
            // Handle artillery when piloting ship2 (artillery loading needs larger radius)
                ...
                const mobileEntitySystem = this.game.mobileEntitySystem;
                // Only track if not occupied, not manned by someone
                const isManning = this.gameState.artilleryManningState?.mannedArtillery === sceneObject;
                if (!isManning && (!mobileEntitySystem || !mobileEntitySystem.isOccupied(artilleryId))) {
                    const dx = this.game.playerObject.position.x - sceneObject.position.x;
                    const dz = this.game.playerObject.position.z - sceneObject.position.z;
                    const distanceSquared = dx * dx + dz * dz;
        ...
        const isPilotingWaterVehicle = mobileState.isActive &&
                                        ...
                                        ['boat', 'sailboat', 'ship2'].includes(mobileState.entityType);

        if (!mobileState.isActive) {
            // Not piloting - track nearest mobile entity for boarding
        ...
        // Only update if not currently towing a cart or artillery
        const isTowingCart = this.gameState.cartAttachmentState?.isAttached;
        const isTowingArtillery = this.gameState.artilleryAttachmentState?.isAttached;
        if (!isTowingCart && !isTowingArtillery) {
            if (nearestTowableEntity) {
                ...
                const isMounted = this.gameState.mobileEntityState?.isActive &&
                                  this.gameState.mobileEntityState?.phase === 'piloting' &&
                                  this.gameState.mobileEntityState?.entityType === 'horse';
        ...
        if (shouldTrackLoadableArtillery && nearestLoadableArtillery) {
            ...
            const proximityRange = 3.5;  // Artillery load range (larger than crate since ship is bigger)
        ...
        ui.updateButtonStates(
            ...
            this.gameState.artilleryAttachmentState,  // Artillery attachment state (horse-only towing)
            this.gameState.nearestMannableArtillery,  // Nearest mannable artillery (player can man to fire)
            this.gameState.artilleryManningState,  // Artillery manning state (firing)
            ...
            this.gameState.artilleryShipLoadState,  // Artillery loaded on ship2
            ...
            this.gameState.horseShipLoadState  // Horses loaded on ship2
        );
