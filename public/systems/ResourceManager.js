...
export class ResourceManager {
    ...
    handleResourceHarvested(payload) {
        ...
        if (!resourceObject && this.game.objectRegistry) {
            ...
            if (resourceObject) {
                console.log(`Found resource ${objectId} in objectRegistry`);
            }
        }
        ...
        if (!resourceObject) {
            this.game.scene.traverse((object) => {
                if (object.userData && object.userData.objectId === objectId && !object.userData.isBoundingBox) {
                    ...
                    console.log(`Found resource ${objectId} via scene traversal (fallback)`);
    ...
    handleOwnHarvest(resourceObject, harvestType) {
        ...
        if (tool) {
            ...
            // Examples:
            //   Tool Q100 + Resource Q100 = 1 loss
            //   Tool Q100 + Resource Q50 = 1 loss (rounded up)
            //   Tool Q50 + Resource Q100 = 2 loss
            //   Tool Q50 + Resource Q50 = 1 loss
            //   Tool Q10 + Resource Q100 = 10 loss
            const resourceQuality = resourceObject.userData.quality;
            const toolQuality = tool.quality;
            const durabilityLoss = Math.ceil(resourceQuality / toolQuality);
    ...
    requestLogSpawn(treeObject) {
        ...
        if (treeObject.isGrowing || (treeObject.growthScale && treeObject.growthScale < 1.0)) {
            console.log(`[ResourceManager] Tree ${treeObject.name} is still growing (scale: ${treeObject.growthScale}), no log spawn`);
