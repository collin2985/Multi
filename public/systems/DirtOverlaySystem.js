...
export class DirtOverlaySystem {
    ...
    drawDockPatch(worldX, worldZ, rotation = 0, materialType = 'limestone') {
        ...
        // Dock dimensions: 4 units wide, 10 units long
        const width = 4.0;
        const depth = 10.0;
        const halfW = (width / 2) * this.scale;
        const halfD = (depth / 2) * this.scale;
        ...
        // Width (2) along canvas-X (world-X), Depth (10) along canvas-Y (world-Z)
    ...
    drawRectangularDirt(worldX, worldZ, width, depth, rotation) {
        ...
        // Use source-over for layered drawing to prevent accumulation
        // Draw from OUTSIDE IN so inner layers overwrite outer ones
        const prevComposite = this.ctx.globalCompositeOperation;
        this.ctx.globalCompositeOperation = 'source-over';

        // Draw soft edge layers from outermost to innermost (so inner overwrites outer)
        const layers = 4;
        ...
        // Draw solid core last (overwrites inner edge layers)
        // Use RED channel only (shader reads .r for dirt, .g for roads)
