...
const EXCLUDED_TYPES = new Set(['dock', 'ship', 'campfire', 'tent', 'road', 'horse', 'boat', 'sailboat', 'crate', 'cart', 'mobilecrate', 'wall', 'artillery']);
...
export class DirtOverlaySystem {
    ...
    setSnapGrid(spacing) {
        ...
        console.log(`[DirtOverlay] Snap grid set to ${spacing} units`);
    }
        ...
        if (this.game?.gameState?.roads) {
            for (const chunkKey of this.current3x3Keys) {
                ...
                for (const road of roads) {
                    ...
                    this.drawPillRoadPatch(wrappedX, wrappedZ, rotationRad, road.materialType || 'limestone');
                    roadCount++;
                }
            }
        }
     ...
     * Follows same pattern as drawRectangularDirt for rotation/layers
     *
     * @param {number} worldX - Road center world X
     * @param {number} worldZ - Road center world Z
    ...
    drawPillRoadPatch(worldX, worldZ, rotation = 0, materialType = 'limestone') {
        ...
        // Draw solid core pill using roundRect with max corner radius
        this.ctx.fillStyle = `rgb(0, ${g}, ${b})`;
        this.roundRect(-halfD, -halfW, halfD * 2, halfW * 2, cornerRadius);
        this.ctx.fill();

        // Draw soft edge layers (same pattern as drawRectangularDirt)
        const marginPx = 0.25 * this.scale;  // Blend zone
        const layers = 3;
        for (let i = 1; i <= layers; i++) {
            const expand = (marginPx * i) / layers;
            const alpha = 1.0 - (i / layers);  // Fade out
            const gVal = Math.round(g * alpha * 0.5);
            const bVal = Math.round(b * alpha * 0.5);

            this.ctx.fillStyle = `rgb(0, ${gVal}, ${bVal})`;
            this.roundRect(
                -halfD - expand,
                -halfW - expand,
                (halfD + expand) * 2,
     ...
     * @param {number} worldX - Road center world X
     * @param {number} worldZ - Road center world Z
     * @param {number} rotation - Rotation in degrees (0 = length along X axis)
     * @param {string} materialType - 'limestone' or 'sandstone'
     */
    paintRoadImmediate(worldX, worldZ, rotation = 0, materialType = 'limestone') {
        ...
        this.drawPillRoadPatch(wrappedX, wrappedZ, rotationRad, materialType);
