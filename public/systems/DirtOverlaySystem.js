...
const EXCLUDED_TYPES = new Set(['dock', 'ship', 'campfire', 'tent', 'road', 'horse', 'boat', 'sailboat', 'crate', 'cart', 'mobilecrate']);
...
export class DirtOverlaySystem {
    ...
    constructor(chunkManager, game = null) {
        ...
        this.scale = this.textureSize / this.worldRange;  // ~2.56 pixels per world unit
    ...
    update(playerX, playerZ) {
        ...
        if (!this.loadingComplete) {
            ...
            if (serverStateReady && objectGenDone) {
                ...
                // Loading just completed - trigger initial paint below by ensuring chunk check fires
            } else {
        ...
        // Check if player crossed a chunk boundary OR this is the initial paint
        // (currentChunkX is null on first run after loading completes)
        if (chunkX !== this.currentChunkX || chunkZ !== this.currentChunkZ) {
            // Chunk boundary crossed (or initial paint) - clear and repaint entire 3x3 grid
            this.currentChunkX = chunkX;
            this.currentChunkZ = chunkZ;

            // Set center ONLY when repainting - dirtOverlayCenter must match
            // the center used when painting the canvas, otherwise UV calculation
            // will be wrong and dirt will appear to slide with player movement
            this.setCenter(playerX, playerZ);

            // Get the new 3x3 chunk keys
            ...
            // Synchronous clear and repaint (prevents flicker)
     ...
     */
    setCenter(x, z) {
        // Snap to pixel grid to prevent UV interpolation wobble
        // worldRange / textureSize = world units per pixel
        const pixelSize = this.worldRange / this.textureSize;
        const snappedX = Math.round(x / pixelSize) * pixelSize;
        const snappedZ = Math.round(z / pixelSize) * pixelSize;

        // Store wrapped center for canvas painting (matches shader coordinate space)
    ...
    /**
     * Rebuild entire overlay from only the current 3x3 chunk grid
    ...
    rebuildFrom3x3Chunks() {
        ...
        // Set up composite operation for additive blending
        ...
        // Only iterate the 9 chunks in current 3x3 grid (not all 441+ chunks)
        ...
        if (this.game?.gameState?.roads) {
            for (const chunkKey of this.current3x3Keys) {
                ...
                for (const road of roads) {
                    ...
                    this.drawRoadPatch(wrappedX, wrappedZ, road.radius || 0.75, road.materialType || 'limestone');
                    roadCount++;
                }
            }
        }
     ...
     * @param {number} worldX - Road center world X
     * @param {number} worldZ - Road center world Z
     * @param {number} radius - Road radius in world units (default 0.75)
     * @param {string} materialType - 'limestone' or 'sandstone'
     */
    paintRoadImmediate(worldX, worldZ, radius = 0.75, materialType = 'limestone') {
        // Wrap coordinates to match shader space
        const wrappedX = wrapCoord(worldX);
        const wrappedZ = wrapCoord(worldZ);

        // Save composite operation and set to lighter for additive blending
        const prevComposite = this.ctx.globalCompositeOperation;
        this.ctx.globalCompositeOperation = 'lighter';

        this.drawRoadPatch(wrappedX, wrappedZ, radius, materialType);
    ...
    drawRectangularDirt(worldX, worldZ, width, depth, rotation) {
        ...
        // Draw solid core - use RED channel only (shader reads .r for dirt, .g for roads)
        this.ctx.fillStyle = 'rgb(255, 0, 0)';  // 1.0 = full dirt
        this.ctx.fillRect(-halfW, -halfD, halfW * 2, halfD * 2);

        // Draw soft edge layers with decreasing red values
        const layers = 4;
