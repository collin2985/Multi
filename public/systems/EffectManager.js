...
export class EffectManager {
    ...
    updateSmokeForInventory(structureId, modelType, inventory) {
        ...
        if (modelType === 'tileworks') {
            const smoke1 = this.smokeEffects.get(structureId + '_1');
            const smoke2 = this.smokeEffects.get(structureId + '_2');
            // Early exit if no effects exist (not in range)
        ...
        // Single smoke effect
        const smoke = this.smokeEffects.get(structureId);
        // Early exit if no effect exists (not in range or not created)
        if (!smoke) return;
    ...
    spawnArtilleryMuzzleFlash(position) {
        // Create a large bright flash sprite
