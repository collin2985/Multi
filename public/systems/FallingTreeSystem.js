...
export class FallingTreeSystem {
    ...
    spawnFallingTree(treeObject, playerPosition) {
        ...
        console.log(`[FallingTree] spawnFallingTree called, treeType: ${treeType}`);
        if (treeType !== 'pine' && treeType !== 'apple') {
            console.log(`[FallingTree] Skipping - not pine or apple`);
            return false;
        }
        ...
        if (!material) {
            console.log(`[FallingTree] No material for ${treeType}`);
            return false;
        }

        const objectId = treeObject.userData?.objectId;
        if (!objectId) {
            console.log(`[FallingTree] No objectId`);
            return false;
        }

        const position = treeObject.position.clone();
        console.log(`[FallingTree] Creating falling tree at position:`, position);
