...
export class ActionManager {
    ...
    completeActiveAction() {
        ...
        if (actionType === 'build_road') {
            ...
            if (hammer) {
                ...
                if (hammer.durability === 0) {
                    ...
                    ui.showToast('Hammer broke!', 'warning');
                }
            }
        ...
        // Handle boat building completion
        if (actionType === 'build_boat') {
            const boatData = this.gameState.activeAction.boatData;

            // Send server message to place boat structure
            this.networkManager.sendMessage('place_boat', {
            ...
            this.gameState.activeAction = null;
            ui.updateChoppingProgress(0);
            ui.showToast('Boat complete!', 'success');
        ...
        if (actionType === 'repair') {
            ...
            this.gameState.activeAction = null;
            ui.updateChoppingProgress(0);
            ui.showToast('Structure repaired!', 'success');
        ...
        if (actionType === 'harvest_deer') {
            ...
            console.log('[ActionManager] harvest_deer completion:', {
                hasActiveAction: !!this.gameState.activeAction,
                deerCorpse: deerCorpse,
                hasCompleteHarvestDeerAction: !!this.game.completeHarvestDeerAction
            });
            ...
            if (this.game.completeHarvestDeerAction && deerCorpse) {
                ...
                console.error('[ActionManager] Failed to complete deer harvest:', {
                    hasFunction: !!this.game.completeHarvestDeerAction,
                    hasDeerCorpse: !!deerCorpse
                });
            }
        ...
        // Handle bear harvesting completion
        if (actionType === 'harvest_bear') {
            const bearCorpse = this.gameState.activeAction?.bearCorpse;

            // DEBUG: Log what we have
            console.log('[ActionManager] harvest_bear completion:', {
                hasActiveAction: !!this.gameState.activeAction,
                bearCorpse: bearCorpse,
                hasCompleteHarvestBearAction: !!this.game.completeHarvestBearAction
            });
            ...
            // Then delegate to game.completeHarvestBearAction
            if (this.game.completeHarvestBearAction && bearCorpse) {
                this.game.completeHarvestBearAction(bearCorpse);
            } else {
                console.error('[ActionManager] Failed to complete bear harvest:', {
                    hasFunction: !!this.game.completeHarvestBearAction,
                    hasBearCorpse: !!bearCorpse
                });
            }
