...
export class ScheduledShipSystem {
    ...
    update(deltaTime) {
        for (const [dockId, shipData] of this.activeShips) {
            ...
            // Update merchant ship collider position
            if (this.game?.physicsManager && shipData.ship?.userData?.physicsBodyId) {
                ...
                // Check for boat collisions during movement phases
    ...
    checkBoatCollisions(merchantShipId) {
        ...
        for (const boatInfo of overlappingBoats) {
            ...
            // Find the boat mesh - first try objectRegistry, then search chunkObjects
            let boatMesh = this.game.objectRegistry?.get(boatInfo.objectId);

            // Fallback: search through chunkObjects if not in objectRegistry
            if (!boatMesh && this.game.chunkManager?.chunkObjects) {
                for (const [, objects] of this.game.chunkManager.chunkObjects) {
                    for (const obj of objects) {
                        if (obj.userData?.objectId === boatInfo.objectId) {
                            boatMesh = obj;
                            break;
                        }
                    }
            ...
            if (!boatMesh) {
                console.warn(`[Ship] Could not find boat mesh for ${boatInfo.objectId}`);
                continue;
            }
    ...
    isMerchantShipsEnabledForDock(dockId) {
        ...
        this.scene.traverse(obj => {
            if (obj.userData?.modelType === 'market') {
                const dist = dockPos.distanceTo(obj.position);
                if (dist < nearestDist) {
                    nearestDist = dist;
                    nearestMarket = obj;
                }
        ...
        return nearestMarket?.userData?.merchantShipsEnabled === true;
    }
    ...
    checkAndTriggerShipSpawns(sendMessage) {
        ...
        for (const [dockId, dock] of this.docks) {
            ...
            if (!this.isMerchantShipsEnabledForDock(dockId)) continue;
