...
export class CraftingSystem {
    ...
    completeChiselingAction(activeAction) {
        ...
        const durabilityLoss = Math.ceil(stoneQuality / chiselQuality);
    ...
    canBeCombined(item1, item2) {
        ...
        // vines + vines = rope
        if (type1 === 'vines' && type2 === 'vines') return true;
    ...
    completeCombineAction(activeAction) {
        ...
        let avgQuality = Math.round((item1.quality + item2.quality) / 2);
        ...
        if (TOOL_TYPES.includes(resultType)) {
            avgQuality = Math.max(avgQuality, 25);
        }
        ...
        if (this.game && this.game.tryAddItemToInventory(resultItem)) {
            const displayName = resultType.charAt(0).toUpperCase() + resultType.slice(1);
            ui.updateActionStatus(`Created ${displayName} (Q${avgQuality})`, 3000);
