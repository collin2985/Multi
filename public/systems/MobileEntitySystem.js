...
export class MobileEntitySystem {
     ...
     */
    updateHorseMovement(keys, deltaTime, entity, isDead = false, physicsManager = null, entityId = null, cartAttachmentState = null, crateLoadState = null) {
        ...
        // Check if towing a cart
        const isTowing = cartAttachmentState?.isAttached || false;
        ...
        // Apply road speed bonus (1.6x on roads)
        ...
        // Apply cart towing speed penalties
        if (isTowing) {
            if (hasCrate) {
        ...
        if (isDead) {
            ...
            const effectiveTurnRate = config.baseTurnRate * CART.MOUNTED_TURN_RATE_MULTIPLIER;
        ...
        if (physicsManager && entityId && (moveX !== 0 || moveZ !== 0)) {
            ...
            // Always use corrected movement from physics - this allows sliding along obstacles
            ...
            if (result.hasCollision) {
                ...
                if (movementRatio < 0.1) {
                    ...
                    console.warn('[Horse STUCK] Movement blocked!', {
        ...
        if (terrainY < 0 && !hitObstacle) {
            // Hit water - stop (same as boat hitting shore)
            console.warn('[Horse STUCK] Water boundary triggered!', {
                position: [entity.position.x, entity.position.y, entity.position.z],
                nextPosition: { x: nextX, z: nextZ },
                terrainY: terrainY
            });
            this.velocity = 0;
            hitWater = true;
