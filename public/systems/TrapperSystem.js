...
const RESOURCE_TYPES = [
    'pine', 'fir', 'oak', 'cypress', 'apple',           // Trees
    'limestone', 'sandstone', 'clay', 'iron',           // Rocks
    'vines', 'mushroom', 'vegetables', 'vegetableseeds', // Gatherable
    'fish', 'deer', 'bear'                               // Wildlife
];
...
export class TrapperSystem {
    ...
    spawnTrapper(chunkX, chunkZ) {
        ...
        if (!position) {
            console.warn(`[TrapperSystem] Could not find valid position for chunk ${chunkKey}`);
            return;
        }
        ...
        // Freeze NPC at frame 1 of idle animation (static pose, no per-frame updates needed)
        if (manGLTF.animations && manGLTF.animations.length > 0) {
            const tempMixer = new THREE.AnimationMixer(npcMesh);
            const idleAnim = manGLTF.animations.find(a =>
                a.name.toLowerCase().includes('idle')
            );

            if (idleAnim) {
                const action = tempMixer.clipAction(idleAnim);
                action.reset();
                action.time = 1 / 24;  // Frame 1 at 24fps
                action.weight = 1.0;
                action.play();
                action.paused = true;
                tempMixer.update(0.001);  // Apply pose to skeleton
            }
            // Don't store mixer - let it be garbage collected
        }
