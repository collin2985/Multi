...
export class DeathManager {
    ...
    killEntity(entity, isAI = false, isPeer = false, deathReason = 'Unknown cause') {
        ...
        if (!isAI && !isPeer && this.gameState.mobileEntityState.isActive) {
            ...
            // BOAT CRATE CLEANUP: When boat sinks, crates are lost with it
            // Release crates to server to clean up tracking, but don't drop them
            const waterVehicles = ['boat', 'sailboat', 'ship2'];
            if (waterVehicles.includes(entityType)) {
                const crateState = this.gameState.crateLoadState;
                if (crateState?.loadedCrates?.length > 0) {
                    console.log(`[Death] Boat sinking with ${crateState.loadedCrates.length} crates - releasing to server`);
                    ...
                    console.log('[Death] All boat crates released (lost with ship)');
                }
            }
    ...
    respawnPlayer() {
        ...
        if (crateState?.loadedCrates?.length > 0) {
            console.warn(`[Respawn] ${crateState.loadedCrates.length} crates still loaded - forcing cleanup`);
            for (const slot of crateState.loadedCrates) {
        ...
        // Clear legacy single-crate state
        if (crateState && crateState.isLoaded) {
            console.warn('[Respawn] Legacy crate state still loaded - forcing cleanup');
    ...
    showSpawnScreenWithError(errorMessage) {
        console.log('[DeathManager] Showing spawn screen with error:', errorMessage);
    ...
    _repopulateObjectRegistry(spawnChunkX, spawnChunkZ) {
        ...
        if (count > 0) {
            console.log(`[Respawn] Re-added ${count} objects to objectRegistry`);
        }
    }
}
