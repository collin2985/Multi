...
export class AmbientSoundSystem {
    ...
    onCombatActivity() {
        ...
        console.log('[AmbientSound] Combat activity detected, silencing plains/forest');
    }
    ...
    updatePlainsSound(deltaSeconds) {
        ...
        if (this.lastCombatTime > 0) {
            ...
            if (timeSinceCombat < this.combatSilenceDuration) {
                ...
                // Debug: log volume reduction (only first few seconds)
                if (timeSinceCombat < 3000 && Math.random() < 0.1) {
                    console.log(`[AmbientSound] Combat silence: ${(fadeProgress * 100).toFixed(1)}% volume (${timeSinceCombat.toFixed(0)}ms since shot)`);
                }
            }
        }
