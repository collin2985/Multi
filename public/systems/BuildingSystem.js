...
export class BuildingSystem {
    ...
    addMaterialToConstruction(constructionSite, materialType, amount = 1) {
        ...
        if (required[materialType]) {
            // Exact match
            isNeeded = true;
            requiredPlankType = materialType;
        ...
        if (isPlankType(materialType) && isPlankType(requiredPlankType)) {
            // For planks, check total of all plank types
            currentAmount = getTotalPlankQuantity(current);
            neededAmount = required[requiredPlankType] - currentAmount;
        } else {
            // For non-planks, check exact type
