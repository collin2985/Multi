...
const LOD_STRUCTURE_TYPES = new Set(['tent', 'outpost', 'campfire', 'horse']);
...
export class StructureModelSystem {
    constructor(scene) {
        ...
        // LOD thresholds - read from CONFIG (quality-dependent)
        // Billboard fades in at STRUCTURE_LOD_START, model hidden at STRUCTURE_LOD_END
        this.lodStartDistance = CONFIG.LOD?.STRUCTURE_LOD_START || 40;
        this.lodEndDistance = CONFIG.LOD?.STRUCTURE_LOD_END || 60;
        this.lodStartSq = this.lodStartDistance * this.lodStartDistance;
        this.lodEndSq = this.lodEndDistance * this.lodEndDistance;

        // Skip thresholds for hysteresis (avoid flicker)
        this.skipShowSq = this.lodStartSq * 0.7;   // Well within show range
        this.skipHideSq = this.lodEndSq * 1.3;     // Well within hide range
