...
import { PHASE } from './ScheduledShipSystem.js';
import { ChunkCoordinates } from '../core/ChunkCoordinates.js';

// Merchant constants
const MERCHANT_CONSTANTS = {
    DOCK_HEIGHT: 0.5,           // Y position on dock (feet on ground)
    ...
    BLUE_SHIRT_COLOR: 0x2266aa, // Blue color for merchant shirt
...
let sharedBlueShirtMaterial = null;
...
export class DockMerchantSystem {
    constructor(scene, scheduledShipSystem) {
        ...
        if (!sharedBlueShirtMaterial) {
            sharedBlueShirtMaterial = new THREE.MeshStandardMaterial({
                color: MERCHANT_CONSTANTS.BLUE_SHIRT_COLOR,
                roughness: 0.8,
                metalness: 0.1
            });
        }
    ...
    spawnMerchant(dockId, dockPosition, dockRotation, lastShipSpawn) {
        ...
        // Calculate position - merchant always spawns on dock
        const merchantX = dockPosition[0] + MERCHANT_CONSTANTS.DOCK_OFFSET_X;
        const merchantZ = dockPosition[2] + MERCHANT_CONSTANTS.DOCK_OFFSET_Z;
        ...
        mesh.traverse((child) => {
            if (child.isMesh || child.isSkinnedMesh) {
                ...
                // Cube001_3 is the shirt (red color 9d1000) - use shared blue material
                if (child.name === 'Cube001_3' && child.material) {
                    child.material = sharedBlueShirtMaterial;
                }
            }
        });
        ...
        // Face toward the dock (rotate to face -Z direction toward dock)
    ...
    removeMerchant(dockId) {
        ...
        merchantData.entity.traverse((child) => {
            ...
            if (child.material && child.material !== sharedBlueShirtMaterial) {
                if (Array.isArray(child.material)) {
                    child.material.forEach(m => {
                        if (m !== sharedBlueShirtMaterial) m.dispose();
                    });
                } else {
