...
class ObjectPlacer {
    ...
    createInstance(modelType, position, scale, rotationY, scene = null) {
        ...
        // Debug log for horse
        if (modelType === 'horse') {
            console.log('[Horse Debug] createInstance in objects.js:', { modelType, model: model ? 'loaded' : 'NOT LOADED', position, scale });
        }
        ...
        if (modelType === 'horse') {
            console.log('[Horse Debug] Clone method:', hasSkeleton ? 'SkeletonUtils.clone()' : 'model.clone()');
        }
        ...
        // Debug: log what the model contains
        if (modelType === 'horse') {
            let meshCount = 0;
            instance.traverse((child) => {
                if (child instanceof THREE.Mesh) {
                    ...
                    console.log('[Horse Debug] Mesh:', child.name, {
            ...
            console.log('[Horse Debug] Total meshes:', meshCount, 'Instance position:', instance.position);
        }
        instance.traverse((child) => {
            if (child instanceof THREE.Mesh) {
                if (child.material) {
                ...
                // EXCEPT for logs and horses - disable frustum culling to prevent bounding box issues
                if (modelType === 'log' || modelType.endsWith('_log') || modelType === 'horse' || modelType === 'cart') {
    ...
    generateChunkObjectsBatch(
        ...
        for (let i = startIndex; i < endIndex; i++) {
            ...
            if (terrainHeight >= config.heightRange.min &&
                ...
                if (['fir', 'pine', 'apple', 'clay', 'limestone', 'sandstone', 'iron'].includes(modelType)) {
                ...
                const isRock = ROCK_TYPES.includes(modelType);
        ...
        objects.forEach(obj => {
            ...
            if (obj.userData.modelType === 'outpost' && obj.userData.objectId) {
                ...
                if (game && game.occupiedOutposts && game.occupiedOutposts.has(obj.userData.objectId)) {
                    ...
                    // Kill local player if they're the occupant
                    if (occupantId === game.gameState.clientId && !game.isDead) {
                        game.playerCombat.die();
                        console.log('Player died from outpost destruction');
                    }
            ...
            if (game && game.terrainGenerator && obj.userData.modelType) {
                const structuresToLevel = ['crate', 'house', 'garden', 'outpost', 'tent', 'market', 'tileworks'];
