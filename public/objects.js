...
class ObjectPlacer {
    ...
    createInstance(modelType, position, scale, rotationY, scene = null) {
        ...
        const isDock = modelType === 'dock';
        instance.traverse((child) => {
            if (child instanceof THREE.Mesh) {
                if (child.material) {
                    this.fixTransparency(child.material);

                    // Apply polygon offset to dock to prevent z-fighting with terrain
                    if (isDock) {
                        const materials = Array.isArray(child.material) ? child.material : [child.material];
                        materials.forEach((mat) => {
                            mat.polygonOffset = true;
                            mat.polygonOffsetFactor = -1;  // Pull dock forward
    ...
    removeObjects(scene, objects, physicsManager = null) {
        objects.forEach(obj => {
            ...
            if (game && game.terrainGenerator && obj.userData.modelType) {
                const structuresToLevel = ['crate', 'house', 'garden', 'outpost', 'tent', 'market', 'tileworks', 'bakery', 'gardener', 'miner', 'woodcutter', 'stonemason'];
