...
export class PlayerController {
    ...
    _updateVehicleStyleMovement(keys, deltaTime) {
        ...
        if (!cart) {
            // DEBUG: Log when cart is null but we're in vehicle movement (only once per occurrence)
            if (!this._loggedCartNull) {
                console.error('[Cart Debug] _updateVehicleStyleMovement called but cart is NULL!', {
                    isAttached: this.cartAttachmentState?.isAttached,
                    attachedCart: this.cartAttachmentState?.attachedCart,
                    cartId: this.cartAttachmentState?.cartId
                });
                this._loggedCartNull = true;
            }
            this.isReversingWithCart = false;
            this.towingVelocity = 0;
            return { isMoving: false, speedRatio: 0, isTurning: false };
        }
        this._loggedCartNull = false; // Reset flag when cart is valid
    ...
    updateWASDMovement(keys, cameraRotation, deltaTime) {
        ...
        if (isTowing) {
            // DEBUG: Log first WASD press while towing (only once per attachment)
            if (!this._loggedTowingWASD && (keys.w || keys.a || keys.s || keys.d)) {
                console.log('[Cart Debug] WASD pressed while towing:', {
                    keys: { w: keys.w, a: keys.a, s: keys.s, d: keys.d },
                    cartAttachmentState: {
                        isAttached: this.cartAttachmentState?.isAttached,
                        hasCart: !!this.cartAttachmentState?.attachedCart,
                        cartId: this.cartAttachmentState?.cartId
                    },
                    towingHeading: this.towingHeading,
                    towingVelocity: this.towingVelocity
                });
                this._loggedTowingWASD = true;
            }
        ...
        this._loggedTowingWASD = false; // Reset when not towing
