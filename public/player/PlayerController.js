...
export class PlayerController {
    constructor(playerObject, terrainGenerator, physicsManager = null, navigationManager = null, game = null) {
        ...
        this._baseSpeed = 0.0005; // Fallback: 0.5 units/sec in units/ms (matches AvatarManager)
        this.stopThreshold = 0.01; // Precise arrival threshold

        // Dock state (Rapier collision is source of truth)
        this.onDock = false;
        this.DOCK_HEIGHT = CONFIG.CONSTRUCTION.STRUCTURE_PROPERTIES.dock.deckHeight; // Fixed dock walking height

        // Safe position tracking for water reversal (reused Vector3, no allocations)
        // lastSafePosition: last safe position on TERRAIN (height >= 0)
        this.lastSafePosition = new THREE.Vector3(
            playerObject.position.x,
            playerObject.position.y,
            playerObject.position.z
        );
        // lastSafeDockPosition: last safe position on DOCK (inside dock sensor)
        this.lastSafeDockPosition = new THREE.Vector3(
            playerObject.position.x,
            playerObject.position.y,
            playerObject.position.z
        );
        ...
        this.onDockStateChangedCallback = null;
        ...
        // Cart towing state (references set by Game class)
        this.cartAttachmentState = null;  // Reference to gameState.cartAttachmentState
        this.crateLoadState = null;       // Reference to gameState.crateLoadState
        this.artilleryManningState = null;  // Reference to gameState.artilleryManningState (blocks WASD)
     ...
     * @param {function} callback - Called with (position, onDock)
     */
    ...
    /**
     * Set callback for when player dock state changes (enters or leaves dock)
     * @param {function} callback - Called with (position, targetPosition, onDock)
     */
    setOnDockStateChangedCallback(callback) {
        this.onDockStateChangedCallback = callback;
    }
    ...
    _updateSpeedMultiplier() {
        // On dock: skip slope calculation (docks are flat, no terrain penalty)
        let newSpeed = 1.0;
        if (!this.onDock) {
            // Get slope-based speed (directional, more precise for current movement)
            newSpeed = this.calculateDirectionalSlopeSpeed(
                this.playerObject.position,
                this.targetPosition
            );
        }

        // Apply road bonus from navigation system (2.0x on roads - TEST: was 1.6x)
        ...
        if (this.cartAttachmentState?.isAttached) {
            const CART = CONFIG.CART_PHYSICS || {
                LOADED_CART_SPEED_MULTIPLIER: 0.5,
                EMPTY_CART_SPEED_MULTIPLIER: 0.9
            };
            if (this.crateLoadState?.isLoaded) {
                // Cart with crate: 50% speed
                newSpeed *= CART.LOADED_CART_SPEED_MULTIPLIER;
            } else {
                // Empty cart: 90% speed
                newSpeed *= CART.EMPTY_CART_SPEED_MULTIPLIER;
            }
        }
    ...
    setTargetPosition(position) {
        ...
        if (this.artilleryManningState?.isManning) {
            return;
        }
    ...
    updateMovement(deltaTime, isDead = false) {
        ...
        if (!this.isMoving) {
            ...
            if (this.playerObject.userData.terrainChanged) {
                ...
                // One-time height adjustment after terrain change (skip if on dock)
                if (this.terrainGenerator && !this.onDock) {
                    const terrainHeight = this.terrainGenerator.getWorldHeight(position.x, position.z);
                    position.y = terrainHeight + 0.03;
                    this.targetY = position.y;
                    console.log('[PlayerController] Updated stationary player height after terrain change');
                }
        ...
        if (distanceSquared <= stopThresholdSquared) {
            ...
            if (this.physicsManager && this.physicsManager.initialized) {
                ...
                // Apply movement (dock is a sensor, won't block)
            ...
            // === DOCK DETECTION: Use overlap test (dock is a sensor) ===
            const isOnDockNow = this.physicsManager?.isInsideDock(position) !== null;
            const terrainHeight = this.terrainGenerator?.getWorldHeight(position.x, position.z) ?? 0;

            // DEBUG: Log dock state changes
            if (isOnDockNow !== this.onDock) {
                console.log('[DOCK DEBUG] Dock state changed:', isOnDockNow ? 'ENTERED dock' : 'LEFT dock',
                    'terrainHeight:', terrainHeight.toFixed(2));
            }

            if (isOnDockNow) {
                // Check if just entered dock (was not on dock before)
                const justEnteredDock = !this.onDock;

                // On dock - use dock height
                this.onDock = true;
                this.targetY = this.DOCK_HEIGHT;
                // Save safe DOCK position (only while on dock)
                this.lastSafeDockPosition.set(position.x, position.y, position.z);

                // Broadcast dock entry to peers
                if (justEnteredDock && this.onDockStateChangedCallback) {
                    this.onDockStateChangedCallback(position.clone(), this.targetPosition.clone(), true);
                }
            } else if (this.onDock) {
                // Was on dock, now left it
                if (terrainHeight >= CONFIG.WATER.MIN_WALKABLE_HEIGHT) {
                    // Stepped onto safe land - clear dock state
                    console.log('[DOCK DEBUG] Left dock to safe land');
                    this.onDock = false;
                    this.targetY = terrainHeight + 0.03;
                    // Save safe TERRAIN position (now on land)
                    this.lastSafePosition.set(position.x, position.y, position.z);

                    // Broadcast dock exit to peers
                    if (this.onDockStateChangedCallback) {
                        this.onDockStateChangedCallback(position.clone(), this.targetPosition.clone(), false);
                    }
                } else {
                    // Stepped toward water - REJECT move, snap back to dock
                    console.log('[DOCK DEBUG] Left dock toward water - REJECTING move, snapping to lastSafeDockPosition');
                    // Reset ACTUAL position (not just target)
                    position.copy(this.lastSafeDockPosition);
                    this.targetPosition.copy(this.lastSafeDockPosition);
                    this.targetY = this.DOCK_HEIGHT;
                    // Stay on dock
                    this.onDock = true;
                    ...
                    // Broadcast water reversal stop
                    if (this.onWaterReversalCallback) {
                        this.onWaterReversalCallback(position.clone(), true); // true = on dock
                    }
                    return false;
                }
            } else {
                // Not on dock - normal terrain logic
                if (terrainHeight < CONFIG.WATER.MIN_WALKABLE_HEIGHT) {
                    // Water! Return to last safe TERRAIN position
                    console.log('[DOCK DEBUG] Hit water (not on dock) - REVERSING. terrainHeight:', terrainHeight.toFixed(2));
                    ...
                    if (this.onWaterReversalCallback) {
                        this.onWaterReversalCallback(this.lastSafePosition.clone(), false); // false = not on dock
                    }
                    return true;
                }
                ...
                if (this.frameCounter % this.RAYCAST_UPDATE_INTERVAL === 0) {
                    this.targetY = terrainHeight + 0.03;
                    // Save safe TERRAIN position (only while on terrain)
                    this.lastSafePosition.set(position.x, position.y, position.z);
                }
            }
    ...
    _updateVehicleStyleMovement(keys, deltaTime) {
        const CART = CONFIG.CART_PHYSICS || {
            LOADED_CART_SPEED_MULTIPLIER: 0.5,
            EMPTY_CART_SPEED_MULTIPLIER: 0.9,
            ROAD_SPEED_MULTIPLIER: 2.0,  // TEST: was 1.6
        ...
        const cart = this.cartAttachmentState?.attachedCart;
        const hasCrate = this.crateLoadState?.isLoaded || false;
        let isTurning = false;
        ...
        if (this.towingVelocity > 0) {
            ...
            if (terrainY < waterLevel) {
                ...
                // Update physics body
                if (this.physicsManager) {
                    this.physicsManager.updateKinematicPosition('player', this.playerObject.position);
                }
            }
    ...
    updateWASDMovement(keys, cameraRotation, deltaTime) {
        ...
        const isTowing = this.cartAttachmentState?.isAttached || false;
        ...
        if (keys.w) dirZ -= 1;  // Forward (into screen)
