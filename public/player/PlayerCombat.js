...
export class PlayerCombat {
     ...
     */
    updateShooting(aiEnemy, aiEnemyController, peerGameData, onShoot, onStopMoving, tentAIEnemies = null) {
        ...
        if (now - this.lastTargetCheckTime >= 1000) {
            ...
            if (tentAIEnemies) {
                tentAIEnemies.forEach((aiData, tentId) => {
                    if (aiData.controller && !aiData.isDead && aiData.controller.enemy) {
                        const enemy = aiData.controller.enemy;
            ...
            peerGameData.forEach((peer, peerId) => {
                if (peer.aiEnemy && !peer.aiEnemy.userData.isDead) {
                    ...
                    const peerDistSquared = dx * dx + dz * dz;  // No Math.pow() or Math.sqrt() needed!
                    if (peerDistSquared < nearestDistanceSquared) {
                        nearestDistanceSquared = peerDistSquared;
                        // Only calculate actual distance if needed (currently not used anywhere)
        ...
        if (distance <= shootingRange && now - this.lastShootTime >= this.shootInterval) {
            ...
            console.log(`Player shooting at ${this.shootTarget.isLocal ? 'local AI' : 'peer AI'}! Hit chance: ${(hitChance * 100).toFixed(1)}%, Result: ${isHit ? 'HIT' : 'MISS'}`);
