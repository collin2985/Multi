...
export class NavigationManager {
     ...
     */
    rebuildRegionForLeveledTerrain(worldX, worldZ, width, depth, rotation, heightProvider) {
        const halfWidth = width / 2;
        const halfDepth = depth / 2;
        const transitionSize = 1.0;

        // Calculate bounding box of the rotated rectangle
        const cosRAbs = Math.abs(Math.cos(rotation));
        const sinRAbs = Math.abs(Math.sin(rotation));
