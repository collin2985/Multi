...
export const MOVEMENT_SPEED = Object.freeze({
    // Surface type multipliers (relative to grass baseline)
    SURFACE: {
        GRASS: 1.0,   // Baseline speed
        DIRT: 0.95,   // Slightly slower
        SAND: 0.9,    // Harder to walk on
        ROCK: 0.85    // Rough, uneven terrain
    },
...
export class ChunkNavigationMap {
     ...
     * @param {number} worldX - World X position
     * @param {number} worldZ - World Z position
     * @param {object} heightCalculator - Terrain height calculator (terrainRenderer.heightCalculator)
     * @returns {number} Slope in degrees (0-90)
     */
    calculateSlope(worldX, worldZ, heightCalculator) {
        // Sample distance - 1.0m for smoother, more representative slope readings
        const sampleDist = 1.0;

        // Get height at center position
        const centerHeight = heightCalculator.calculateHeight(worldX, worldZ);

        // Sample heights in 4 cardinal directions
        const northHeight = heightCalculator.calculateHeight(worldX, worldZ - sampleDist);
        const southHeight = heightCalculator.calculateHeight(worldX, worldZ + sampleDist);
        const eastHeight = heightCalculator.calculateHeight(worldX + sampleDist, worldZ);
        const westHeight = heightCalculator.calculateHeight(worldX - sampleDist, worldZ);
     ...
     *
     * Priority order:
     * 1. Slope > 30° → ROCK (steep slope override)
     * 2. Height > 4.5 → ROCK (rocky/snowy elevations)
     * 3. Height < 1.3 && worldX > 0 → SAND (beach biome)
     * 4. Height < 1.3 && worldX <= 0 → DIRT (dirt biome)
     * 5. Height >= 1.3 → GRASS (default terrain)
     *
     * Note: Water (height < 1.02) is handled separately via the WATER flag
     *
     * @param {number} worldX - World X position
     * @param {number} worldZ - World Z position
    ...
    determineSurfaceType(worldX, worldZ, height, slopeDegrees) {
        ...
        // High elevations are rocky (includes snow areas)
        if (height > 4.5) {
            return SURFACE_TYPE.ROCK;
        }

        // Low elevations split between sand (x > 0) and dirt (x <= 0)
        if (height < 1.3) {
            if (worldX > 0) {
     ...
     */
    determineSurfaceTypeAtCell(cellX, cellZ, heightCalculator) {
        const { worldX, worldZ } = this.cellToWorld(cellX, cellZ);
    ...
    getMovementSpeedMultiplier(worldX, worldZ) {
        ...
        // Check if walkable
        if ((flags & NAV_FLAGS.WALKABLE) === 0) {
            // Allow slower movement on unwalkable terrain (so player can escape if stuck)
            // Physics collisions will prevent actually entering these areas
            if ((flags & NAV_FLAGS.OBSTACLE) !== 0) {
                return 0.5; // On obstacle - allow reduced speed to escape
            }
            if ((flags & NAV_FLAGS.WATER) !== 0) {
                return 0.5; // In water - allow escape (physics at 1.02 handles blocking)
            }
            return 0.1; // Other unwalkable (steep slope) - allow slow escape
        }
     ...
     *
     * @param {object} heightCalculator - Terrain height calculator (terrainRenderer.heightCalculator)
     * @param {number} waterLevel - Water surface level (default: 1.02)
     * @returns {object} Build statistics {processedCells, walkableCells, waterCells, blockedCells, timeMs}
     */
    buildTerrainGrid(heightCalculator, waterLevel = 1.02) {
