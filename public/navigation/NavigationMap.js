...
export class ChunkNavigationMap {
    ...
    buildTerrainGrid(heightProvider, waterLevel = 0) {
        ...
        for (let cellZ = 0; cellZ < NAV_CONFIG.GRID_SIZE; cellZ++) {
            for (let cellX = 0; cellX < NAV_CONFIG.GRID_SIZE; cellX++) {
                ...
                // Every 10th cell in each dimension (200/10 = 20)
                if (cellX % 10 === 0 && cellZ % 10 === 0) {
                    const sampleX = cellX / 10;
                    const sampleZ = cellZ / 10;
     ...
     * This is much faster than buildTerrainGrid() - <1ms vs ~500ms per chunk
     *
    ...
    buildSimpleGrid(objects, gridDimensions) {
        ...
        if (objects && objects.length > 0 && gridDimensions) {
            const stats = this.addObstaclesFromObjectList(objects, gridDimensions);
            obstacleCount = stats.total;
        }
