 ...
 */
function isWalkable(navManager, x, z, ignoreSlopes = false) {
 ...
 */
function jump(navManager, x, z, dx, dz, goalX, goalZ, ignoreSlopes, maxDist = 200) {
    let dist = 0;
    ...
    while (dist < maxDist) {
        ...
        if (dx !== 0 && dz !== 0) {
            if (!isWalkable(navManager, x + dx * CELL_SIZE, z, ignoreSlopes) ||
                !isWalkable(navManager, x, z + dz * CELL_SIZE, ignoreSlopes)) {
                return null;
            }
        }

        // Hit obstacle or out of bounds
        if (!isWalkable(navManager, nx, nz, ignoreSlopes)) {
            return null;
        }
        ...
        if (dx !== 0 && dz !== 0) {
            ...
            if (!isWalkable(navManager, x - dx * CELL_SIZE, z, ignoreSlopes) &&
                isWalkable(navManager, x - dx * CELL_SIZE, z + dz * CELL_SIZE, ignoreSlopes)) {
                return { x, z };
            }
            // Blocked vertically but open diagonally
            if (!isWalkable(navManager, x, z - dz * CELL_SIZE, ignoreSlopes) &&
                isWalkable(navManager, x + dx * CELL_SIZE, z - dz * CELL_SIZE, ignoreSlopes)) {
                return { x, z };
            }

            // For diagonal, also check horizontal and vertical jumps
            if (jump(navManager, x, z, dx, 0, goalX, goalZ, ignoreSlopes, maxDist - dist) !== null ||
                jump(navManager, x, z, 0, dz, goalX, goalZ, ignoreSlopes, maxDist - dist) !== null) {
                return { x, z };
            }
        } else {
            ...
            if (dx !== 0) {
                // Moving horizontally
                if ((!isWalkable(navManager, x, z + CELL_SIZE, ignoreSlopes) &&
                     isWalkable(navManager, x + dx * CELL_SIZE, z + CELL_SIZE, ignoreSlopes)) ||
                    (!isWalkable(navManager, x, z - CELL_SIZE, ignoreSlopes) &&
                     isWalkable(navManager, x + dx * CELL_SIZE, z - CELL_SIZE, ignoreSlopes))) {
                    return { x, z };
                }
            } else {
                // Moving vertically
                if ((!isWalkable(navManager, x + CELL_SIZE, z, ignoreSlopes) &&
                     isWalkable(navManager, x + CELL_SIZE, z + dz * CELL_SIZE, ignoreSlopes)) ||
                    (!isWalkable(navManager, x - CELL_SIZE, z, ignoreSlopes) &&
                     isWalkable(navManager, x - CELL_SIZE, z + dz * CELL_SIZE, ignoreSlopes))) {
                    return { x, z };
                }
            }
 ...
 */
function getSuccessors(navManager, node, goalX, goalZ, ignoreSlopes) {
    const successors = [];
    const x = node.worldX;
    const z = node.worldZ;
    ...
    if (node.parent) {
        ...
        if (dx !== 0 && dz !== 0) {
            ...
            // Horizontal
            if (isWalkable(navManager, x + dx * CELL_SIZE, z, ignoreSlopes)) {
                const jp = jump(navManager, x, z, dx, 0, goalX, goalZ, ignoreSlopes);
                if (jp) successors.push(jp);
            }
            // Vertical
            if (isWalkable(navManager, x, z + dz * CELL_SIZE, ignoreSlopes)) {
                const jp = jump(navManager, x, z, 0, dz, goalX, goalZ, ignoreSlopes);
                if (jp) successors.push(jp);
            }
            // Diagonal
            if (isWalkable(navManager, x + dx * CELL_SIZE, z, ignoreSlopes) &&
                isWalkable(navManager, x, z + dz * CELL_SIZE, ignoreSlopes)) {
                const jp = jump(navManager, x, z, dx, dz, goalX, goalZ, ignoreSlopes);
                if (jp) successors.push(jp);
            }
            // Forced neighbors
            if (!isWalkable(navManager, x - dx * CELL_SIZE, z, ignoreSlopes) &&
            ...
            if (!isWalkable(navManager, x, z - dz * CELL_SIZE, ignoreSlopes) &&
                isWalkable(navManager, x + dx * CELL_SIZE, z, ignoreSlopes)) {
                const jp = jump(navManager, x, z, dx, -dz, goalX, goalZ, ignoreSlopes);
                if (jp) successors.push(jp);
            }
        } else if (dx !== 0) {
            ...
            if (!isWalkable(navManager, x, z + CELL_SIZE, ignoreSlopes) &&
                isWalkable(navManager, x + dx * CELL_SIZE, z + CELL_SIZE, ignoreSlopes)) {
                const jp = jump(navManager, x, z, dx, 1, goalX, goalZ, ignoreSlopes);
                if (jp) successors.push(jp);
            }
            if (!isWalkable(navManager, x, z - CELL_SIZE, ignoreSlopes) &&
                isWalkable(navManager, x + dx * CELL_SIZE, z - CELL_SIZE, ignoreSlopes)) {
            ...
            if (!isWalkable(navManager, x + CELL_SIZE, z, ignoreSlopes) &&
                isWalkable(navManager, x + CELL_SIZE, z + dz * CELL_SIZE, ignoreSlopes)) {
                const jp = jump(navManager, x, z, 1, dz, goalX, goalZ, ignoreSlopes);
                if (jp) successors.push(jp);
            }
            if (!isWalkable(navManager, x - CELL_SIZE, z, ignoreSlopes) &&
                isWalkable(navManager, x - CELL_SIZE, z + dz * CELL_SIZE, ignoreSlopes)) {
...
export function findPath(navigationManager, startX, startZ, goalX, goalZ, maxIterationsOrOptions = 5000) {
    ...
    if (!isWalkable(navigationManager, startWorldX, startWorldZ, ignoreSlopes)) {
        let found = false;
        for (let r = 1; r <= 5 && !found; r++) {
            for (let dx = -r; dx <= r && !found; dx++) {
                for (let dz = -r; dz <= r && !found; dz++) {
                    ...
                    if (isWalkable(navigationManager, nx, nz, ignoreSlopes)) {
                        startWorldX = nx;
                        startWorldZ = nz;
                        found = true;
                    }
    ...
    if (!isWalkable(navigationManager, goalWorldX, goalWorldZ, ignoreSlopes)) {
        let found = false;
        for (let r = 1; r <= 5 && !found; r++) {
            for (let dx = -r; dx <= r && !found; dx++) {
                for (let dz = -r; dz <= r && !found; dz++) {
    ...
    while (openList.length > 0 && iterations < maxIterations) {
        ...
        const successors = getSuccessors(navigationManager, current, goalAdjustedX, goalAdjustedZ, ignoreSlopes);
