...
export class PhysicsManager {
    ...
    createStaticCollider(objectId, shape, position, rotation = 0, collisionGroup = COLLISION_GROUPS.STRUCTURE) {
        ...
        try {
            ...
            // For docks, offset collider down so top is at deck level
    ...
    isInsideDock(position) {
        ...
        const point = new RAPIER.Vector3(position.x, position.y, position.z);
        let foundDock = null;
    ...
    raycast(origin, direction, maxDistance = 100.0, collisionMask = 0xFFFF) {
        ...
        // castRay params: ray, maxToi, solid, filterFlags, filterGroups
        const hit = this.world.castRay(ray, maxDistance, false, null, filterGroups);
    ...
    removeCollider(objectId) {
        ...
        this.pendingColliders = this.pendingColliders.filter(p => p.objectId !== objectId);
