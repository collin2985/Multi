...
export class CameraController {
    constructor(camera) {
        ...
        this.zoomMin = 0.9;   // Minimum zoom (10% closer than default)
    ...
    update(deltaTime = 0) {
        ...
        const lerpedOffset = new THREE.Vector3();
        lerpedOffset.lerpVectors(this.defaultOffset, this.overheadOffset, this.pitch);

        // Apply zoom to the interpolated offset
        const zoomedOffset = lerpedOffset.clone().multiplyScalar(this.zoom);

        // Rotate the offset around Y axis
        const cos = Math.cos(this.rotation);
        const sin = Math.sin(this.rotation);
        const rotatedOffset = new THREE.Vector3(
            zoomedOffset.x * cos - zoomedOffset.z * sin,
            zoomedOffset.y,
            zoomedOffset.x * sin + zoomedOffset.z * cos
        );

        // Set target camera position
        this.targetPosition.copy(this.target.position).add(rotatedOffset);
        ...
        lookAtPoint.x += currentLookAhead * sin;
        lookAtPoint.z -= currentLookAhead * cos;
