...
export class GameState {
    constructor() {
        ...
        this.nearestSwitchableMobileEntity = null;  // Nearby boat when already piloting (for switching)
        this.mobileEntityState = {
            ...
            // Ship2 multi-occupant extensions (only used when entityType === 'ship2'):
            shipOccupants: null,          // { pilot, portGunner, starboardGunner } - tracks all occupants
            myShipPosition: null,         // 'pilot' | 'portGunner' | 'starboardGunner' - my position on ship
            shipFaction: null             // Faction of first boarder (locks ship to same faction)
        };
        ...
        // Crate loading state (crate loaded onto cart or boat)
        // Multi-crate support: loadedCrates array holds all loaded crates with slot indices
        this.nearestLoadableCrate = null;  // { object: THREE.Object3D, distance: number }
        this.crateLoadState = {
            // Array of loaded crates (supports multi-crate for ship2)
            // Each entry: { slotIndex, crate, crateId, crateChunkKey, crateQuality, crateLastRepairTime, crateInventory }
            loadedCrates: [],

            // Legacy single-crate properties (for cart backward compatibility)
            // These are kept in sync with loadedCrates[0] when cart is used
            isLoaded: false,              // Whether a crate is loaded (cart uses this)
        ...
        // Artillery ship loading state (artillery loaded onto ship2)
        // Pattern matches crateLoadState for consistency
        this.nearestLoadableArtillery = null;  // { object: THREE.Object3D, distance: number }
        this.artilleryShipLoadState = {
            // Array of loaded artillery (supports 2 for ship2 port/starboard)
            // Each entry: { slotIndex, artillery, artilleryId, artilleryChunkKey, artilleryQuality, artilleryLastRepairTime, artilleryInventory }
            loadedArtillery: []
        };

        // Horse ship loading state (horses loaded onto ship2)
        // Pattern matches artilleryShipLoadState for consistency
        this.nearestLoadableHorse = null;  // { object: THREE.Object3D, distance: number }
        this.horseShipLoadState = {
            // Array of loaded horses (supports 4 for ship2)
            // Each entry: { slotIndex, horse, horseId, horseChunkKey, horseQuality, horseLastRepairTime }
            loadedHorses: []
        };
    ...
    getDefaultInventoryItems() {
        ...
        return [
            ...
            // TEMP: 3 shells for testing artillery
            {
