...
export class SmokeEffect {
    constructor(scene, position = { x: 0, y: 0, z: 0 }) {
        ...
        this.particlePool = [];
        this.spawnTimer = 0;
        this.spawnInterval = 0.25; // Spawn rate (covers ~10s lifespan with pool of 40)
        this.active = true;
        this.camera = null; // Set by EffectManager for billboarding

        this.init();
    }

    init() {
        // Load smoke texture from local file
        ...
        const smokeMaterial = new THREE.MeshLambertMaterial({
            ...
            side: THREE.DoubleSide // Render both sides for visibility from any angle
        });

        const smokeGeo = new THREE.PlaneGeometry(1.5, 1.5); // Scaled to game units

        // Create particle pool
        for (let p = 0; p < 40; p++) { // Pool sized to cover particle lifespan
            ...
            particle.userData = {
                ...
                fadeInDuration: 0.5,
                fadeOutDuration: 1.5,
                riseHeight: 11.2, // Height in game units (doubled)
                age: 0,
                totalLifespan: 0,
                arcStrength: 0,
                wobble: 0
            ...
            particle.renderOrder = 999; // Render after opaque objects for proper transparency
    ...
    spawnParticle() {
        ...
        for (let i = 0; i < this.particlePool.length; i++) {
            ...
            if (!particle.userData.active) {
                ...
                particle.userData.velocity = Math.random() * 0.35 + 0.56; // Rise speed (reduced 30%)
    ...
    update(delta) {
        // Cap delta to prevent issues when tab is inactive (max ~100ms)
    ...
    evolveSmoke(delta) {
        ...
        for (let i = this.smokeParticles.length - 1; i >= 0; i--) {
            ...
            // Billboard: make particle face camera (if camera is set)
            if (this.camera) {
                particle.lookAt(this.camera.position);
            }

            // Rotate the particle around its facing direction (Z-axis after lookAt)
