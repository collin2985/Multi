...
// Shared texture - created once, reused
...
function createDirtTexture() {
    ...
    // Soft circular gradient - brown/tan
    const gradient = ctx.createRadialGradient(size/2, size/2, 0, size/2, size/2, size/2);
    gradient.addColorStop(0, 'rgba(139, 119, 101, 1)');    // Brown center
    gradient.addColorStop(0.5, 'rgba(139, 119, 101, 0.6)');
    gradient.addColorStop(1, 'rgba(139, 119, 101, 0)');    // Transparent edge
...
export class DirtKickup {
     ...
     */
    constructor(scene, position, shooterPosition = null, camera = null) {
        this.scene = scene;
        this.camera = camera;
        this.particles = [];
        this.disposed = false;
...
export function spawnMissEffect(scene, targetPosition, shooterPosition = null, camera = null) {
    ...
    return new DirtKickup(scene, impactPos, shooterPosition, camera);
}
