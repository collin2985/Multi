...
import { generateChunkObjects, removeObjects } from './objects.js';
...
// This prevents cache thrashing by keeping frequently-accessed entries
class LRUCache {
    constructor(maxSize) {
    ...
    get(key) {
        ...
        // This is the key optimization: recently accessed entries move to the back
    ...
    set(key, value) {
        // If exists, remove it (will re-add at end as most recently used)
        ...
        // First entry in Map is least recently used (oldest)
...
export class HeightCalculator {
    constructor(seed = CONFIG.TERRAIN.seed) {
        ...
        this.heightCache = new LRUCache(this.MAX_CACHE_SIZE);
    }
...
export class TerrainMaterialFactory {
   static createTerrainMaterial(textures) {
    const vertexShader = `
        attribute vec3 color;  // Vertex color attribute for road blending
        ...
        varying float vRoadWeight;  // Pass road weight to fragment shader
        ...
        void main() {
            ...
            vRoadWeight = color.r;  // R channel stores road weight
    ...
    const fragmentShader = `
        ...
        uniform sampler2D uRoad;  // Road texture
        ...
        varying vec3 vWorldPosition;
        varying float vFogDepth;
        varying float vRoadWeight;  // Road blend weight from vertex colors
        ...
        void main() {
            ...
            float wSand2Base = 1.0 - smoothstep(0.5, 1.29, vHeight); // Full sand2 below 0.5, fade to 1.29
            ...
            // Apply road texture based on vertex color weight
            if (vRoadWeight > 0.0) {
                // Use 2x more detail for road texture with anti-tiling
                vec3 road = sampleTextureMulti(uRoad, vWorldPosition.xz, repeat * 2.0);
                baseColor = mix(baseColor, road, vRoadWeight);
            }
    ...
    const material = new THREE.ShaderMaterial({
        ...
        uniforms: {
            ...
            fogDensity: { value: GAME_CONFIG.RENDERING.FOG_DENSITY }
        },
    ...
    static createProceduralTextures() {
        ...
        // Advanced sand2 texture with multiple colors and multi-frequency noise
        const createSand2Texture = () => {
            const canvas = document.createElement('canvas');
            canvas.width = canvas.height = size;
            const ctx = canvas.getContext('2d');
            const imgData = ctx.createImageData(size, size);
            ...
            // Define sand color palette (5 shades for natural variation)
            const sandColors = [
                { r: 210, g: 165, b: 115 },  // Darker sand
                { r: 224, g: 180, b: 131 },  // Medium dark
                { r: 235, g: 188, b: 135 },  // Medium
                { r: 242, g: 192, b: 138 },  // Medium light
                { r: 247, g: 196, b: 138 }   // Lighter sand
            ];
            ...
            for (let y = 0; y < size; y++) {
                for (let x = 0; x < size; x++) {
                    ...
                    // Select color based on noise
                    const colorIndex = Math.floor(combinedNoise * sandColors.length);
                    const clampedIndex = Math.min(colorIndex, sandColors.length - 1);
                    const baseColor = sandColors[clampedIndex];

                    // Add slight per-pixel variation (less than dirt for smoother sand)
                    const variation = (rng() - 0.5) * 15;
        ...
        const roadTexture = new THREE.TextureLoader().load('./terrain/road.png', (texture) => {
            texture.wrapS = texture.wrapT = THREE.RepeatWrapping;
            texture.minFilter = THREE.LinearMipmapLinearFilter;
            texture.magFilter = THREE.LinearFilter;
            texture.generateMipmaps = true;
        ...
        return {
            ...
            sand2: createSand2Texture(), // Advanced multi-frequency sand texture
            road: roadTexture  // Road texture for vertex color blending
...
export class TerrainWorkerManager {
    ...
    generateWorkerCode() {
        ...
        return `
            ...
            const workerHeightCache = new LRUCache(MAX_CACHE_SIZE);
...
export class SimpleTerrainRenderer {
    constructor(scene, collisionManager = null, gameState = null) {
        this.scene = scene;
        this.collisionManager = collisionManager;
        this.gameState = gameState;
        this.chunkMap = new Map();
        this.chunkObjects = new Map(); // To track all world objects per chunk for cleanup
        this.chunkRoads = new Map(); // To track road positions per chunk for nav map building
        ...
        this.pendingRoads = []; // Queue for roads that need to be painted after chunks are created
        this.pendingTerrainLeveling = []; // Queue for terrain leveling operations when chunks aren't loaded yet
        this.chunksNeedingNavMaps = new Set(); // Track chunks that need navigation maps built (deferred until idle)
        // Don't call init() here - will be called explicitly after renderer is ready
    }

    init() {
        // Create textures AFTER WebGL renderer exists and is ready
        // This prevents "immutable texture" and "dimension overflow" errors
        if (!this.textures) {
    ...
    /**
     * Check if chunk loading is idle (all queues empty)
     * Used to determine when it's safe to build navigation maps
     * @returns {boolean} True if all chunk loading queues are empty
     */
    isChunkLoadingIdle() {
        return this.pendingVertexUpdates.length === 0 &&
               this.pendingObjectGeneration.length === 0 &&
               this.processingChunks.size === 0;
    }

    /**
     * Build navigation map for a chunk
     * Called when chunk loading is idle to avoid performance impact
     * @param {string} chunkKey - Chunk key (e.g., "0,0")
     * @returns {object|null} Build statistics or null if failed
     */
    buildNavigationMapForChunk(chunkKey) {
        if (!this.navigationManager) {
            console.warn('[TERRAIN] NavigationManager not set, cannot build nav map for chunk', chunkKey);
            return null;
        }

        const startTime = performance.now();

        // Parse chunk key to get grid coordinates
        const [gridX, gridZ] = chunkKey.split(',').map(Number);
        const chunkId = `chunk_${chunkKey}`;

        // Calculate world coordinates (chunk center)
        const chunkSize = CONFIG.TERRAIN.chunkSize;
        const worldX = gridX * chunkSize;
        const worldZ = gridZ * chunkSize;

        // Create navigation map for this chunk
        const physicsManager = this.navigationManager?.physicsManager || null;
        ...
        // Step 1: Build terrain grid (walkability, slopes, surface types)
        const terrainStats = navMap.buildTerrainGrid(this.heightCalculator);

        // Step 2: Add obstacles from chunk objects (PRIMARY METHOD)
        let obstacleStats = { cylindrical: 0, rectangular: 0, total: 0 };
        const objects = this.chunkObjects.get(chunkKey);
        if (objects) {
            obstacleStats = navMap.addObstaclesFromObjectList(objects, GAME_CONFIG.CONSTRUCTION.GRID_DIMENSIONS);
        }

        // Step 3: Populate object metadata from chunkObjects
        let objectCount = 0;
        if (objects) {
            for (const obj of objects) {
                const objectId = obj.userData?.objectId;
                const objectType = obj.userData?.modelType;
                const quality = obj.userData?.quality;

                if (objectId && objectType) {
                    navMap.objectMetadata.set(objectId, {
                        type: objectType,
        ...
        // Step 4: Add roads from tracked positions
        let roadCount = 0;
        const roads = this.chunkRoads?.get(chunkKey);
        if (roads) {
            for (const roadPos of roads) {
                navMap.addRoad(roadPos[0], roadPos[2], 1.0);
                roadCount++;
            }
        }

        // Step 5: Register with NavigationManager
        this.navigationManager.addChunk(chunkId, navMap);
    ...
    /**
     * Paint vertex colors for road placement
     * @param {Array} roadPosition - [x, y, z] world position of road center
     */
    paintRoadVertices(roadPosition) {
        const roadX = roadPosition[0];
        const roadZ = roadPosition[2];
        const roadRadius = 1.0;  // Total influence radius
        const solidRadius = 0.5;  // Solid road center radius
        const chunkSize = CONFIG.TERRAIN.chunkSize;

        // Pre-calculate squared radii to avoid sqrt in tight loop
        const roadRadiusSquared = roadRadius * roadRadius;
        const solidRadiusSquared = solidRadius * solidRadius;

        // Track road for nav map building
        const { chunkX, chunkZ } = ChunkCoordinates.worldToChunk(roadX, roadZ);
        const roadChunkKey = `${chunkX},${chunkZ}`;

        if (!this.chunkRoads.has(roadChunkKey)) {
            this.chunkRoads.set(roadChunkKey, []);
        }
        this.chunkRoads.get(roadChunkKey).push(roadPosition);

        // Calculate which chunks might be affected
        const { chunkX: minChunkX, chunkZ: minChunkZ } = ChunkCoordinates.worldToChunk(roadX - roadRadius, roadZ - roadRadius);
        const { chunkX: maxChunkX, chunkZ: maxChunkZ } = ChunkCoordinates.worldToChunk(roadX + roadRadius, roadZ + roadRadius);
        ...
        // If not all chunks exist, queue this road for later
        if (!allChunksExist) {
            this.pendingRoads.push(roadPosition);
            return;
        }
        ...
        for (let cx = minChunkX; cx <= maxChunkX; cx++) {
            for (let cz = minChunkZ; cz <= maxChunkZ; cz++) {
                const chunkKey = `${cx},${cz}`;  // Use comma format like the rest of terrain.js
                const chunk = this.chunkMap.get(chunkKey);
                ...
                if (!colors) continue;  // Skip if no color attribute
                ...
                for (let i = 0; i < vertexCount; i++) {
                    const vx = positions[i * 3];      // X position
                    const vz = positions[i * 3 + 2];  // Z position

                    // Calculate squared distance from road center (avoid sqrt)
                    const dx = vx - roadX;
                    const dz = vz - roadZ;
                    const distSquared = dx * dx + dz * dz;

                    // Early exit using squared distance (most vertices fail this check)
                    if (distSquared <= roadRadiusSquared) {
                        let roadWeight = 0;

                        if (distSquared <= solidRadiusSquared) {
                            // Solid road center - no sqrt needed
                            roadWeight = 1.0;
                        } else {
                            // Blend edge - only NOW do we need the actual distance
                            const dist = Math.sqrt(distSquared);
                            roadWeight = 1.0 - ((dist - solidRadius) / (roadRadius - solidRadius));
                        }

                        // Use MAX to merge overlapping roads naturally
                        const currentWeight = colorArray[i * 3];  // R channel
                        if (roadWeight > currentWeight) {
                            colorArray[i * 3] = roadWeight;  // Update R channel
                            needsUpdate = true;
                        }
                    }
                }
    ...
    /**
     * Process any pending roads that were waiting for chunks to be created
     */
    processPendingRoads() {
        if (this.pendingRoads.length === 0) return;

        // Process all pending roads
        const roadsToProcess = [...this.pendingRoads];
        this.pendingRoads = []; // Clear the queue

        for (const roadPosition of roadsToProcess) {
            this.paintRoadVertices(roadPosition);
        }
    }

    processVertexUpdateQueue(playerPos = null, frameStartTime = null) {
        if (this.pendingVertexUpdates.length === 0) {
            return false; // Nothing to process
        }

        // Check frame budget - skip if we're already using too much time this frame
        if (frameStartTime) {
            const elapsed = performance.now() - frameStartTime;
            if (elapsed > 10) {  // If we've used 10ms already, skip terrain this frame
                return false;
            }
        }
        ...
        if (playerPos) {
            ...
            const { chunkX: playerChunkX, chunkZ: playerChunkZ } = ChunkCoordinates.worldToChunk(playerPos.x, playerPos.z);
        ...
        if (endIndex >= results.length) {
            ...
            mesh.frustumCulled = true;  // Enable frustum culling to skip rendering off-screen chunks
            ...
            // Process any pending terrain leveling operations for this chunk
            this.processPendingTerrainLeveling();

            // Process any pending roads that were waiting for this chunk
            this.processPendingRoads();
    ...
    /**
     * Get positions from neighboring chunk objects near boundaries
     * Used for minimum distance checking across chunk borders
     */
    getNeighborPositions(chunkX, chunkZ) {
        ...
        const maxCheckDistance = 2.5; // Slightly larger than max minDistance (2.0)
        ...
        for (const [nx, nz] of neighbors) {
            ...
            const neighborObjects = this.chunkObjects.get(neighborKey);

            if (!neighborObjects) continue;

            // Extract positions from objects near the shared boundary
            for (const obj of neighborObjects) {
                const pos = obj.position;
                const userData = obj.userData;

                // Calculate distance to the boundary of the current chunk
                const distToChunk = this.getDistanceToChunkBoundary(pos.x, pos.z, chunkX, chunkZ, chunkSize);

                // Only include objects within maxCheckDistance of the boundary
                if (distToChunk <= maxCheckDistance) {
                    // Get minDistance from MODEL_CONFIG if available
    ...
    getMinDistanceForModel(modelType) {
        // Import MODEL_CONFIG to check minDistance
    ...
    processObjectGenerationQueue() {
        ...
        // Gather positions from neighboring chunks for boundary checking
        const neighborPositions = this.getNeighborPositions(alignedChunkX, alignedChunkZ);

        // Build set of objects already added from server data
        const addedObjectIds = new Set();
        const existingObjectsForCheck = this.chunkObjects.get(key) || [];
        existingObjectsForCheck.forEach(obj => {
        ...
        const trees = generateChunkObjects(
            this.scene,
            ...
            removedObjectIds,
            addedObjectIds, // Pass objects already added from server
            neighborPositions, // Pass neighbor positions for minimum distance checking
            this.gameState // Pass gameState for physics radius check
        );

        // Merge with existing objects (e.g., server-added logs)
        const existingObjects = this.chunkObjects.get(key) || [];
        const allObjects = [...existingObjects, ...trees];
        this.chunkObjects.set(key, allObjects);

        // Add newly generated objects to objectRegistry for fast lookups
        if (window.game && window.game.objectRegistry && trees.length > 0) {
            trees.forEach(obj => {
                if (obj.userData?.objectId) {
        ...
        // Flush TreeInstanceManager updates to GPU (push batched instances to render)
        if (window.game && window.game.treeInstanceManager && trees.length > 0) {
            window.game.treeInstanceManager.flushUpdates();
        }

        // Mark this chunk as needing a navigation map (will be built when idle)
        this.chunksNeedingNavMaps.add(key);
    ...
    // Convert world coordinates to chunk coordinates using CENTER-BASED system
    const { chunkX: gridX, chunkZ: gridZ } = ChunkCoordinates.worldToChunk(chunkX, chunkZ);
    // Get the aligned world position for this chunk
    const chunkCenter = ChunkCoordinates.getChunkCenter(gridX, gridZ);
    const alignedChunkX = chunkCenter.centerX;
    const alignedChunkZ = chunkCenter.centerZ;
    ...
    if (this.geometryPool.length > 0) {
        ...
        // Reset vertex colors for reused geometry
        const colors = geometry.attributes.color;
        if (colors) {
            colors.array.fill(0);  // Reset all colors to 0 (no roads)
        }
    } else {
        ...
        // Add vertex color attribute for road blending
    ...
    // gridX and gridZ are already defined above from ChunkCoordinates.worldToChunk()
    ...
    disposeChunk(key, physicsManager = null) {
        const chunk = this.chunkMap.get(key);
        ...
        if (chunk) {
            ...
            const trees = this.chunkObjects.get(key);
            const treeCount = trees ? trees.length : 0;

            this.scene.remove(chunk.mesh);

            // Remove procedural objects AND their physics colliders
            if (trees) {
                // Remove smoke effects for any campfires in this chunk
                if (window.game) {
                    trees.forEach(obj => {
                        if (obj.userData.modelType === 'campfire' && obj.userData.objectId) {
                            window.game.removeCampfireSmoke(obj.userData.objectId);
                        }
                    });
                }

                // Remove objects from objectRegistry
                if (window.game && window.game.objectRegistry) {
                    trees.forEach(obj => {
                        if (obj.userData.objectId) {
                ...
                removeObjects(this.scene, trees, physicsManager);
            ...
            // Remove navigation map from NavigationManager
            if (this.navigationManager) {
                const chunkId = `chunk_${key}`;
                this.navigationManager.removeChunk(chunkId);
            }
        }

        // Remove from pending nav map queue if it's there
        this.chunksNeedingNavMaps.delete(key);
    }
    ...
    /**
     * Level terrain for structure placement
     * @param {number} centerX - World X coordinate of structure center
     * @param {number} centerZ - World Z coordinate of structure center
     * @param {number} width - Structure width
     * @param {number} depth - Structure depth
     * @param {number} targetHeight - Target height to level to
     * @param {string} structureType - Type of structure being placed
     * @param {number} rotation - Rotation in degrees
     */
    levelTerrainForStructure(centerX, centerZ, width, depth, targetHeight, structureType, rotation) {
        const chunkSize = CONFIG.TERRAIN.chunkSize;
        const segments = CONFIG.TERRAIN.segments;

        // Calculate the area affected (rectangular flat area + 1 unit smooth transition on all sides)
        const halfWidth = width / 2; // Half of structure width
        const halfDepth = depth / 2; // Half of structure depth
        const transitionSize = 1.0; // Transition zone extends 1 unit beyond structure edges

        // Convert rotation to radians
        // Negate rotation to match coordinate system
        const rotationRad = -rotation * Math.PI / 180;
        const cosR = Math.abs(Math.cos(rotationRad));
        const sinR = Math.abs(Math.sin(rotationRad));

        // Calculate axis-aligned bounding box of the rotated rectangle
        // This is needed for chunk detection in world coordinates
        const boundingHalfWidth = halfWidth * cosR + halfDepth * sinR;
        const boundingHalfDepth = halfWidth * sinR + halfDepth * cosR;

        // Find all chunks that might be affected
        // Note: Chunks are CENTERED at their grid positions, not aligned to them
        // Chunk 0,0 spans from -25 to +25, not 0 to 50
        // So we need to shift coordinates by chunkSize/2 before dividing
        const halfChunk = chunkSize / 2;
        const minChunkX = Math.floor((centerX - boundingHalfWidth - transitionSize + halfChunk) / chunkSize);
        const maxChunkX = Math.floor((centerX + boundingHalfWidth + transitionSize + halfChunk) / chunkSize);
        const minChunkZ = Math.floor((centerZ - boundingHalfDepth - transitionSize + halfChunk) / chunkSize);
        const maxChunkZ = Math.floor((centerZ + boundingHalfDepth + transitionSize + halfChunk) / chunkSize);

        // Track vertices that need normal recalculation
        const modifiedVertices = new Map(); // Map of "chunkKey" -> Set of vertex indices

        // Debug: Log which chunks will be affected
        const affectedChunkCount = (maxChunkX - minChunkX + 1) * (maxChunkZ - minChunkZ + 1);
        ...
        // If chunks aren't loaded yet, queue this operation for later
        if (!allChunksLoaded) {
            this.pendingTerrainLeveling.push({
                centerX, centerZ, width, depth, targetHeight, structureType, rotation
            });
            return;
        }
        ...
        for (let cX = minChunkX; cX <= maxChunkX; cX++) {
            for (let cZ = minChunkZ; cZ <= maxChunkZ; cZ++) {
                ...
                if (!chunk || !chunk.geometry) {
                    console.warn(`Terrain leveling: Chunk ${chunkKey} not loaded (this shouldn't happen)`);
                    continue;
                }


                // Always use the mesh's geometry for rendering updates, not the stored reference
                const actualGeometry = chunk.mesh && chunk.mesh.geometry ? chunk.mesh.geometry : chunk.geometry;
                const position = actualGeometry.attributes.position;
                const normal = actualGeometry.attributes.normal;
                const chunkWorldX = cX * chunkSize;
                const chunkWorldZ = cZ * chunkSize;

                // Track modified vertices for this chunk
                const modifiedInChunk = new Set();

                // Process each vertex in the chunk
                for (let z = 0; z <= segments; z++) {
                    for (let x = 0; x <= segments; x++) {
                        // Calculate world position of this vertex
                        const localX = (x / segments - 0.5) * chunkSize;
                        const localZ = (z / segments - 0.5) * chunkSize;
                        const worldX = chunkWorldX + localX;
                        const worldZ = chunkWorldZ + localZ;

                        // Transform vertex to structure's local coordinate system
                        // Get vertex position relative to structure center
                        const relX = worldX - centerX;
                        const relZ = worldZ - centerZ;

                        // Rotate backwards to structure's local space (inverse rotation)
                        const cosRInv = Math.cos(-rotationRad);
                        const sinRInv = Math.sin(-rotationRad);
                        const structLocalX = relX * cosRInv - relZ * sinRInv;
                        const structLocalZ = relX * sinRInv + relZ * cosRInv;

                        // Calculate distance from rectangle edge in structure's local space
                        const distX = Math.abs(structLocalX);
                        const distZ = Math.abs(structLocalZ);

                        // Calculate distance from rectangle edge (Chebyshev distance)
                        // Negative = inside rectangle, 0 = on edge, positive = outside
                        const edgeDist = Math.max(distX - halfWidth, distZ - halfDepth);

                        const vertexIndex = z * (segments + 1) + x;

                        if (edgeDist <= 0) {
                            // Inside structure footprint - completely flat
                            const beforeHeight = position.array[vertexIndex * 3 + 1];
                            position.array[vertexIndex * 3 + 1] = targetHeight;

                            modifiedInChunk.add(vertexIndex);
                        } else if (edgeDist <= transitionSize) {
                            // In smooth transition zone
                            const currentHeight = position.array[vertexIndex * 3 + 1];

                            // Calculate smooth falloff (using smoothstep function)
                            const t = edgeDist / transitionSize; // Normalized distance (0 to 1)
                            const smoothT = t * t * (3.0 - 2.0 * t); // Smoothstep interpolation

                            // Blend between target height and original height
                            const newHeight = targetHeight * (1.0 - smoothT) + currentHeight * smoothT;
                            position.array[vertexIndex * 3 + 1] = newHeight;
                            modifiedInChunk.add(vertexIndex);
                        }
                    }
                }

                // Store modified vertices for normal recalculation
                if (modifiedInChunk.size > 0) {
                    modifiedVertices.set(chunkKey, modifiedInChunk);

                    // Try updating both the stored geometry and the mesh's geometry
                    position.needsUpdate = true;

                    // Also try updating through the mesh directly
                    if (chunk.mesh && chunk.mesh.geometry && chunk.mesh.geometry.attributes.position) {
        ...
        // Recalculate normals for all modified vertices
        modifiedVertices.forEach((vertexSet, chunkKey) => {
            const chunk = this.chunkMap.get(chunkKey);
            if (!chunk || !chunk.geometry) return;

            // Always use the mesh's geometry for rendering updates
            const actualGeometry = chunk.mesh && chunk.mesh.geometry ? chunk.mesh.geometry : chunk.geometry;
            const position = actualGeometry.attributes.position;
            const normal = actualGeometry.attributes.normal;
            ...
            // Recalculate normals for modified vertices
            vertexSet.forEach(vertexIndex => {
                const z = Math.floor(vertexIndex / (segments + 1));
                const x = vertexIndex % (segments + 1);

                // Get the vertex position
                const vx = position.array[vertexIndex * 3];
                const vy = position.array[vertexIndex * 3 + 1];
                const vz = position.array[vertexIndex * 3 + 2];

                // Sample neighboring vertices for normal calculation
                const stepSize = chunkSize / segments;
                const samples = [];

                // Get heights of neighboring vertices (handling chunk boundaries)
                if (x > 0) {
                    const leftIndex = z * (segments + 1) + (x - 1);
                    samples.push({
                        dx: -stepSize,
                        dz: 0,
                        height: position.array[leftIndex * 3 + 1]
                    });
                }
                if (x < segments) {
                    const rightIndex = z * (segments + 1) + (x + 1);
                    samples.push({
                        dx: stepSize,
                        dz: 0,
                        height: position.array[rightIndex * 3 + 1]
                    });
                }
                if (z > 0) {
                    const topIndex = (z - 1) * (segments + 1) + x;
                    samples.push({
                        dx: 0,
                        dz: -stepSize,
                        height: position.array[topIndex * 3 + 1]
                    });
                }
                if (z < segments) {
                    const bottomIndex = (z + 1) * (segments + 1) + x;
                    samples.push({
                        dx: 0,
                        dz: stepSize,
                        height: position.array[bottomIndex * 3 + 1]
                    });
                }
                ...
                if (samples.length >= 2) {
                    ...
                    samples.forEach(sample => {
                        if (sample.dx !== 0) {
                            gradX += (sample.height - vy) / sample.dx;
                            countX++;
                        }
                        if (sample.dz !== 0) {
                            gradZ += (sample.height - vy) / sample.dz;
                            countZ++;
                        }
                    });

                    if (countX > 0) gradX /= countX;
                    if (countZ > 0) gradZ /= countZ;
                ...
                // Update normal
                normal.array[vertexIndex * 3] = nx;
                normal.array[vertexIndex * 3 + 1] = ny;
                normal.array[vertexIndex * 3 + 2] = nz;
            });

            normal.needsUpdate = true;

            // Also update through mesh directly
            if (chunk.mesh && chunk.mesh.geometry && chunk.mesh.geometry.attributes.normal) {
                chunk.mesh.geometry.attributes.normal.needsUpdate = true;
            }

            // Compute bounding sphere on the actual geometry we modified
            actualGeometry.computeBoundingSphere();

            // Update height texture data if it exists
            if (chunk.heightTextureData && chunk.textureSize) {
                const textureSize = chunk.textureSize;
                ...
                vertexSet.forEach(vertexIndex => {
                    ...
                    // Map vertex to texture coordinates
                    const texX = Math.round((x / segments) * (textureSize - 1));
                    const texZ = Math.round((z / segments) * (textureSize - 1));
                    const texIndex = texZ * textureSize + texX;

                    // Normalize height to 0-1 range (matching worker normalization)
                    const height = position.array[vertexIndex * 3 + 1];
                    const minHeight = -10;
                    const maxHeight = 80;
                    const normalizedHeight = (height - minHeight) / (maxHeight - minHeight);

                    chunk.heightTextureData[texIndex] = Math.max(0, Math.min(1, normalizedHeight));
                });
            }
        });

        // Update edge cache for affected chunk boundaries
        modifiedVertices.forEach((vertexSet, chunkKey) => {
            const [cX, cZ] = chunkKey.split(',').map(Number);

            // Clear edge cache for this chunk and its neighbors
            this.edgeCache.delete(`${cX},${cZ},top`);
            this.edgeCache.delete(`${cX},${cZ},bottom`);
            this.edgeCache.delete(`${cX},${cZ},left`);
            this.edgeCache.delete(`${cX},${cZ},right`);
            this.edgeCache.delete(`${cX},${cZ-1},bottom`);
            this.edgeCache.delete(`${cX},${cZ+1},top`);
            this.edgeCache.delete(`${cX-1},${cZ},right`);
            this.edgeCache.delete(`${cX+1},${cZ},left`);
        });


        // Final verification - check if our modifications are still there
        modifiedVertices.forEach((vertexSet, chunkKey) => {
            const chunk = this.chunkMap.get(chunkKey);
            if (chunk && chunk.geometry) {
                // Check the actual mesh geometry that gets rendered
                const actualGeometry = chunk.mesh && chunk.mesh.geometry ? chunk.mesh.geometry : chunk.geometry;
                const position = actualGeometry.attributes.position;
                // Sample the first modified vertex
                const firstVertex = vertexSet.values().next().value;
                const height = position.array[firstVertex * 3 + 1];
            }
        });
    }

    /**
     * Process pending terrain leveling operations
     * Called after chunks are created to apply queued operations
     */
    processPendingTerrainLeveling() {
        if (this.pendingTerrainLeveling.length === 0) return;


        // Take a copy of pending operations and clear the queue
        const operationsToProcess = [...this.pendingTerrainLeveling];
        this.pendingTerrainLeveling = [];

        // Process all operations
        // Operations that can't be processed yet will be re-queued by levelTerrainForStructure
        for (const op of operationsToProcess) {
            this.levelTerrainForStructure(
                op.centerX,
                op.centerZ,
                op.width,
                op.depth,
        ...
        if (this.pendingTerrainLeveling.length > 0) {
        }
    }
