...
export class SimpleTerrainRenderer {
    ...
    processObjectGenerationQueue() {
        ...
        console.log('[DEBUG] terrain.js generating objects for chunk', key, 'removedObjectIds:', removedObjectIds ? removedObjectIds.size : 'undefined');
    ...
    levelTerrainForStructure(centerX, centerZ, width, depth, targetHeight, structureType, rotation) {
        ...
        console.log(`\n=== Terrain Leveling for ${structureType} ===`);
        console.log(`Structure center: (${centerX.toFixed(1)}, ${centerZ.toFixed(1)})`);
        console.log(`Structure size: ${width}x${depth}, rotation: ${rotation}°`);
        console.log(`Bounding box: ${(boundingHalfWidth * 2).toFixed(1)}x${(boundingHalfDepth * 2).toFixed(1)}`);
        console.log(`Affected chunks: X[${minChunkX} to ${maxChunkX}], Z[${minChunkZ} to ${maxChunkZ}]`);
        const affectedChunkCount = (maxChunkX - minChunkX + 1) * (maxChunkZ - minChunkZ + 1);
        console.log(`Total chunks to process: ${affectedChunkCount}`);
        ...
        if (!allChunksLoaded) {
            console.log(`Terrain leveling: Queueing for ${structureType} - chunks not loaded yet`);
        ...
        for (let cX = minChunkX; cX <= maxChunkX; cX++) {
            for (let cZ = minChunkZ; cZ <= maxChunkZ; cZ++) {
                ...
                // Debug: Check if geometry references match
                console.log(`\n=== Chunk ${chunkKey} Geometry Check ===`);
                console.log('Geometry ID:', chunk.geometry.id);
                console.log('Mesh geometry ID:', chunk.mesh.geometry.id);
                console.log('Are they same?', chunk.geometry === chunk.mesh.geometry);
                ...
                for (let z = 0; z <= segments; z++) {
                    for (let x = 0; x <= segments; x++) {
                        ...
                        if (edgeDist <= 0) {
                            ...
                            // Debug first vertex modification in detail
                            if (modifiedInChunk.size === 0) {
                                console.log(`First vertex in ${chunkKey}:`);
                                console.log(`  Before: ${beforeHeight.toFixed(3)}`);
                                console.log(`  After: ${targetHeight.toFixed(3)}`);
                ...
                if (modifiedInChunk.size > 0) {
                    console.log(`Modified ${modifiedInChunk.size} vertices in chunk ${chunkKey}`);
                    ...
                    if (chunk.mesh && chunk.mesh.geometry && chunk.mesh.geometry.attributes.position) {
                        ...
                        console.log('Set needsUpdate on MESH geometry directly');
                    }
                }
            }
        }
        ...
        modifiedVertices.forEach((vertexSet, chunkKey) => {
            ...
            if (chunk.mesh && chunk.mesh.geometry && chunk.mesh.geometry.attributes.normal) {
                ...
                console.log(`Updated normals for chunk ${chunkKey} through mesh`);
            }

            // Compute bounding sphere on the actual geometry we modified
            actualGeometry.computeBoundingSphere();
            console.log(`Computed bounding sphere for chunk ${chunkKey}`);
        ...
        console.log(`Leveled terrain for ${structureType} at (${centerX.toFixed(1)}, ${targetHeight.toFixed(1)}, ${centerZ.toFixed(1)})`);
        ...
        modifiedVertices.forEach((vertexSet, chunkKey) => {
            ...
            if (chunk && chunk.geometry) {
                ...
                console.log(`Chunk ${chunkKey} first modified vertex height: ${height.toFixed(3)} (target was ${targetHeight.toFixed(3)})`);
            }
        });
    }
    ...
    processPendingTerrainLeveling() {
        ...
        console.log(`Processing ${this.pendingTerrainLeveling.length} pending terrain leveling operations`);
        ...
        if (this.pendingTerrainLeveling.length > 0) {
            console.log(`${this.pendingTerrainLeveling.length} terrain leveling operations still pending`);
        }
    }
