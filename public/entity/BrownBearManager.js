...
class BrownBearVisual {
    ...
    update(deltaTime) {
        ...
        if (isAttackPlaying) {
            ...
            // Wandering uses walk animation, chasing/returning uses run
            if (this.isWandering && this.walkAction) {
                // Wandering - use walk animation
                ...
                // Chasing/returning - use run animation
    ...
    async _loadModel() {
        ...
        try {
            ...
            for (const spawn of this.pendingSpawns) {
                ...
                if (!entity || entity.controller) continue;
                ...
                if (visual) {
                    ...
                    // FIX: Register name tag now that mesh exists (was missing, unlike DeerManager)
