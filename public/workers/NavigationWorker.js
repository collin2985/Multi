...
function getSuccessors(node, goalX, goalZ, ignoreSlopes) {
    ...
    if (successors.length === 0) {
        ...
        for (const [fdx, fdz] of fallbackDirs) {
            ...
            if (fdx !== 0 && fdz !== 0) {
                if (!isWalkable(x + fdx * CELL_SIZE, z, ignoreSlopes) ||
                    !isWalkable(x, z + fdz * CELL_SIZE, ignoreSlopes)) {
                    continue;
                }
            }
            if (isWalkable(nx, nz, ignoreSlopes)) {
 ...
 */
function hasLineOfSight(x1, z1, x2, z2, ignoreSlopes) {
    const dx = x2 - x1;
    const dz = z2 - z1;
    ...
    for (let i = 1; i < steps; i++) {
        ...
        const x = x1 + dx * t;
        const z = z1 + dz * t;
        if (!isWalkable(x, z, ignoreSlopes)) {
            return false;
        }
    }
    return true;
 ...
 */
function smoothPath(path, ignoreSlopes) {
...
self.onmessage = function(e) {
    ...
    switch (type) {
        ...
        case 'find_path': {
            ...
            let path = findPath(startX, startZ, goalX, goalZ, { maxIterations, ignoreSlopes });

            // Smooth path to remove unnecessary waypoints (string-pulling)
            if (path && path.length > 2) {
                path = smoothPath(path, ignoreSlopes);
            }
