...
export class ChunkManager {
    ...
    createNavMapForChunk(gridX, gridZ) {
        ...
        // Build terrain grid (walkability, slopes, surface types)
        const terrainStats = navMap.buildTerrainGrid(this.terrainGenerator, CONFIG.WATER?.LEVEL || 0);

        // Add obstacles from chunk objects if they exist
        const objects = this.chunkObjects.get(chunkKey);
        if (objects && objects.length > 0 && CONFIG.CONSTRUCTION?.GRID_DIMENSIONS) {
            navMap.addObstaclesFromObjectList(objects, CONFIG.CONSTRUCTION.GRID_DIMENSIONS);
        }

        // Add roads from gameState.roads (for speed bonus)
        const roads = this.game.gameState?.roads?.get(chunkKey);
        if (roads && roads.length > 0) {
            for (const road of roads) {
                navMap.addRoad(road.x, road.z, road.radius || 1.0);
            }
        }
