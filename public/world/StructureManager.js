...
export class StructureManager {
    ...
    /**
     * Find valid dock X position near the cursor (PREVIEW MODE - no ship lane check)
    ...
    findValidDockX(mouseX, z, checkShipLane = false) {
        ...
        const SHORE_MIN_HEIGHT = 0.8;  // West end: 0.8 to 1.8
        const SHORE_MAX_HEIGHT = 1.8;  // Allows player to step onto dock
        const WATER_MAX_HEIGHT = -0.5; // East end must be below this
        const SEARCH_RANGE = 5;        // Search ±5 units from cursor
        ...
        for (let offset = 0; offset <= SEARCH_RANGE; offset += 0.5) {
            ...
            for (const sign of [1, -1]) {
                ...
                if (shoreHeight >= SHORE_MIN_HEIGHT &&
                    ...
                    if (checkShipLane) {
                        const shipLaneClear = this.checkShipLane(testX, z);
                        if (shipLaneClear) {
                            validX = testX;
                            break;
                        } else {
    ...
    /**
     * Check if ship lane is clear (cached per Z coordinate)
     * @param {number} x - Dock X position
     * @param {number} z - Dock Z position
     * @returns {boolean} - True if ship lane is clear
     */
    checkShipLane(x, z) {
        const SHIP_LANE_DISTANCE = 100;
        const SHIP_LANE_INTERVAL = 10;   // Check every 10 units (was 5)
        const SHIP_LANE_MIN_DEPTH = -3;

        // Round Z to cache key (ship lane mostly depends on Z)
        const cacheKey = Math.round(z);

        // Check cache
        if (this.shipLaneCache.has(cacheKey)) {
            const cached = this.shipLaneCache.get(cacheKey);
            // Cache is valid if X is close enough (within 10 units)
            if (Math.abs(cached.x - x) < 10) {
                return cached.clear;
            }
        }
        ...
        for (let dist = 10; dist <= SHIP_LANE_DISTANCE; dist += SHIP_LANE_INTERVAL) {
            const laneHeight = heightCalc.getWorldHeight(x + dist, z);
        ...
        // Cache result
        this.shipLaneCache.set(cacheKey, { x, clear: shipLaneClear });

        // Limit cache size
        if (this.shipLaneCache.size > 50) {
    ...
    validateDockPlacement(placement) {
        ...
        // Check 2: Ship lane check (skipped during preview, checked on confirm)
        const shipLaneClear = this.checkShipLane(pos.x, pos.z);
        if (!shipLaneClear) {
            return {
                isValid: false,
                invalidReason: 'Need deep water with no islands 100 units to the east'
            };
        }
