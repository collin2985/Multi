...
export class StructureManager {
    ...
    updateStructurePreview(mouseX, mouseZ, mouseY, placementState, ui) {
        ...
        if (placement.phase === 'position') {
            ...
            if (structure && structure.requiresWater) {
                ...
                // Roads follow exact terrain height + 0.5 offset (no snapping, no 4-corner averaging)
                const terrainHeight = this.terrainRenderer.heightCalculator.calculateHeight(
                    placement.position.x,
                    placement.position.z
                );

                // Add 0.5 offset so circle is visible above terrain
                let roadHeight = terrainHeight + 0.5;

                // Water-level protection (no roads below water)
                if (roadHeight < 1.52) {  // 1.02 water + 0.5 offset
                    roadHeight = 1.52;
                }
    ...
    updateStructurePreviewColors(placement, ui) {
        ...
        // Roads and campfires show immediate validation colors
        const isRoadOrCampfire = placement.structure && (placement.structure.type === 'road' || placement.structure.type === 'campfire');
        const isRoad = placement.structure && placement.structure.type === 'road';
        const targetMesh = isRoad
            ? placement.previewBox.userData.circleMesh
            : placement.previewBox.userData.glowOutline;
        ...
        if (placement.phase === 'position') {
            if (isRoadOrCampfire) {
                // Roads and campfires show validation colors immediately (green/red on first click)
                if (placement.isValid) {
                    if (isRoad) {
                        targetMesh.material.color.setHex(0x00ff00);  // Green
                    } else {
                        // Campfire uses glowOutline, update all materials
                    ...
                    if (isRoad) {
                        targetMesh.material.color.setHex(0xff0000);  // Red
                    } else {
                        // Campfire uses glowOutline, update all materials
                        targetMesh.traverse((child) => {
                            if (child instanceof THREE.Mesh && child.material) {
                ...
                // Other structures show white (no validation during position)
                targetMesh.traverse((child) => {
                    if (child instanceof THREE.Mesh && child.material) {
    ...
    validateStructurePlacement(placementState, phase = null, playerPosition = null) {
        ...
        // Special validation for roads and campfires (player distance check)
