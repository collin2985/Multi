...
export class StructureManager {
    ...
    validateStructurePlacement(placementState, phase = null, playerPosition = null) {
        ...
        // Special validation for roads, campfires, tents, walls, and plantable items (player distance check)
        const nearbyOnlyTypes = ['road', 'campfire', 'tent', 'wall', 'planted_pine', 'planted_fir', 'planted_apple', 'planted_vegetables'];
        ...
        const restrictedStructures = ['crate', 'outpost', 'tent', 'house', 'market', 'tileworks', 'ironworks', 'blacksmith', 'bakery', 'gardener', 'miner', 'woodcutter', 'stonemason', 'wall', 'planted_pine', 'planted_fir', 'planted_apple', 'planted_vegetables'];
        ...
        const maxSlopeRad = 37 * Math.PI / 180; // 37 degrees in radians
    ...
    validateWaterPlacement(pos, structure, rotation) {
        ...
        const terrainHeight = Math.max(
            this.terrainGenerator.getWorldHeight(pos.x + cosR * hd - sinR * hw, pos.z - sinR * hd - cosR * hw),
            this.terrainGenerator.getWorldHeight(pos.x + cosR * hd + sinR * hw, pos.z - sinR * hd + cosR * hw),
            this.terrainGenerator.getWorldHeight(pos.x - cosR * hd - sinR * hw, pos.z + sinR * hd - cosR * hw),
            this.terrainGenerator.getWorldHeight(pos.x - cosR * hd + sinR * hw, pos.z + sinR * hd + cosR * hw)
        );
