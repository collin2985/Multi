...
async function initializeServer() {
    ...
    // Load persisted server state (tick, version)
    const serverState = await chunkManager.loadServerState();
    const initialTick = serverState.tick;
    const serverVersion = serverState.version + 1; // Increment version on each startup

    // Save immediately with new version
    await chunkManager.saveServerState(initialTick, serverVersion);
    console.log(`Server state: tick=${initialTick}, version=${serverVersion}`);
    ...
    const messageHandlers = new MessageHandlers(chunkManager, messageRouter, clients, cookingSystem, tileworksSystem, ironworksSystem, blacksmithSystem, bakerySystem, fishermanSystem);
    ...
    return { wss, clients, chunkManager, messageRouter, messageHandlers, notificationInterval, pendingPositionRequests, initialTick, serverVersion };
}

// Start the server
initializeServer().then(({ wss, clients, chunkManager, messageRouter, messageHandlers, notificationInterval, pendingPositionRequests, initialTick, serverVersion }) => {
...
setInterval(() => {
    ...
    if (serverTick % 60 === 0) {
        // Save server state (tick + version) to persist across restarts
        chunkManager.saveServerState(serverTick, serverVersion);
...
wss.on('connection', ws => {
    ...
    ws.send(JSON.stringify({ type: 'welcome', message: 'Welcome to the server!', serverVersion }));
    ...
    ws.on('message', async (message) => {
        ...
        switch (type) {
            ...
            case 'place_wall':
                await messageHandlers.handlePlaceWall(payload);
                break;
