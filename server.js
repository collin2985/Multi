...
// Periodic ban enforcement - kicks players banned via CLI while online
// Runs every 30 seconds, uses O(1) cache lookup per player
setInterval(() => {
    wss.clients.forEach((ws) => {
        if (ws.fingerprintHash && AuthManager.isFingerprintBannedCached(ws.fingerprintHash)) {
            ws.send(JSON.stringify({ type: 'kicked', reason: 'banned' }));
            ws.close();
        }
    });
}, 30000);

console.log('Ban enforcement checker started (every 30 seconds)');
