...
const AISpawnerSystem = require('./server/AISpawnerSystem');
...
const cookingSystem = new CookingSystem(chunkManager, messageRouter);
const aiSpawnerSystem = new AISpawnerSystem(chunkManager, messageRouter, clients);
const messageHandlers = new MessageHandlers(chunkManager, messageRouter, clients, cookingSystem);
...
// Start AI spawner system update loop (every 3 seconds)
const aiSpawnerInterval = setInterval(() => {
    aiSpawnerSystem.update();
}, 3000);
...
wss.on('connection', ws => {
    ...
    ws.on('message', message => {
        ...
        switch (type) {
            ...
            case 'place_road':
                messageHandlers.handlePlaceRoad(payload);
                break;

            case 'place_campfire':
                messageHandlers.handlePlaceCampfire(payload);
                break;
            ...
            case 'ai_death':
                ...
                if (payload.aiId && payload.spawnerId) {
                    aiSpawnerSystem.handleAIDeath(payload.aiId, payload.spawnerId);
                }
                break;
...
wss.on('close', () => {
    ...
    clearInterval(aiSpawnerInterval);
});
