...
**Last Updated:** 2025-12-17 | **Files:** 121 | **Lines:** ~80,000
...
| **Combat Effects** | `public/effects/MuzzleFlash.js`, `public/effects/DirtKickup.js` |
...
| **AI Enemies (Bandits)** | `public/ai/AIController.js` (logic/authority), `public/ai-enemy.js` (visual), `public/entity/AIEnemyManager.js`, `public/core/TickManager.js` (deterministic sync) |
| **Bears** | `public/ai/BearController.js` (behavior), `public/entity/BearManager.js` (spawning) |
| **Deer** | `public/ai/DeerController.js` (behavior), `public/entity/DeerManager.js` (spawning) |
| **Name Tags** | `public/entity/NameTagManager.js` |
| **Tick Sync** | `public/core/TickManager.js` (client), tick broadcast in `server.js` |
| **Pathfinding** | `public/navigation/AStar.js`, `public/navigation/NavigationMap.js` |
...
## Server Files (~8,039 lines)

### server.js (~727 lines)
Main entry point. Initializes WebSocket server on port 8080, creates all modular systems, registers message handlers. Routes client-triggered messages (decay, ship spawns) to handlers.
...
### server/ChunkStore.js (~509 lines)
Chunk data persistence with caching, database fallback to JSON files.
...
### server/DatabaseManager.js (~210 lines)
PostgreSQL connection manager with schema initialization.
...
### AuthClient.js (~297 lines)
Client-side authentication: register, login, logout, session validation.
- **Exports:** `AuthClient`
...
### EffectManager.js (~278 lines)
Visual particle effects (smoke) for campfires, houses, tileworks.
...
### RotationControls.js (~310 lines)
Structure rotation UI component.
...
### BearManager.js (~284 lines)
Bear entity spawning and lifecycle management.
- **Exports:** `BearManager`
- **Interacts with:** BearController, AIEnemyManager patterns
...
## Client: AI System (public/ai/) - ~4,697 lines

Authority-based bandit AI - one client simulates each AI, others interpolate.
...
AIController (behavior) --> AIEnemy (visual)
       |                            |
       | spawning                   | animations
       | pathfinding                | death effects
       | shooting                   | rifle visibility
       | state machine              |
       | authority system           |
       | P2P sync (state broadcast) |
```
...
### BearController.js (~1,678 lines)
Authority-based bear AI with aggressive/fleeing behavior. Similar architecture to AIController.
- **Exports:** `BearController`
...
- **Interacts with:** BearManager, NavigationManager, TickManager
...
## Client: Navigation (public/navigation/) - ~2,570 lines
...
## Client: Spawn (public/spawn/) - ~430 lines

### SpawnSystem.js (~287 lines)
Spawn point calculations with faction zones and friend spawning.
- **Exports:** `SpawnSystem`, `FACTION_ZONES`
...
## Client: Effects (public/effects/) - ~277 lines
...
### SmokeEffect.js (~246 lines)
Particle effects for campfires.
- **Exports:** `SmokeEffect`
