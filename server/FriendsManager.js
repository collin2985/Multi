...
class FriendsManager {
    ...
    async initialize() {
        if (db.isConnected) {
            ...
            console.log('✓ FriendsManager: Using PostgreSQL');
        } else {
            console.log('✓ FriendsManager: Running in local mode');
        }
    }
    ...
    async sendFriendRequest(playerId, targetUsername) {
        ...
        if (!this.useDatabase) {
            ...
            console.log(`Local friend request: ${playerId} -> ${targetUsername}`);
        ...
        try {
            ...
            console.log(`Database friend request: ${playerId} -> ${targetUsername}`);
    ...
    async acceptFriendRequest(playerId, requestId) {
        if (!this.useDatabase) {
            ...
            for (const [senderId, friends] of this.localFriends) {
                ...
                if (request) {
                    ...
                    console.log(`Local friend request accepted: ${requestId}`);
        ...
        try {
            ...
            console.log(`Database friend request accepted: ${requestId}`);
    ...
    async declineFriendRequest(playerId, requestId) {
        if (!this.useDatabase) {
            ...
            for (const [senderId, friends] of this.localFriends) {
                ...
                if (index !== -1) {
                    ...
                    console.log(`Local friend request declined: ${requestId}`);
                    return { success: true };
                }
            }
            return { success: false, message: 'Request not found' };
        ...
        try {
            ...
            console.log(`Database friend request declined: ${requestId}`);
            return { success: true };
    ...
    async removeFriend(playerId, friendId) {
        if (!this.useDatabase) {
            ...
            console.log(`Local friend removed: ${playerId} <-> ${friendId}`);
            return { success: true };
        }
        ...
        try {
            ...
            console.log(`Database friend removed: ${playerId} <-> ${friendId}`);
            return { success: true };
    ...
    isPlayerOnline(playerId) {
        ...
        // Clients map is keyed by clientId, but we need to check accountId
        for (const [clientId, client] of this.connectedClients) {
            if (client.accountId === playerId) {
                // Also check if they have a currentChunk (means they're actually spawned in-game)
