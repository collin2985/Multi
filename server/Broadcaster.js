...
class MessageRouter {
    ...
    /**
     * Send a message to a client by their accountId
    ...
    sendToAccount(accountId, message) {
        for (const [clientId, clientData] of this.clients) {
            if (clientData && clientData.ws && clientData.ws.accountId === accountId &&
                clientData.ws.readyState === WebSocket.OPEN) {
                try {
                    clientData.ws.send(JSON.stringify(message));
                    return true;
                } catch (err) {
                    console.error(`Failed to send ${message.type} to account ${accountId}:`, err);
                    return false;
                }
            }
        }
