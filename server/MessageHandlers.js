...
class MessageHandlers {
    constructor(chunkManager, messageRouter, clients, cookingSystem = null, tileworksSystem = null, ironworksSystem = null, blacksmithSystem = null, bakerySystem = null, fishermanSystem = null) {
    ...
    async handleJoinChunk(ws, payload) {
        ...
        if (existingClient && existingClient.currentChunk) {
            this.chunkManager.removePlayerFromChunk(existingClient.currentChunk, clientId);
        }
        ...
        // Batch-fetch owner factions for artillery targeting (avoids N+1 queries)
        ...
        const enrichedObjectChanges = await Promise.all(objectChanges.map(async obj => {
            ...
            if (isStructure && obj.action === 'add') {
                ...
                // Add owner's faction for artillery targeting
                if (enriched.owner) {
    ...
    async handleReleaseMobileEntity(ws, payload) {
        try {
            ...
            // Preserve inventory from original entity (for artillery, crate, etc.)
            ...
            if (oldChunkId !== newChunkId) {
                ...
                if (entityOwnerId) {
                    entityChange.ownerId = entityOwnerId;
                }
            ...
            try {
                ...
                if (entity) {
                    ...
                    if (playerCurrentChunk && playerCurrentChunk !== chunkId) {
                        const parsed = ChunkCoordinates.parseChunkIdSafe(playerCurrentChunk);
                        if (!parsed) {
                            console.error(`[MobileEntity] Invalid player chunk ID: ${playerCurrentChunk}`);
                            continue;
                        }
    ...
    async handleBuildConstruction(payload) {
        try {
            ...
            // For houses, gardens, campfires, tileworks, ironworks, blacksmith, bakery, fisherman, and markets, initialize inventory (like crates)
            ...
            // Include inventory for houses, gardens, campfires, tileworks, ironworks, blacksmith, bakery, and markets
            if (constructionSite.targetStructure === 'house' ||
                constructionSite.targetStructure === 'garden' ||
                constructionSite.targetStructure === 'campfire' ||
                constructionSite.targetStructure === 'tileworks' ||
    ...
    async handleSaveCrateInventory(ws, payload) {
        try {
            ...
            if (crateIndex !== -1) {
                ...
                const isBurningStructure = structure.name === 'campfire' || structure.name === 'house' || structure.name === 'tileworks' || structure.name === 'ironworks' || structure.name === 'blacksmith' || structure.name === 'bakery';
    ...
    async handleConvertToRuin(ws, payload) {
        try {
            ...
            // When destroyed, remove them entirely (no ruin)
