...
class AuthManager {
    ...
    async addInfluence(playerId, amount) {
        ...
        if (!this.useDatabase) {
            ...
            stats.influence = (stats.influence || 0) + amount;
    ...
    async changeFaction(playerId, newFactionId, chunkManager, preserveOwnership = false) {
        ...
        // Validate faction ID (1=Southguard, 3=Northmen, null=Neutral)
        if (newFactionId !== null && ![1, 3].includes(newFactionId)) {
            return { success: false, message: 'Invalid faction' };
        }
    ...
    async banFingerprint(playerId, reason, adminUsername) {
        ...
        try {
            ...
            console.log(`Fingerprint banned: ${player.username} by ${adminUsername} - ${reason}`);
