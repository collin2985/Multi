...
class AuthManager {
     ...
     */
    async changeFaction(playerId, newFactionId, chunkManager) {
        if (!playerId || playerId.startsWith('session_')) {
        ...
        // Clear ownership of all tents/houses owned by this player
        if (chunkManager) {
            await chunkManager.clearTentHouseOwnership(playerId);
        }

        if (!this.useDatabase) {
            // LOCAL MODE - change faction, clear home, set cooldown
            const data = this.localPlayerData.get(playerId) || {};
            ...
            this.localPlayerData.set(playerId, data);
            console.log(`Local faction change: ${playerId} -> faction ${newFactionId}`);
            return { success: true };
        }
        ...
        try {
            ...
            console.log(`Database faction change: ${playerId} -> faction ${newFactionId}`);
            return { success: true };
        } catch (error) {
