...
class CookingSystem {
    ...
    checkForCookableItems(structureId, chunkId, inventory, structureType = 'campfire') {
        ...
        inventory.items.forEach((item, index) => {
            if (this.canBeCookedItem(item.type)) {
                // Only start cooking if not already cooking (check for cookingStartTick)
                if (!item.cookingStartTick) {
                    console.log(`[COOKING] Starting to cook ${item.type} (${item.id}) in ${structureType}`);
                    this.startCooking(structureId, item.id, index, chunkId, structureType);
                }
            }
        });
    }
    ...
    /**
     * Start cooking an item - stamps tick-based timing on item
    ...
    async startCooking(structureId, itemId, itemIndex, chunkId, structureType = 'campfire') {
        try {
            ...
            if (!item) {
                console.error(`[COOKING] Item ${itemId} not found in structure ${structureId}`);
                return;
            }

            // Don't start if already cooking
            if (item.cookingStartTick) {
                return;
            }
            ...
            // Stamp tick-based cooking info
            const durationTicks = this.getProcessingDurationTicks(item.type, structureType);
            item.cookingStartTick = this.serverTick;
            item.cookingDurationTicks = durationTicks;
            ...
            console.error(`[COOKING] Error starting cooking for item ${itemId}:`, error);
        }
    }
