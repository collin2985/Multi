...
class SpawnTasks {
    ...
    async processShipTrading(dock, dockChunkId) {
        ...
        if (influenceGained > 0 && market.owner && this.authManager) {
            await this.authManager.addInfluence(market.owner, influenceGained);
            console.log(`[Ship Trade] Awarded ${influenceGained} influence to market owner ${market.owner}`);
        }
