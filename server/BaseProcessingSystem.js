...
class BaseProcessingSystem {
    ...
    async startProcessingBatch(structureId, itemIds, chunkId) {
        ...
        try {
            ...
            console.log(`${this.logTag} Started processing ${stampedCount} item(s) at tick ${this.serverTick}`);
    ...
    async completeProcessing(structureId, itemId, chunkId) {
        try {
            ...
            console.log(`${this.logTag} Completed: ${rawItem.type} -> ${processedType} (quality: ${quality}, durability: ${durability})`);
            ...
            if (structureIndex !== -1) {
                ...
                if (structure.inventory && Array.isArray(structure.inventory.items)) {
                    ...
                    if (item && item.processingStartTick) {
                        ...
                        console.log(`${this.logTag} Cancelled processing for ${item.type}`);
                    }
                }
            }
        } catch (error) {
            ...
            for (let i = 0; i < structure.inventory.items.length; i++) {
                ...
                if (ticksElapsed >= item.processingDurationTicks) {
                    ...
                    if (processedType) {
                        ...
                        completedCount++;
                        needsSave = true;
                        console.log(`${this.logTag} NPC completed: ${item.type} -> ${processedType}`);
                    }
                }
            }
